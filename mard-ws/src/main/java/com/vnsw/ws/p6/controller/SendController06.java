package com.vnsw.ws.p6.controller;
...
import com.vnsw.ws.p6.entity.json.SendMessage;
...
import com.vnsw.ws.p6.service.BackendService06;
import com.vnsw.ws.p6.service.EnvelopeService06;
...
@RestController
@RequestMapping("/send/06")
public class SendController06 {
    public static final Logger logger = LoggerFactory.getLogger(SendController06.class);
    private static final String CLASS_NAME = "SendController06";
    ...
    @Autowired
    EnvelopeService06 envelopeService;
    ...
    public ResponseEntity<ResponseJson> sendAll(@RequestBody SendMessage sendMessage) {
        ...
        Envelope06 envelopeSend = null;
        Header06 header = null;
        Body06 body = null;
        ...
        try {
            ...
            String url = environment.getRequiredProperty("URI_BACKEND_ADDRESS") + Constants06.RES_URI.URI_GET_HS_BY_ID + fiIdHoso;
...
//            String isTest = environment.getRequiredProperty("IS_TEST");
            ...
            if (response != null && response.isSuccess()) {
                ...
                Hoso06 hoso06 = mapper.readValue(jsonData, Hoso06.class);
                if (hoso06 != null) {
                    header = envelopeService.createSendHeader(
                            maHoso,
                            Constants.MARD_PRO.MARD06,
                            sendMessage.getType(),
                            sendMessage.getFunction(),
                            "CTY");
                    switch (sendMessage.getType()) {
                        case Constants06.MARD06_TYPE.TYPE_10: // DN gui ho so
                            content.setHoso06(hoso06);
                            ...
                            if (isSuccess && objReturn == null) {
                                ...
                                objReturn.setData(hoso06);
                            }
                            logger.info("Sent something!");
                            break;

                        case Constants06.MARD06_TYPE.TYPE_12: //DN huy ho so
                            DNHuyHS dnHuyHS = new DNHuyHS();
                            dnHuyHS.setFiNSWFileCode(hoso06.getFiNSWFileCode());
                            dnHuyHS.setFiRegistrationComfirmNo(hoso06.getFiNSWFileCode());
                        ...
                        case Constants06.MARD06_TYPE.TYPE_13://DN Yeu cau Huy ho so
                            ...
                            dnYeucauHuyHoso.setFiRegistrationComfirmNo(hoso06.getFiNSWFileCode());
                            dnYeucauHuyHoso.setFiNSWFileCode(hoso06.getFiNSWFileCode());
                        ...
                        case Constants06.MARD06_TYPE.TYPE_15://DN yeu cau sua ho so
                            DNYeucauSuaHoso dnYeucauSuaHoso = new DNYeucauSuaHoso();
                            dnYeucauSuaHoso.setFiReason(sendMessage.getReason());
                            dnYeucauSuaHoso.setFiRegistrationProfile(hoso06);
     ...
     * @throws Exception
     */
    private ResponseJson send(Envelope06 envelopeSend, String signedXml, String fiMaHoSo, String msgType) {
        ...
        String documentType = Constants.MARD_PRO.MARD06;
        ...
        Envelope06 envelopeReturn = convertXmlService.xmlToEnvelope(xml, Envelope06.class);
        if (envelopeReturn != null) {
            if (checkTypeReturn(envelopeReturn)) {
                isSuccess = true;
            } else {
                List<Error06> errors = envelopeReturn.getBody().getContent().getErrorList();
     ...
     * @return
     */
    private boolean checkTypeReturn(Envelope06 envelopeReturn) {
        return envelopeReturn != null && envelopeReturn.getHeader() != null
                && envelopeReturn.getHeader().getSubject() != null && Constants06.MARD06_FUNCTION.FUNCTION_99
