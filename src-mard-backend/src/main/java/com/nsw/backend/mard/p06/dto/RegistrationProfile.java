...
public class RegistrationProfile implements Serializable {
    ...
    public static RegistrationProfile parse(TbdHoso06 regProfile) {
...
//            dtoProduct.setFiPackageUnitName(CmonHelper.instance().findUomByCode(product.getFiPackageUnitCode()).getUnitname());
        ...
        regProfile.getFiProcessingList().forEach(processing -> {
            RegistrationProcessing dtoProcessing = new RegistrationProcessing();
            BeanUtils.copyProperties(processing, dtoProcessing);
            dto.getFiProcessingList().add(dtoProcessing);
        });
        regProfile.getFiExporterCountryList().forEach(country -> {
            RegistrationExporterCountry dtoCountry = new RegistrationExporterCountry();
            BeanUtils.copyProperties(country, dtoCountry);
            dto.getFiExporterCountryList().add(dtoCountry);
        });
        regProfile.getFiLocationQuarantineList().forEach(isoLoc -> {
            RegistrationLocationQuarantine dtoIsoLoc = new RegistrationLocationQuarantine();
            BeanUtils.copyProperties(isoLoc, dtoIsoLoc);
            dto.getFiLocationQuarantineList().add(dtoIsoLoc);
        });
        return dto;
    }
