...
<div class="table-responsive">
	<table class="table table-striped table-bordered table-hover table-checkable order-column small-table" id="sample_1">
		<thead>
		...
		<tr data-bind="if: fiFileGroup == $root.selectedTep()">
			...
			<td><span data-bind="text: fiName"></span><span data-bind="if: fiRequired" class="nsw-require-field">*</span></td>
			<td>
				<div>
					...
					<div data-bind="foreach: { data: $root.teps()[$index()], as: 'item' }">
						<p>
							<i> <a data-bind="text: item.fiFileName, attr: { 'href':  item.link}" target="_blank"></a><span data-bind="if: (!$root.xemHoSo())">
								<c:if test="${isView == false}">
			...
			<td align="center">
				<c:if test="${isView == false}">
				<p>
						<span data-bind="if: ($root.teps()[$index()]().length > 0)"> <a style="" title="<spring:message code="mard.14.form.tepDinhKem.actionDelete" />"  data-bind="click: $root.xoaTepTin.bind($data, $index())"> <i style="color: #337ab7" class="fa fa-trash-o" aria-hidden="true"></i>&nbsp;
						</a>
						</span>
				</p> </span>
				</c:if>
	...
	<div class="alert alert-danger" role="alert">
		<spring:message code="mard.16.uploadInfo"></spring:message>
	</div>
