...
<tbody>
	...
	<tr>
		...
		<td class="td-left"><label class="label-control"><spring:message code="<%= code %>"></spring:message> <c:if test="<%= i < 8 %>">
					<span class="nsw-require-field">(*)</span>
				</c:if></label></td>
		<td class="td-left"><c:if test="<%= form != null && !form.isXemHoSo() && form.getIdHS() > 0 %>">
				<input type="file" id="fileUpload<%= i %>" multiple="multiple" accept=".jpg, .pdf, .doc, .tif" data-bind=" event:{ change: $root.uploadFileChangeEvent.bind($data, <%= i == 8 ? -1 : i %>, 'fileUpload<%= i %>') }">
				</div>
			</c:if>
			<div data-bind="foreach: { data: tepTinModel.tepLoai<%= i %>(), as: 'item' }">
		...
		<td align="center"><c:if test="<%= form != null && !form.isXemHoSo() %>">
				...
				<p data-bind="if: tepTinModel.tepLoai<%= i %>().length > 0">
					<a class="btn red bt-center" title="<spring:message code="monre_01_button_title_delete" />" data-bind="click: xoaTepTin.bind($data, <%= i == 8 ? -1 : i %>)"> <i class="fa fa-trash-o" aria-hidden="true"></i>&nbsp; <spring:message code="monre_01_button_title_delete" />
					</a>
				</p>
			</c:if></td>
	</tr>
