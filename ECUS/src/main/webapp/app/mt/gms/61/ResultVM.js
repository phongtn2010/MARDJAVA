...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/mt/61/giayphep/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdgpLichsu61(res.data) : [];
                    self.historyItems([]);
...
function GiayPhepVM(item) {
    ...
    //Processing checkbox loai hinh phương tiện
    if (item.loaipt61.fiXetai == 1) {
        $("#xetai").prop('checked', true);
    }
    if (item.loaipt61.fiXekhach == 1) {
        $("#xekhach").prop('checked', true);
    }
    if (item.loaipt61.fiXekhac == 1) {
        $("#xekhac").prop('checked', true);
    }
...
function ResultVM(options) {
    ...
    if (data) {
        self.lstGiayPhep(mapTbdgiayphep61(data));
    }
