...
$(document).ready(function () {
    function ViewModel() {
        ...
        var SearchFormModel = function () {
            ...
            this.fiSoVanBan = ko.observable(null).extend({
                validation: [{
                    validator: function (val, lenght) {
                        return !val || val.length <= lenght;
                    },
...
//            })
            ...
            success: function (d) {
                ...
                if (d.success) {
                    ...
                    for (const item of d.data) {
                        self.danhSachTrangThai.push(app.convertObjectToObservable(item, new Tbdhoso()));
                    }
                } else {
                    msg = data.message;
                    fun = 'error';
        ...
        self.searching = function (showMsg) {
            ...
            app.makePost({
                url: '/mt/56/call_service',
                data: JSON.stringify({
                    URL_BACKEND: "/mt/56/hoso/fullSearchPaging",
                    URL_GET_TOTAL:"/mt/56/hoso/getTotalSearch",
                    METHOD: "POST",
        ...
        self.xemGiayPhep = function (item) {
            location.href = app.appContext + '/mt/56/view/license/' + procedureId +'/'+ item.fiIdHoSo()+'/1';
        }

        self.typeForm = ko.observable('search');

        self.suaHoSo = function (item) {
            location.href = app.appContext + '/mt/56/edit/' + procedureId + "/" + item.fiIdHoSo();
        }
        self.xemHoSo = function (item) {
            location.href = app.appContext + '/mt/56/view/' + procedureId + "/" + item.fiIdHoSo();
        }
        self.themMoiHoSo = function (item) {
            location.href = app.appContext + '/mt/56/new/'+procedureId;
        }
                ...
                buttons: [
                    {
                        ...
                        action: function () {
                            app.makePost({
                                url: '/mt/56/call_service',
                                data: JSON.stringify({
                                    URL_BACKEND: "/mt/56/hoso/delete_all",
                                    METHOD: "POST",
        ...
        self.xemLichSu = function (item) {
            var url = '/mt/56/history/' + item.fiIdHoSo()+'/1';
                    ...
                    if (d.success) {
                        ...
                        $('#' + 'lich-su-pagination').pagination({
                            ...
                            onPageClick: function (pageNumber, event) {
                                var url = '/mt/56/history/' + item.fiIdHoSo() + '/' + pageNumber;
                                app.bindData({
                                    ministryCode: 'mt',
                                    code: 'p56',
