...
$(document).ready(function () {
    function ViewModel() {
        ...
        self.dong = function () {
            location.href = app.appContext + '/mt/56/home/' + self.hoSoChiTiet.Tbdhoso.fiLoaiHoSo();
        }
        ...
        self.action = function (item, functionType) {
        	self.hoSoChiTiet.Tbdhoso.fiTenTuyen($("#tuyenvantai option:selected").text());
...
//            console.log('Luu: ', JSON.stringify(app.convertFormObservableJson(self.hoSoChiTiet)));
            ...
            if (self.danhSachFileCheck().length > 0) {
                app.uploadMutipleFile({
                    url: '/mt/56/saveWithFile/' + functionType,
...
/*---- end phương tiện -----*/
            ...
            if (self.hoSoChiTiet.Tbdhoso.fiMaTuyen()) {
                for (const tuyen of self.danhSachTuyen()) {
                    ...
                    if (tuyen.fiMaTuyen() === self.hoSoChiTiet.Tbdhoso.fiMaTuyen()) {
                    	self.getBenXeById(tuyen.fiBenDi(), self.hoSoChiTiet.Tbdhoso.fiBenDi, self.hoSoChiTiet.Tbdhoso.fiTenTinhDi, self.hoSoChiTiet.Tbdhoso.fiMaTinhDi);
                        self.getBenXeById(tuyen.fiBenDen(), self.hoSoChiTiet.Tbdhoso.fiBenDen, self.hoSoChiTiet.Tbdhoso.fiTenTinhDen, self.hoSoChiTiet.Tbdhoso.fiMaTinhDen);
        ...
        self.getTinhById = function (fiMa, func, funcMaTinh) {
            app.makePost({
                url: '/mt/56/call_service',
                data: JSON.stringify({
        ...
        self.getBenXeById = function (fiMaBen, funcGa, funcTinh, funcMaTinh) {
            app.makePost({
                url: '/mt/56/call_service',
                data: JSON.stringify({
                ...
                success: function (d) {
                    ...
                    if (d.success) {
                        if(d.data.length>0){
                            ...
                            self.getTinhById(d.data[0].fiMaTinh, funcTinh, funcMaTinh);
                        }
                    } else {
                        msg = data.message;
                        fun = 'error';
        ...
        self.download = function (item) {
            if (item.fiIdDinhKem()) {
                ...
                app.makeGet({
                    ...
                    url: "/mt/56/download/" + item.fiIdDinhKem(), // my URL
                    responseType: 'blob',
