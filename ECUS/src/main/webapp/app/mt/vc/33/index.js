...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/mt/13/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdlichsu2(res.data, data.currentPage * data.pageSize) : [];
                    self.historyItems([]);
...
function IndexVM(params) {
    ...
    self.search = function (page, pushState) {
        if (!RAW_HS_STATUS || RAW_HS_STATUS.length <= 0) {
            app.getCategory('/mt/13/danhmuc', 'HS_TRANGTHAI', null, function (res) {
                if (res.success) {
    ...
    self.searchAfterHasCatgory = function (page, pushState) {
        ...
        app.makePost({
            url: "/mt/13/hoso/timkiem",
            data: JSON.stringify(data),
            success: function (res) {
                window.stateChanging = false;
                if (res.success) {
                    self.Items([]);
                    var list = res.data ? mapTbdhoso2(res.data, data.currentPage * data.pageSize) : [];
                    self.Items(list);
                    self.paging().update({
                        totalCount: res.data ? res.total : 0,
                        pageSize: self.pageSize(),
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/mt/13/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
...
$(document).ready(function () {
    var d = {};
    $('#loading10').show();
    $.when(app.getCategory('/mt/13/danhmuc', 'HS_TRANGTHAI', null, function (res) {
        if (res.success) {            
