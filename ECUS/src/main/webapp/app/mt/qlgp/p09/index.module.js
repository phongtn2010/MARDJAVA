    ...
    function ViewModel() {
        ...
        self.searching = function (showMsg) {
            ...
            app.makePost({
                url: '/mt/qlgp/09/call_service',
                data: JSON.stringify({
                    URL_BACKEND: "/mt/09/hoso/fullSearchPagingFCAP",
                    URL_GET_TOTAL: "/mt/09/hoso/getTotalSearchFCAP",
                    METHOD: "POST",
        ...
        self.xemGiayPhep = function (item) {
            location.href = app.appContext + '/mt/qlgp/09/view/license/' + procedureId + '/' + item.fiIdHoso();
        }

        self.typeForm = ko.observable('search');

        self.xemHoSo = function (item) {
            location.href = app.appContext + '/mt/qlgp/09/view/' + procedureId + "/" + item.fiIdHoso();
        }
