...
$(document).ready(function () {
    function ViewModel() {
        ...
        var SearchFormModel = function () {
            ...
            this.fiManguoitao = {VALUE: '', OPERATOR: '='};
        ...
        self.searching = function (showMsg) {
            ...
            app.makePost({
                url: '/mt/47/call_service',
                data: JSON.stringify({
                    URL_BACKEND: "/mt/47/hoso/searchPaging",
                    URL_GET_TOTAL: "/mt/47/hoso/getTotalRecord",
                    METHOD: "POST",
        ...
        self.xemGiayPhep = function (item) {
            location.href = app.appContext + '/mt/47/view/license/' + procedureId + '/' + item.fiIdHoso()+'/1';
        }

        self.typeForm = ko.observable('search');

        self.suaHoSo = function (item) {
            location.href = app.appContext + '/mt/47/edit/' + procedureId + "/" + item.fiIdHoso();
        }
        self.xemHoSo = function (item) {
            location.href = app.appContext + '/mt/47/view/' + procedureId + "/" + item.fiIdHoso();
        }
        self.themMoiHoSo = function (item) {
            location.href = app.appContext + '/mt/47/new/' + procedureId;
        }
                ...
                buttons: [
                    {
                        ...
                        action: function () {
                            app.makePost({
                                url: '/mt/47/call_service',
                                data: JSON.stringify({
                                    URL_BACKEND: "/mt/47/hoso/delete_all",
                                    METHOD: "POST",
        ...
        self.xemLichSu = function (item) {
            var url = '/mt/47/history/' + item.fiIdHoso() + '/1';
                ...
                success: function (d) {
                    ...
                    if (d.success) {
                        ...
                        var callback = function (html) {
                            var pop = app.popup({
                                title: NSWLang["sbv_01_lichsh_table_00"] + ' - Mã hồ sơ ' + item.fiMaHoso(),
                                html: html,
                                width: 960,
                                buttons: [
                                    {
                        ...
                        $('#' + 'lich-su-pagination').pagination({
                            ...
                            onPageClick: function (pageNumber, event) {
                                var url = '/mt/47/history/' + item.fiIdHoso() + '/' + pageNumber;
                                app.bindData({
                                    ministryCode: 'mt',
                                    code: 'p47',
