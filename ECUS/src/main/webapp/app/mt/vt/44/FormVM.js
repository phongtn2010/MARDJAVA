...
function XeVM(op) {
    ...
    self.fiBksXeKoHd = ko.observable(xe ? xe.fiBksXeKoHd() : null);
...
function FormVM(options) {
    ...
    self.fiDiemDung = ko.observable(null).extend({
        required: {message: NSWLang["common_msg_formvaild_required"], params: true},
        maxLength: {message: 'Tối đa 255 ký tự', params: 255}
    });
...
//    self.lstCuaKhauDiDen = ko.observableArray(mapCategory(options ? options.lstCuaKhauDiDen : [], 'fiMaCuakhau', 'fiTenCuakhau'));
    ...
    var hosoVG = [self.fiMaCqxl, self.fiTenDn, self.fiDiachiDn, self.fiSoGtVb, self.fiDiachiDn, self.fiSdtDn
        ...
        self.fiMaTinhDen, self.fiBenDen, self.fiKhoangcach, self.fiHanhtrinh, self.fiDiemDung,
    ...
    self.init = function (hoso) {
        ...
        if (hoso !== null) {
            ...
            self.fiDiemDung(hoso.hasOwnProperty('fiDiemDung') ? hoso.fiDiemDung : null);
...
//            self.fiTenCkXn(hoso.hasOwnProperty('fiTenCkXn') ? hoso.fiTenCkXn : null);
            ...
            self.lstXe(mapTbdhsXe44(hoso.hasOwnProperty('lstXe') ? hoso.lstXe : []));
                        ...
                        action: function () {
                            if (self.xeVM.isValid()) {
                                ...
                                if (null == xeInfo.fiIdXe) {
                                    ...
                                    var xeModel = new TbdhsXe44(xeInfo);
    ...
    self.getTuyenInfoOnClick = function () {
        if (self.fiMaTuyen()) {
            app.getCategory('/mt/44/danhmuc', 'HS_TUYENVANTAI_CHITIET', self.fiMaTuyen(), function (d) {
                if (d.success) {
                    var tuyenInfo = d.data;
                    if (tuyenInfo) {
                        console.log(tuyenInfo);
                        ...
                        self.fiDiemDung(tuyenInfo.fiDiemDung);
    ...
    self.toJSON = function () {
        var mapping = {
            'ignore': ["removeCarOnClick", "addCarOnClick", "isValidForm", "init",
                ...
                'getTuyenInfoOnClick', 'errorXeMessage', 'errorDinhKemMessage']
        };
