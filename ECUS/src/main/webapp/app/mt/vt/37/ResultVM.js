...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/mt/37/giayphep/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdgiayphep37(res.data) : [];
                    self.historyItems([]);
...
function ResultVM(options) {
    ...
    if (data) {
        self.lstGiayPhep(mapTbdgiayphep37(data));
    }
    ...
    self.makeRequest = function (d, cb) {
        app.makePost({
            url: '/mt/37/giayphep/chitiet',
            data: JSON.stringify({
                fiIdGp: d.fiIdGp()
            }),
            success: function (d) {
