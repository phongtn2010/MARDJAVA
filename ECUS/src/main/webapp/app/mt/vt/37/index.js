...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/mt/37/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdlichsu37(res.data, data.currentPage * data.pageSize) : [];
                    self.historyItems([]);
...
function IndexVM(params) {
    ...
    self.tenThuTuc = {
        'BGTVT0600037': 'Thủ tục cấp GP vận tải loại A,E; loại B,C,F,G lần đầu trong năm',
        'BGTVT0600038': 'Thủ tục cấp lại giấy phép vận tải loại A, E do hết hạn',
        'BGTVT0600039': 'Thủ tục cấp lại giấy phép vận tải loại B, C, F, G do hết hạn',
        'BGTVT0600040': 'Thủ tục cấp giấy phép vận tải loại B,C,F,G lần thứ hai trở đi trong năm'
    };
    ...
    var propCode = items[items.length - 2];
    ...
    self.search = function (page, pushState) {
        if (!RAW_HS_STATUS || RAW_HS_STATUS.length <= 0) {
            app.getCategory('/mt/37/danhmuc', 'HS_TRANGTHAI', null, function (res) {
                if (res.success) {
    ...
    self.searchAfterHasCatgory = function (page, pushState) {
        ...
        app.makePost({
            url: "/mt/37/hoso/search",
            data: JSON.stringify(data),
            success: function (res) {
                window.stateChanging = false;
                if (res.success) {
                    self.Items([]);
                    var list = res.data ? mapTbdhoso37(res.data, data.currentPage * data.pageSize) : [];
                    self.Items(list);
                    self.paging().update({
                        totalCount: res.data ? res.total : 0,
                        pageSize: self.pageSize(),
    ...
    self.btnAddNewClick = function (e) {
        document.location = app.appContext + '/mt/37/edit?maThuTuc=' + self.maThuTuc();
        return false;
    };
    ...
    self.bXemClick = function (item, e) {
        document.location = app.appContext + '/mt/37/view?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        return false;
    };
    /**
     * Xem Thông báo
    ...
    self.bXemThongBaoClick = function (item, e) {
        document.location = app.appContext + '/mt/37/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=cert&maThuTuc=' + self.maThuTuc();
        return false;
    };
    /**
     * Xem Lich su
    ...
    self.bSuaClick = function (item, e) {
        document.location = app.appContext + '/mt/37/edit?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        return false;
    };
    /**
     * Hủy hồ sơ (xóa)
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/mt/37/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
    ...
    self.guiYcXuly = function (message) {
        $.ajax({
            ...
            cache: false,
            crossDomain: true,
            url: app.appContext + '/mt/37/hoso/xuly',
...
$(document).ready(function () {
    var d = {};
    $('#loading10').show();
    $.when(app.getCategory('/mt/37/danhmuc', 'HS_TRANGTHAI', null, function (res) {
        if (res.success) {
