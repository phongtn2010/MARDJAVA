...
//        }
    ...
    self.btnUploadTemp = function (e) {
        ...
        //khong cho tai file ngoai excel
        debugger;
...
//                                break;
//                            }
//                            obj.fiKieuIn = objKieuIn.fiKieuIn;
                            ...
                            if (obj.fiDonViTocDoIn.trim() != null && obj.fiDonViTocDoIn.trim() != "") {
...
//                        self.lstThietBiNk02.push(tbnkModel);
    ...
    self.btnAddNewNKClick = function () {
        self.popupTBNK(null,null,'Thêm mới thông tin thiết bị')
        return false;
    };
    self.viewPopupTBNK = function (item) {
        self.popupTBNK(item,null,'Xem thông tin thiết bị')
        return false;
    };
    self.btnEditOnClick = function (item) {
        self.popupTBNK(item, 'edit','Sửa thông tin thiết bị');
        return false;
    }
    self.popupTBNK = function (item, action,title) {
        var html = [
...
function TBNKFormVM(options) {
    ...
    self.fiDonViTinhKhuonKho = ko.observable(tbnk ? tbnk.fiDonViTinhKhuonKho() : null);
    ...
    self.fiMaKichThuoc = ko.observable(tbnk ? tbnk.fiMaKichThuoc() : null);
    ...
    var tbnkVG = [self.fiTenMay, self.fiKieuIn, self.fiSoMauIn, self.fiModelMay, self.fiSoDinhDanhMay, self.fiNuocSx, self.fiNamSx,
        self.fiSoLuong, self.fiChatLuong, self.fiKhuanKhoBanIn, self.fiTocDoIn, self.fiGiaThietBi,
        self.fiTenHangSx, self.fiMaTv, self.fiMaChatLuong];
    ...
    //check gia tri thiet bi
    self.checkGiaTri = function(){
        var gttb = self.fiGiaThietBi().toString();
        if(!gttb.includes(".") && gttb.length>15){
            $("#gttb-valid").show();
            return false;
        }else{
            $("#gttb-valid").hide();
    ...
    self.isValidTbnk = function () {
        if (self.TbnkErrors().length > 0 || !self.checkGiaTri()) {
