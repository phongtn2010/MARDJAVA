function TbdThietB03ViewModel(self) {
    ...
    self.titles.push({key: 'fiHinhThucKhac', value: i18nextko.t('TbdThietBi03.fiHinhThucKhac'), align: 'right', required: true, columnType: TABLE_COLUMN_DISPLAY_TEXT});
    ...
    self.addName = function(name) {
        ...
        self.fullDataTable().forEach(function (item, index) {
            ...
            tongBD += item.fiSoLuongBang();
            tongBD += item.fiSoLuongCatset();
            tongBD += item.fiSoLuongDia();
            tongSB += item.fiSoBan();
        });
        ...
        self.tbdHoSo03.fiTongSoXBPham(tongSB);
        //self.tbdHoSo03.fiTongSoBangDia(tongBD);
    }
