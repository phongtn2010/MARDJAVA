...
function HistoryPopupView(item) {
    ...
    self.titles.push({key: 'fiProcessDate', value: i18nextko.t('TbdKetQuaXuLy.ngayXuLy'), dateType: true, dateFormat: 'DD/MM/YYYY HH:mm', columnType: TABLE_COLUMN_DISPLAY_DATE});
    ...
    self.titles.push({key: 'fiLink', value: i18nextko.t('TbdKetQuaXuLy.taiLieu'), icon: 'fa fa-download', columnType: TABLE_COLUMN_DISPLAY_LINK_BLANK});
    ...
    self.load = function(page) {
        ...
        callApi('/mic/03/tbdKetQuaXuLy03/page?fiIdHoSo='+item.fiIdHoSo(), self.pageable, function (d) {
            ...
            if (d.data.content) {
                d.data.content.forEach(function (item) {
                    if (item.fiLink && !item.fiLink.startsWith("http")) {
                        item.fiLink = urlDownload + item.fiLink;
                    }
