...
function SuaGiayPhepPopupViewModel(item) {
    ...
    self.tbdYeuCauSuaGP03 = new TbdYeuCauSuaGP03();
    self.tbdYeuCauSuaGP03.fiNgayYeuCau(new Date());


    callApi('/mic/api/03/giayPhep/findByFiIdHoSo/' + item.fiIdHoSo(), null, function (d) {
        self.dataApi(d.data);
    });
    ...
    self.dataApi.subscribe(function (d) {
        ...
        self.tbdYeuCauSuaGP03.fiIdGiayPhep(d.tbdGiayPhep03DTO.fiId);
    ...
    function layDanhSachTepTinTheoHS(d){
        ...
        ds.forEach(function(t, tIndex){
            $(d.tbdGiayPhep03DTO.tbdYeuCauSuaGP03DTOS).each(function(index, loopItem){
        ...
        uploadTo(files, function (d) {
            if (d.message === "0") {
                ...
                iDFiles.forEach(function (value, index) {
                    ...
                    var tepTin = new TbdDinhKemYCSGP03();
    ...
    self.isValid = function(isGuiHoSo) {
        if (self.tbdYeuCauSuaGP03.valid.errors().length > 0) {
            showError(self.tbdYeuCauSuaGP03);
            return false;
        }
        var check = true;
        if (isGuiHoSo) {
    ...
    self.guiHoSo = function () {
        ...
        callBack(function () {
            if (self.isValid(true)) {
                var p = convertKnockoutToObject(self.tbdYeuCauSuaGP03, createObject(self.tbdYeuCauSuaGP03));
                p.tbdDinhKemYCSGP03DTOS = self.getTepDinhKems();
                callApi('/mic/api/03/tbdYeuCauSuaGP03/create', p, function (d) {
                    var post = {
                        fiReason: self.tbdYeuCauSuaGP03.fiNoiDung(),
                        fiXml: null,
                        fiIdHoSo: self.dataApi().tbdGiayPhep03DTO.fiIdHoSo,
                        fiAction: 3,
