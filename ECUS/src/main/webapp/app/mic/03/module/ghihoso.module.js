...
$(document).ready(function () {
    ...
    function GhiHoSoViewModel() {
        ...
        callApi('/mic/api/03/edit/' + self.idHoSo, null, function (d) {
            ...
            var sum = 0;
            if (d.data.hoSo.tbdThietBi03DTOS) {
                self.tbdHoSo03.fiTongSoXBPham(d.data.hoSo.tbdThietBi03DTOS.length);
                if (d.data.hoSo.tbdThietBi03DTOS) {
                    d.data.hoSo.tbdThietBi03DTOS.forEach(function (item) {
                        sum += item.fiSoLuongBang;
                        sum += item.fiSoLuongCatset;
                        sum += item.fiSoLuongDia;
                    })
                }
              //  self.tbdHoSo03.fiTongSoBangDia(sum);
            }
