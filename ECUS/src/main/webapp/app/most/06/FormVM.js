...
//    self.fiMucDichSuDungKhac = ko.observable(null);
    ...
    self.fiCamKetTraLaiNguon = ko.observable(null).extend({
        required: {message: NSWLang["common_msg_formvaild_required"], params: true}
    });
    ...
    var pxKinVG = [self.fiMaDongViPhongXa, self.fiTenDongViPhongXa, self.fiHangSanXuat, self.fiHoatDo, self.fiHoatDoDonVi, self.fiNgayXacDinhHoatDo, self.fiMaMucDichSuDung, self.fiXuatXuNguon, self.fiXuatXuNguonNgaycap, self.fiXuatXuNguonSgp, self.fiCamKetTraLaiNguon];
...
function FormPxKinQsdVM(options) {
    ...
    self.fiHoatDo = ko.observable(null);
    self.fiHoatDoDonVi = ko.observable(null);
    self.fiTenHoatDoDonVi = ko.observable(null);
    self.fiNgayXacDinhHoatDo = ko.observable(null).extend({
        required: {message: NSWLang["common_msg_formvaild_required"], params: true}
    });
    ...
    var pxKinQsdVG = [self.fiMaDongViPhongXa, self.fiTenDongViPhongXa, self.fiNgayXacDinhHoatDo, self.lstMucdichsudungqsd];
    ...
    self.isValid = function () {
        ...
        if (self.lstMucdichsudungqsd().length <= 0) {
            self.errormucdicpxkqsdMessage('Thông tin bắt buộc nhập');
...
//    self.fiSoGiayPhep = ko.observable(null);
    self.fiSoGiayPhep = ko.observable(null).extend({
        required: {message: NSWLang["common_msg_formvaild_required"], params: true}
    });
    ...
    self.errorNgaydkdenNuoctn = ko.observable(null);
...
//        delete self.xeVM;
        ...
        self.errorNgaydkdenNuoctn(null);
        ...
        //Kiem tra thong tin phong xa kin

        if (!self.lstTbdhsPhieuKbNguonPxk06() || self.lstTbdhsPhieuKbNguonPxk06().length <= 0) {
            self.errorlistpxkMessage('* Chưa khai báo thông tin phóng xạ kín');
            errorpxk = false;
        }

        if (!self.lstTbdhsPhieuNguonPxkQsd06() || self.lstTbdhsPhieuNguonPxkQsd06().length <= 0) {
            self.errorlistpxkqsdMessage('* Chưa khai báo thông tin phóng xạ kín qua sử dụng');
            errorpxkqsd = false;
        }

        if (!self.lstTbdhsPhieuKbNguonPxh06() || self.lstTbdhsPhieuKbNguonPxh06().length <= 0) {
            self.errorlistpxhoMessage('* Chưa khai báo thông tin phóng xạ hở');
            errorpxh = false;
        }
...
//        return errorHoso && errorpxk && errorDinhkem;
        return errorHoso && errorpxk && errorpxkqsd && errorpxh && errorDinhkem;
    };
    //VALIDATE DATA ON FORM
