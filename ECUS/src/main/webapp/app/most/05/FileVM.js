    ...
    self.downloadUrl = ko.computed(function () {
        if (self.fiTenTep()) {
            if (self.fiIdHoso() === null) {
                return app.appContext + '/most/05/file/most/05/' + self.fiGuiid();
            } else {
                return app.appContext + '/most/05/download/' + self.fiGuiid() + '/' + self.fiIdDk();
            }
        }
        return null;
    }, this);
...
//        });
        ...
        var urlToken = "https://vietesoft.com/api/ApiAccount/Login?clientKey=676e8b1b13e894b7bc65c085d120fc25&clientId=NSW&type=json";
        ...
        $.ajax({
            ...
            contentType: false,
            processData: false,
            complete: function (e1, x, s) {
                var data = JSON.parse(e1.responseText);
                console.log(data);
                postFile(files, e, data.data.token);
            },
            error: function (j, t, e1) {
 //               app.Alert('Lỗi Server: ko lấy được token');
                return;
            }
        });
