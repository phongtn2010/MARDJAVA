...
//        self.isValidht();
        ...
        if (!self.lstTbdhsNkpxTtpnpxk05() || self.lstTbdhsNkpxTtpnpxk05().length <= 0) {
            self.errorlistpxkMessage('* Chưa khai báo thông tin phóng xạ kín');
            errorpxk = false;
        }
        //Kiem tra thong tin phong xa ho
        if (!self.lstTbdhsTtpPxh05() || self.lstTbdhsTtpPxh05().length <= 0) {
        ...
        return errorHoso && errorpxk && errorpxh && errorDinhkem
    };
//VALIDATE DATA ON FORM
...
function FormPhongXaKinVM(options) {
    ...
    self.fiCamKetTraNguon = ko.observable(null).extend({
        required: {message: NSWLang["common_msg_formvaild_required"], params: true},
        maxLength: {message: 'Tối đa 1 ký tự', params: 1}
    });
    ...
    var pxkVG = [self.fiMaDongViPhongXa, self.fiTenDongViPhongXa, self.fiHangSanXuat, self.fiHoatDo, self.fiHoatDoDonVi,
        self.lstMucdichsudung, self.fiCamKetTraNguon];
