...
function TbdHanghoaVM(options) {
    ...
    var lstVG = [self.fiMaHh, self.fiKieu, self.fiTenNsx, self.fiMaQg, self.fiCapCx];
...
//                app.Alert('Bạn phải nhập vào lý do sửa hồ sơ.');
                $('#fiLydoDc').parent().append("<span class = 'validationMessage'>Lý do bổ xung không được bỏ trống</span>");
                $('#fiLydoDc').focus();
                haveReason = false;
            }
        }
        ...
        if (self.hangHoas() && self.hangHoas().length > 0) {
            for (var i = 0; i < self.hangHoas().length; i++) {
                ...
                if (!hh.fiTenHh() || !hh.fiTenNsx() || !hh.fiMaQg() || !hh.fiCapCx()) {
                    errorHanghoa = true;
                    break;
                }
            }
