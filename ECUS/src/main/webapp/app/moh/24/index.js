...
function RutHoSoVM(fiMsg, item) {
    ...
    self.makeRequest = function (cb) {
        ...
        app.makePost({
            url: '/moh/18/hoso/yc-rut',
            data: JSON.stringify({
                fiIdHoso: self.fiIdHoso(),
                reason: self.fiContent().trim()
            }),
            success: function (d) {
                var msg = '';
                if (d.success) {
...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/moh/18/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdlichsu18(res.data) : [];
                    self.historyItems([]);
...
function IndexVM(params) {
    ...
    self.title = ko.observable(NSWLang["moh_" + self.fiThuTucId() + "_tenthutuc"]);
    ...
    self.search = function (page, pushState) {
        if (!RAW_HS_STATUS || RAW_HS_STATUS.length <= 0) {
            app.getCategory('/moh/18/danhmuc', 'DANHMUC_TRANGTHAI', null, function (res) {
                if (res.success) {
    ...
    self.searchAfterHasCatgory = function (page, pushState) {
        ...
        app.makePost({
            url: "/moh/18/hoso/timkiem",
            data: JSON.stringify(data),
            success: function (res) {
                window.stateChanging = false;
                if (res.success) {
                    self.Items([]);
                    var list = res.data ? mapTbdhoso18(res.data, data.currentPage * data.pageSize) : [];
                    self.Items(list);
                    self.paging().update({
                        totalCount: res.data ? res.total : 0,
                        pageSize: self.pageSize(),
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/moh/18/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
...
$(document).ready(function () {
    ...
    $('#loading10').show();
    $.when(app.getCategory('/moh/18/danhmuc', 'DANHMUC_TRANGTHAI', null, function (res) {
        if (res.success) {
