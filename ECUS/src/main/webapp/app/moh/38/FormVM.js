...
function FormVM(options) {
    ...
    self.fiMucDichNk = ko.observable(null);
    ...
    self.fiChucvuNgDd = ko.observable(null);
    ...
    self.fiTccl = ko.observable(null);
    ...
    //Danh muc
    self.lstMucDichNhapKhau = ko.observableArray(mapCategory(options ? options.lstMucDichNhapKhau : [], 'id', 'name'));
    ...
    self.lstLoaiDonHang = ko.observableArray(mapCategory(options ? options.lstLoaiDonHang : [], 'id', 'name'));
    self.lstNguyenLieu = ko.observableArray(mapCategory(options ? options.lstNguyenLieu : [], 'fiMaNl', 'fiTenNl'));

    var hosoVG = [self.fiTenDn, self.fiMstDn, self.fiDiachiDn, self.fiMaTinh, self.fiNguoiDd, self.fiSoDk
                , self.fiTenHang, self.fiBaoChe, self.fiDongGoi, self.fiSoLuong, self.fiMaDvtinh, self.fiTenCoSoSx, self.fiMaQgSx, self.lstCuaKhaus];
    ...
    self.init = function (hoso) {
        ...
        if (hoso !== null) {
            ...
            self.fiMstDn(hoso != null && hoso.doanhNghiep.hasOwnProperty('fiMstDn') ? hoso.doanhNghiep.fiMstDn : user.username);
            self.fiTenDn(hoso != null && hoso.doanhNghiep.hasOwnProperty('fiTenDn') ? hoso.doanhNghiep.fiTenDn : user.companyName);
            self.fiDiachiDn(hoso !== null && hoso.doanhNghiep.hasOwnProperty('fiDiachiDn') ? hoso.doanhNghiep.fiDiachiDn : user.companyAddress);
            ...
            self.lstNguyenLieus(mapTbdhsNglieu38(hoso.donHang.hasOwnProperty('lstNguyenLieus') ? hoso.donHang.lstNguyenLieus : []));
    ...
    self.isValidForm = function () {
        ...
        if (self.lstNguyenLieus() && self.lstNguyenLieus().length > 0) {
            for (var i = 0; i < self.lstNguyenLieus().length; i++) {
                var xe = self.lstNguyenLieus()[i];
                if (!xe.fiTongKl() || !xe.fiMaNglieu()) {
                    errorXe = false;
                    break;
                }
            }
...
//        }
    ...
    self.toJSON = function () {
        ...
        for (var i = 0; i < model.lstNguyenLieus.length; i++) {
            var idx = options.lstNguyenLieu.Search('fiMaNl', model.lstNguyenLieus[i].fiMaNglieu);
            if (idx >= 0) {
                lstNguyenLieu.push({
                    'fiMaNglieu': options.lstNguyenLieu[idx].fiMaNl,
                    'fiTenNglieu': options.lstNguyenLieu[idx].fiTenNl,
                    'fiLoaiNglieu': options.lstNguyenLieu[idx].fiLoaiNl,
                    'fiTongKl': model.lstNguyenLieus[i].fiTongKl});
            }
        }

        delete model.lstNguyenLieus;

        model.donHang.lstNguyenLieus = lstNguyenLieu;
