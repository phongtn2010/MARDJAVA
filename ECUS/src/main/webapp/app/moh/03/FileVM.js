...
function FileVM(item, idHoso) {
    ...
    if (idHoso > 0) {
        ...
        for (var i = 0; i <= len - 1; i++) {
            if (item.fiLoaiTailieu == DINHKEMDATA[i].fiLoaiTailieu) {
            ...
            complete: function (data) {
                ...
                if (rs) {
                    var fileObj = {
                        ...
                        fiDuongdanTl:BYT.URL_DOWNLOAD_FILE_CT + rs.FileName,
                        fiSize: fileSize
                    }
    ...
    self.doDownloadFileItem = function (item, e) {
        var url = BYT.URL_DOWNLOAD_FILE_CT + item.fiTenTailieu;
...
function mapFiles03VM(data, idHoso) {
