...
function EditVM(options) {
    ...
    self.formVM().lstDinhKems.subscribe(function (items) {
        ...
        self.fileVM().refreshFileType();
    });
	
	self.formVM().fiLoaiTtb.subscribe(function (val) {
        self.paymentVM().refreshName(val);
    });
    
    self.lstDinhKems = ko.observableArray([]);
    self.lstThanhToans = ko.observableArray([]);
    self.propTitle = ko.observable('HỒ SƠ ĐỀ NGHỊ CẤP MỚI SỐ LƯU HÀNH ĐỐI VỚI TTBYT THUỘC LOẠI B,C VÀ D');
    if(options.maThuTuc == 'BYTE0400009') {
        self.propTitle('CẤP GIẤY PHÉP LƯU HÀNH TỰ DO(CFS) CHO TTBYT');
    }
    ...
    self.btnLuuClick = function () {
        ...
        if(!paymentIsVaild){
            app.Alert('Thông tin xác nhận phí cần được cung cấp đầy đủ.');
            return;       
        }
        */
        ...
        var url = !fiIdHoso || fiIdHoso <= 0 ? '/moh/07/hoso/taomoi' : '/moh/07/hoso/capnhap';
    ...
    self.btnTroLaiClick = function () {
        //History.go(-1);
        document.location = context + '/moh/07/home?maThuTuc=' + options.maThuTuc;
    };
    ...
    window.addEventListener('message', function (event) {        
        ...
        if (d.action == 'onUploadSuccessFromDVCTTBYTE') {
            $("#dialog").dialog("close");            
            if (d.frameId == "frame1") {
                self.fileVM().updateAfterUpload(self.formVM().currentAttach, d);
            } else if (d.frameId == "frame2") {
                self.paymentVM().updateAfterUpload(d);
            } else if (d.frameId == "frame3") {
                if(self.formVM().equipmentVM) {
                    if(self.formVM().equipmentVM.chungLoaiVM) {
                        self.formVM().equipmentVM.chungLoaiVM.updateAfterUpload(self.formVM().currentAttach, d);
                    }
                }
            }
        }
...
$(document).ready(function () {
    ...
    $.when(
            app.getCategory('/moh/07/danhmuc', 'DANHMUC_TINHTHANH', null, function (res) {
                if (res.success) {
            ...
            app.getCategory('/moh/07/danhmuc', 'DANHMUC_COSOSANXUAT', options.maThuTuc, function (res) {
                if (res.success) {
            ...
            app.getCategory('/moh/07/danhmuc', 'DANHMUC_QUOCGIA', null, function (res) {
                if (res.success) {
            ...
            app.getCategory('/moh/07/danhmuc', 'DANHMUC_DONVINHAN', null, function (res) {
                if (res.success) {
            ...
            app.getCategory('/moh/07/danhmuc', 'DANHMUC_DONDK_NHOMTTBYT', options.maThuTuc, function (res) {
                if (res.success) {
                    options.lstNhomTtb = res.data;
                } else {
                    options.lstNhomTtb = [];
                }
            }),
            app.getCategory('/moh/07/danhmuc', 'DANHMUC_DONDK_LOAITTBYT', options.maThuTuc, function (res) {
                if (res.success) {
            ...
            app.getCategory('/moh/07/danhmuc', 'DANHMUC_DONDK_LOAI_HOSO_TTBYT', options.maThuTuc, function (res) {
                if (res.success) {
            ...
            app.getCategory('/moh/07/danhmuc', 'DANHMUC_TTB_NHOM', options.maThuTuc, function (res) {
                if (res.success) {
            ...
            app.getCategory('/moh/07/danhmuc', 'DANHMUC_TTB_PHANNHOM', options.maThuTuc, function (res) {
                if (res.success) {
            ...
            app.getCategory('/moh/07/danhmuc', 'DANHMUC_TTB_LOAI', options.maThuTuc, function (res) {
                if (res.success) {
            ...
            ,
            app.getCategory('/moh/07/danhmuc', 'DANHMUC_TAILIEU', options.maThuTuc + '/1', function (res) {
                if (res.success) {
            ...
            ,
            app.getCategory('/moh/07/danhmuc', 'DANHMUC_TAILIEU_TTB', options.maThuTuc, function (res) {
                if (res.success) {
                    DINHKEM_TTB_DATA = res.data;
                } else {
                    DINHKEM_TTB_DATA = [];
                }
            }) 
    ...
    var init = function () {
        ...
        //options.lstTTBinhKems = DINHKEM_TTB_DATA;
        if(options.maThuTuc == 'BYTE0400009') {//CFS
            DINHKEM_TTB_DATA = [
                {'id': 12, 'name': 'Tài liệu mô tả chức năng, thông số kỹ thuật của TTBYT chung do chủ sở hữu ban hành (tài liệu này ko bắt buộc nhập )', fiBatBuoc: false},
                {'id': 13, 'name': 'Tài liệu hướng dẫn sử dụng của trang thiết bị y tế;', fiBatBuoc: true},
                {'id': 14, 'name': 'Mẫu nhãn sẽ sử dụng khi lưu hành tại Việt Nam của trang thiết bị y tế;', fiBatBuoc: true}
            ];
        } else {//BCD
            DINHKEM_TTB_DATA = [
                {'id': 14, 'name': 'Tài liệu mô tả chức năng, thông số kỹ thuật của TTBYT chung do chủ sở hữu ban hành (tài liệu này ko bắt buộc nhập )', fiBatBuoc: false},
                {'id': 15, 'name': 'Tài liệu hướng dẫn sử dụng của trang thiết bị y tế;', fiBatBuoc: true},
                {'id': 16, 'name': 'Mẫu nhãn sẽ sử dụng khi lưu hành tại Việt Nam của trang thiết bị y tế;', fiBatBuoc: true}
            ];
        }
        options.lstTTBinhKems = DINHKEM_TTB_DATA;
        options.lstLoaiInvitro = [
            {'id': 1, 'name': 'Là Invitro'},
            {'id': 2, 'name': 'Không là Invitro'}
        ];
        options.lstQtxlNhanh = [
            {'id': 0, 'name': 'Xử lý thông thường'},
            {'id': 1, 'name': 'Xử lý nhanh'}
        ]; 
        if (options && options.fiIdHoso > 0) {
            var url = '/moh/07/hoso/t/' + options.fiIdHoso;
            app.makePost({
                url: url,
                data: JSON.stringify({}),
                success: function (d) {
                    if (d.success) {
                        ...
                        app.getCategory('/moh/07/danhmuc', 'DANHMUC_QUANHUYEN', options.hoso.fiMaTinh, function (res) {
                            if (res.success) {
                            ...
                            $("#fiLoaiInvitro").select2({placeholder: '-- Chọn --', width: '100%', allowClear: true});
                            $("#fiQtxlNhanh").select2({placeholder: '-- Chọn --', width: '100%', allowClear: true});
