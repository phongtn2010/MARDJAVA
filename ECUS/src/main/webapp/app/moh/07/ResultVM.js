...
function ResultVM(options) {
    ...
    self.fifiNgaycapText = ko.observable(data != null ? new Date(data.fiNgaycap).toDayFirstString() : null);
    self.fiNgayKyText = ko.observable(data != null ? new Date(data.fiNgayKy).toDayFirstString() : null);
    self.fiHieulucTuText = ko.observable(data != null ? new Date(data.fiHieulucTu).toDayFirstString() : null);
    self.fiHieulucDenText = ko.observable(data != null ? new Date(data.fiHieulucDen).toDayFirstString() : null);
    ...
    if (options && options.fiIdHoso > 0) {
        ...
        app.makePost({
            ...
            success: function (d) {
                if (d.success) {
                    if (d.data != null) {
                        d.data.maThuTuc = options.maThuTuc;
                        options = d.data;
                        cb();
                    }
                }
