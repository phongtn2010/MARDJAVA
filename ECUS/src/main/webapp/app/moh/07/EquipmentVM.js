...
function ChungLoaiVM(data, formVM) {
    ...
    self.isView = ko.observable(data.hasOwnProperty('isView') ? !data.isView : false);
    self.fiLoaiTtb = ko.observable(data.hasOwnProperty('fiLoaiTtb') ? data.fiLoaiTtb : -1);
    ...
    self.fiTenTtbYt = ko.observable(data.hasOwnProperty('fiTenTtbYt') ? data.fiTenTtbYt : null).extend({
    ...
    self.fiMaSp = ko.observable(data.hasOwnProperty('fiMaSp') ? data.fiMaSp : null).extend({
        maxLength: {message: 'Tối đa 250 ký tự', params: 245}
    });
    ...
    self.hideAttach = ko.computed(function(){
        return formVM.fiMaThutuc() != PROCEDUCE.MOH_08 || parseInt(self.fiLoaiTtb()) != 1;        
    });
    ...
    for (var i = 0; i < len; i++) {
        var item = self.lstDinhKemHangHoas()[i];
...
//        }
    ...
    self.toJSON = function () {
        ...
        var _exclude = ['tbErrors', 'lstQuocGia', 'onXoaCoSoSanXuat',
            ...
            'errorCssxFile', 'lstDinhKemHangHoas', 'fiLoaiTtb', 'isView'];
        ...
        model.lstDinhKemHangHoas = self.toAttachs();
        return model;
    };
}
...
function EquipmentVM(data, formVM) {
    ...
    self.fiSoPtnCb = ko.observable(data.hasOwnProperty('fiSoPtnCb') ? data.fiSoPtnCb : null).extend({
        maxLength: {message: 'Tối đa 50 ký tự', params: 50},
        required: {
            message: NSWLang["common_msg_formvaild_required"],
            onlyIf: function () {
    ...
    self.fiMaQgNk = ko.observable(data.hasOwnProperty('fiMaQgNk') ? data.fiMaQgNk : null).extend({
        maxLength: {message: 'Tối đa 50 ký tự', params: 50},
        required: {
            message: NSWLang["common_msg_formvaild_required"],
            onlyIf: function () {
    ...
    self.fiTenQgNk = ko.observable(data.hasOwnProperty('fiTenQgNk') ? data.fiTenQgNk : null);
    ...
    self.OnBlurEvent = function(d,e){
        if(self.fiSoDklh()) {
            var url = '/moh/07/dklh/check';
            app.makePost({
                url: url,
                data: JSON.stringify({
                    'fiSoVb': self.fiSoDklh()
                }),
                success: function (d) {
                    if (d && d.success) {                        
                        self.fibDaCoDklh(parseInt(d.total) > 0);
                    } else {
                        self.fibDaCoDklh(false);
                    }
                },
                error: function (e) {
                    self.fibDaCoDklh(false);
    ...
    self.isShowCSBH = ko.observable(false);
    if (formVM.fiMaThutuc() == PROCEDUCE.MOH_07 && self.fiNhomTtb() != '0') {
        self.isShowCSBH(true);
    }
    ...
    var vg = [self.fiTenTtb, self.fiSoDklh, self.fiLoaiTtb, self.fiPhannhom, self.fiNhomTtb, self.fiTenCsh
                , self.fiDiachiCsh, self.fiMaQaCsh, self.fiTieuchuan, self.fiNhieuCssx, self.fiSoPtnCb];
    self.tbErrors = ko.validation.group(vg, {deep: true, live: true, observable: true});
    self.showErrorLoaiHangHoa = ko.observable(false);
    self.showErrorBaoHanh = ko.observable(false);
    ...
    self.onKhaiBaoChungLoai = function () {
        self.onEditChungLoai(null, false);
        return false;
    };

    self.onSuaChungLoaiHangHoa = function (item) {
        self.onEditChungLoai(item, false);
    };

    self.onXemChungLoaiHangHoa = function (item) {
        self.onEditChungLoai(item, true);
    };
    ...
    self.onEditChungLoai = function (item, isView) {
        var html = [
        ...
        obj.fiLoaiTtb = self.fiLoaiTtb() ? self.fiLoaiTtb() : -1;
    ...
    self.onFiMaQgNKChange = function () {
        if (self.lstQuocGia()) {
            self.fiTenQgNk(null);
            var index = self.lstQuocGia.firstIndexOf(function (item) {
                return item.id == self.fiMaQgNk();
            });
            if (index >= 0) {
                self.fiTenQgNk(self.lstQuocGia()[index].name);
            }
        }
    };

    self.onFiNhomTtbChange = function () {
        if (parseInt(self.fiNhomTtb()) == 0) {
            self.isShowCSBH(false);
        } else {
            self.isShowCSBH(true);
        }
    };
    ...
    self.isVaild = function () {
        ...
        if (null == self.lstLoaiHangHoas() || self.lstLoaiHangHoas().length == 0) {
            self.showErrorLoaiHangHoa(true);
            return false;
        }

        if (self.isShowCSBH() && self.fiNhomTtb() != '0') {
            if (null == self.lstBaoHanhHangHoas() || self.lstBaoHanhHangHoas().length == 0) {
                self.showErrorBaoHanh(true);
                return false;
            }
        }
