...
function PaymentVM(data, formVM) {
    ...
    self.showErrorNgayNop = ko.observable(false);
    self.showErrorFileDinhKem = ko.observable(false);
    ...
    var paymentVaild = [self.fiNguoiNop, self.fiSdt,
    ...
    self.isBCD = ko.observable(true);
    if (data.maThuTuc == 'BYTE0400009') {
        self.isBCD(false);
    }
    
    self.fiMaHoso = ko.computed(function () {
        return formVM.fiMaHoso();
    });
    self.fiLoaiTTBYT = ko.observable(true);
    
    self.fiThongTinThanhToan = ko.computed(function () {
        return formVM.fiThongTinThanhToan();
    });
    self.fiThongTinTaiKhoan = ko.computed(function () {
        return formVM.fiThongTinTaiKhoan();
    });
    ...
    self.btnSavePaymentClick = function () {
        ...
        if (null == self.fiMaTep()) {
            self.showErrorFileDinhKem(true);
            app.Alert('Chứng từ xác nhận nộp lệ phí phải được tải lên!');
            return;
        } else {
            self.showErrorFileDinhKem(false);
        }

        if (!self.fiNgayNop()) {
            self.showErrorNgayNop(true);
            app.Alert('Thông tin ngày nộp phải được cung cấp!');
            return;
        } else {
            self.showErrorNgayNop(false);
        }
    ...
    self.isVaild = function () {
        return !(null == self.lstThanhToans() || self.lstThanhToans().length == 0)
    };
	
	self.refreshName = function(v){
		self.fiLoaiTTBYT($("#fiLoaiTtb :selected").html());		
	}	
}
