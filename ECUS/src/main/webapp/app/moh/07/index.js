...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/moh/07/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdlichsu7(res.data) : [];
                    self.historyItems([]);
...
function IndexVM(params) {
    ...
    self.fiTrangthaiList = ko.observableArray(params.hasOwnProperty('lstTrangThai') ? params.lstTrangThai : RAW_HS_STATUS);
    
    self.propTitle = ko.observable('HỒ SƠ ĐỀ NGHỊ CẤP MỚI SỐ LƯU HÀNH ĐỐI VỚI TTBYT THUỘC LOẠI B,C VÀ D');
    
    if(params.maThuTuc == 'BYTE0400009') {
    ...
    self.searchAfterHasCatgory = function (page, pushState) {
        ...
        app.makePost({
            url: "/moh/07/hoso/timkiem",
            data: JSON.stringify(data),
            success: function (res) {
                window.stateChanging = false;
                if (res.success) {
                    self.Items([]);
                    var list = res.data ? mapTbdhoso7(res.data, data.currentPage * data.pageSize) : [];
                    self.Items(list);
                    self.paging().update({
                        totalCount: res.data ? res.total : 0,
                        pageSize: self.pageSize(),
    ...
    self.bXemClick = function (item, e) {
        document.location = app.appContext + '/moh/07/view?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        return false;
    };
    /**
     * Xem Thông báo
    ...
    self.bXemThongBaoClick = function (item, e) {
        document.location = app.appContext + '/moh/07/result?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=cert&maThuTuc=' + self.maThuTuc();
        return false;
    };
    /**
     * Xem Lich su
    ...
    self.bSuaClick = function (item, e) {
        document.location = app.appContext + '/moh/07/edit?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        ;
        return false;
    };
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/moh/07/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
...
$(document).ready(function () {
    ...
    function init() {
        ...
        var vm = new IndexVM(app.parseQuerystring());
    ...
    $('#loading10').show();
    $.when(app.getCategory('/moh/07/danhmuc', 'DANHMUC_TRANGTHAI', null, function (res) {
        if (res.success) {
