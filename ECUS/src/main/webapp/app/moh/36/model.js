...
var mapTbdlichsu36 = function (dataFromServer) {
    return ko.utils.arrayMap(dataFromServer, function (item, index) {
        return new Tbdlichsu36(index + 1, item);
    });
};
...
var mapTbdhoso36 = function (d, p) {
    return ko.utils.arrayMap(d, function (item, index) {
        return new Tbdhoso36((index + p) + 1, item);
    });
};
...
var mapTbdhsNglieu36 = function (d) {
    return ko.utils.arrayMap(d, function (item, index) {
        item.fiStt = index + 1;
        return new TbdhsNglieu36(item);
    });
};
