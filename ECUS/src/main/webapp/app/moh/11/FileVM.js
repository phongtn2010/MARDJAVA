...
function mapTbddinhkem11(data, idHoso, formVM) {
    return ko.utils.arrayMap(data, function (item, index) {
        return new File(item, idHoso, formVM);
    });
}
...
function FileVM(options, formVM) {
    ...
    if (options && options.fiIdHoso > 0) {
        self.lstDinhKems(mapTbddinhkem11(options.hoso.lstDinhKems, options.fiIdHoso, formVM));
    } else {
        var listFiles = [];
    ...
    self.isVaild = function () {
        ...
        for (var i = 0; i <= len - 1; i++) {
            if (self.lstDinhKems()[i].fiBatBuoc()
                    && (!self.lstDinhKems()[i].fiMaTep()
