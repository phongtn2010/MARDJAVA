...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/moh/11/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdlichsu11(res.data) : [];
                    self.historyItems([]);
...
function IndexVM(params) {
    ...
    self.search = function (page, pushState) {
        if (!RAW_HS_STATUS || RAW_HS_STATUS.length <= 0) {
            app.getCategory('/moh/11/danhmuc', 'DANHMUC_TRANGTHAI', null, function (res) {
                if (res.success) {
    ...
    self.searchAfterHasCatgory = function (page, pushState) {
        ...
        app.makePost({
            url: "/moh/11/hoso/timkiem",
            data: JSON.stringify(data),
            success: function (res) {
                window.stateChanging = false;
                if (res.success) {
                    self.Items([]);
                    var list = res.data ? mapTbdhoso11(res.data, data.currentPage * data.pageSize) : [];
                    self.Items(list);
                    self.paging().update({
                        totalCount: res.data ? res.total : 0,
                        pageSize: self.pageSize(),
    ...
    self.btnAddNewClick = function (e) {
        document.location = app.appContext + '/moh/11/edit?maThuTuc=' + self.maThuTuc();
        return false;
    };
    ...
    self.bXemClick = function (item, e) {
        document.location = app.appContext + '/moh/11/view?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        return false;
    };
    /**
     * Xem Thông báo
    ...
    self.bXemThongBaoClick = function (item, e) {
        document.location = app.appContext + '/moh/11/result?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&maThuTuc=' + self.maThuTuc();
        return false;
    };
    /**
     * Xem Lich su
    ...
    self.bSuaClick = function (item, e) {
        document.location = app.appContext + '/moh/11/edit?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        ;
        return false;
    };
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/moh/11/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
    ...
    self.fiTrangThaiClick = function (item, e) {
        switch (item.fiTrangthai()) {
            case  DUOC_CAP_GIAYPHEP:
                document.location = app.appContext + '/moh/11/result?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=cert&maThuTuc=' + self.maThuTuc();
                break;
            case  DA_TIEP_NHAN:
...
$(document).ready(function () {
    ...
    $('#loading10').show();
    $.when(app.getCategory('/moh/11/danhmuc', 'DANHMUC_TRANGTHAI', null, function (res) {
        if (res.success) {
