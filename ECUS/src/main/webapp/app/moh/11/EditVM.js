...
function EditVM(options) {
    ...
    self.formVM().fiDacoSoDklh.subscribe(function (item) {
        var p = item ? "1":"0";
        app.getCategory('/moh/11/danhmuc', 'DANHMUC_TAILIEU', p, function (res) {
            if (res.success) {
                ...
                self.fileVM().lstDinhKems(mapFilesVM(listFiles, null, self, 'frame1'));
            }
        });
    });
    ...
    self.btnLuuClick = function () {
        ...
        */
        ...
        var url = !fiIdHoso || fiIdHoso <= 0 ? '/moh/11/hoso/taomoi' : '/moh/11/hoso/capnhap';
    ...
    self.btnTroLaiClick = function () {
        document.location = context + '/moh/11/home?maThuTuc=' + options.maThuTuc;
    };
...
$(document).ready(function () {
    ...
    $.when(
            app.getCategory('/moh/11/danhmuc', 'DANHMUC_TINHTHANH', null, function (res) {
                if (res.success) {
            ...
            app.getCategory('/moh/11/danhmuc', 'DANHMUC_QUOCGIA', null, function (res) {
                if (res.success) {
            ...
            app.getCategory('/moh/11/danhmuc', 'DANHMUC_DONVINHAN', null, function (res) {
                if (res.success) {
            ...
            app.getCategory('/moh/11/danhmuc', 'DANHMUC_TAILIEU', "0", function (res) {
                if (res.success) {
                    //DINHKEMDATA
                    options.lstDinhKems = res.data;
                } else {
                    options.lstDinhKems = [];
                }
            })
    ...
    var init = function () {
        if (options && options.fiIdHoso > 0) {
            var url = '/moh/11/hoso/t/' + options.fiIdHoso;
            app.makePost({
                url: url,
                data: JSON.stringify({}),
                success: function (d) {
                    if (d.success) {
                        ...
                        app.getCategory('/moh/11/danhmuc', 'DANHMUC_QUANHUYEN', options.hoso.fiMaTinh, function (res) {
                            if (res.success) {
