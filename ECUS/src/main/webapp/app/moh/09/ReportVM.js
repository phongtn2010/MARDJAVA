...
function Attachment(d){
    var self = this;
    self.attachId = -1 * new Date().getTime();
...
function ReportVM(data) {
    ...
    self.removeAttach = function (item, e) {
        self.lstDinhKems.remove(function (o) {
            return o.attachId == item.attachId;
        });
    };
    ...
    self.sendReport = function (onSuccess, onFail) {
        ...
        if(!self.fiNoidung()) {
            app.Alert('Đề nghị nhập nội dung báo cáo.!');
            return;
        }
        ...
        app.makePost({
            ...
            success: function (res) {
                if (res.success) {
                   ...
                   app.Alert('Không gửi được dữ liệu báo cáo. Chi tiết lỗi: ' + res.message);                     
                }
            },
            error: function (e) { 
                app.Alert('Không gửi được dữ liệu báo cáo.'); 
            }
        });
    };
