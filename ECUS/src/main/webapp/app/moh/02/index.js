...
function IndexVM(params) {
    ...
    self.search = function (page, pushState) {
        ...
        if (!RAW_HS_STATUS || RAW_HS_STATUS.length <= 0) {
            app.getCategory('/moh/p02/danhmuc', 'DM_TRANGTHAI', null, function (res) {
                if (res.success) {
                    RAW_HS_STATUS = res.data;
                    self.fiTrangthaiList(mapCategory(RAW_HS_STATUS, "id", "name"));
                }
            });
        }
    };
    ...
    self.btnAddNewClick = function (e) {
        document.location = app.appContext + '/moh/02/edit';
        return false;
    };
    ...
    self.bXemClick = function (item, e) {
        document.location = app.appContext + '/moh/02/view?fiIdHoso=' + item.fiIdHoso();
        return false;
    };
    ...
    self.bXemGpClick = function(item) {
        document.location = app.appContext + '/moh/02/view_gp?fiMaHoso=' + item.fiMaHoso();
        return false;
    };

    // Sửa hồ sơ
    self.bSuaClick = function (item, e) {
        document.location = app.appContext + '/moh/02/edit?fiIdHoso=' + item.fiIdHoso();
        return false;
    };
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/moh/p02/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d && d.success) {
