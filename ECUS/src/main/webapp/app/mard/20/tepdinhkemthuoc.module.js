...
function TepDinhKemThuocView() {
    ...
    var callback = function (html) {
            var pop = app.popup({
                title: '<b style="text-transform: uppercase">' + NSWLang["mard.20.TbdThuoc20.fiProductFile"] + ' - ' + item.fiNameOfGoods() + '</b>',
                html: html,
                width: 960,
                buttons: [
                ]
            ...
            if(self.teps().length > 0 ){
                $(self.teps()).each(function(index, loopItem){
                    if(loopItem.fiId() != undefined && loopItem.fiId() !== null){
                        $.getJSON(app.appContext + '/mard/20/tbdDinhKem20/delete/' + loopItem.fiId(), function (d) {
