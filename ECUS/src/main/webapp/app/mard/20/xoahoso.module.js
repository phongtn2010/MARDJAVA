	                ...
	                buttons: [
	                    {
	                        ...
	                        action: function () {
	                            app.makePost({
	                                url:'/mard/20/tbdHoSo20/delete/'+item.fiIdHoSo(),
	                                data: JSON.stringify(postData),
	                                success: function (d) {
	                                },
	                                error: function (e) {
	                                    if (e.readyState) {
	                                    	toastr.error(NSWLang["mard.20.msg.xoaHoSoNo"], NSWLang["common_msg_thong_bao"]);
	                                    } else {
	                                    	app.toast({
		                                        title: NSWLang["common_msg_thong_bao"],
		                                        message: NSWLang["mard.20.msg.xoaHoSoOk"],
		                                    });
	                                    }
	                                    setTimeout(function(){
	                                    	location.reload();
