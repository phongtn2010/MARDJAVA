function LoadHoSoView(self) {
	...
	self.layDanhSachTrangThai = function() {
		...
		app.makePost({
			url : '/mard/20/tbsTrangThai20/findAll',
			data : ko.toJSON({
				"number" : 0,
				"size" : 100,
				"sort" : "fiId",
			...
			error : function(e) {
				self.danhSachTrangThai.push({fiCode: '-1', fiStatusName: NSWLang["mard.20.select.option"]});
	...
	self.load = function(page) {
		...
		app.makePost({
			url : encode('/mard/20/tbdHoSo20/page', self.searchForm),
			data : ko.toJSON({
				"number" : page - 1,
