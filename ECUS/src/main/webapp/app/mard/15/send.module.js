function SendHoSoView() {
    ...
    self.show = function (item) {
        ...
        var pop = app.popup({
            title: NSWLang["mard.15.send.confirm"],
            html: '<i class="fa fa-3x fa-warning"></i> ' + NSWLang["mard.15.send.confirm"] + ' <b>' + docCode + '</b>',
            width: 400,
            buttons: [
                {
    ...
    self.send = function (popup, data) {
        ...
        app.makePost({
            ...
            error: function (e) {
                ...
                if (!e.message) e.message = NSWLang["mard.15.send.timeout"];
    ...
    self.thongBao = function (message, success) {
        ...
        if (!isDevTest || (isDevTest && success == true)) {
            setTimeout(function () {
                location.href = app.appContext + '/mard/15/home';
            }, 1000);
        } else if (!success) {
            setTimeout(function () {
                location.href = app.appContext + '/mard/15/edit/' + idHoSo;
            }, 1000);
        }
