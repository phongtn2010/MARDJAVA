...
function ViewModel() {
    ...
    self.xemChiTietHoSo = function (item) {
        location.href = app.appContext + '/mard/16/view/' + item.fiIdHoSo();
    }

	self.xinSua = function(item) {
		location.href = app.appContext + '/mard/16/ycs/'+item.fiIdHoSo();
	}

    self.themMoiHoSo = function() {
        location.href = app.appContext + '/mard/16/edit';
    }


    self.chinhSuaHoSo = function(item) {
        location.href= app.appContext + '/mard/16/edit/' + item.fiIdHoSo();
    }
