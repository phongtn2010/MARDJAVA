    ...
    self.show = function(item, success) {
        if (item.fiStatus() == 1 || item.fiStatus() == 5 || item.fiStatus() == 6) {
            var pop = app.popup({
                ...
                html: '<i class="fa fa-3x fa-warning"></i> ' + NSWLang["mard.18.index.table.th.12"] + ' <b>'+ item.fiDocumentName() +'</b>',
                width: 400,
                buttons: [
                    {
        ...
        var callback = function (html) {
            var pop = app.popup({
                title:  '<b style="text-transform: uppercase">' + NSWLang["mard.18.index.table.th.12"] + " - " + item.fiDocumentName() + '</b>',
                html: html,
                width: 960,
                buttons: [
                    {
