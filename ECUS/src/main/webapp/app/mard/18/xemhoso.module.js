function PopupView() {
	...
	self.show = function(item) {
		var url = '/mard/api/18/edit/' + item.fiIdHoSo();
		app.makePost({
			url: url,
			data: JSON.stringify({}),
			error: function (d) {
				var idHoSo = d.hoSo.fiIdHoSo;
				d.downloadFileHoSoUrl = app.appContext  + "/mard/api/18/xuatFileHoSo/" + idHoSo;
				const vacXin = [16, 17, 18];
				d.hasVacXin = false;
				d.thuocs.forEach(function (thuoc) {
					console.log(thuoc.fiProductType);
					if (vacXin.indexOf(thuoc.fiProductType) !== -1 ) d.hasVacXin = true;
