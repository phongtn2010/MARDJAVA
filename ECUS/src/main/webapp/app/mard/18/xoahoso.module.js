	                ...
	                buttons: [
	                    {
	                        ...
	                        action: function () {
	                            app.makePost({
	                                url:'/mard/18/tbdHoSo18/delete/'+item.fiIdHoSo(),
	                                data: JSON.stringify(postData),
	                                success: function (d) {
	                                },
	                                error: function (e) {
	                                    if (e.readyState) {
	                                    	toastr.error(NSWLang["mard.18.msg.xoaHoSoNo"], NSWLang["common_msg_thong_bao"]);
	                                    } else {
	                                    	app.toast({
		                                        title: NSWLang["common_msg_thong_bao"],
		                                        message: NSWLang["mard.18.msg.xoaHoSoOk"],
		                                    });
	                                    }
	                                    setTimeout(function(){
	                                    	location.reload();
