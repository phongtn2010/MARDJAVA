...
function SuaGiayPhepView() {
    ...
    self.show = function(item) {
        var callback = function (html) {
            var pop = app.popup({
                title: '<b style="text-transform: uppercase">' +  NSWLang["mard.18.index.table.th.14"] + " - " + item.fiDocumentName() + '</b>',
                html: html,
                width: 960,
                buttons: [
                ]
        ...
        $.ajax({
            ...
            success : function(response) {
                if (response.status == "Successful" ) {
                    ...
                    alert('File upload không thành công vui lòng thử lại');
                    getToken();
                }
            },
        });
    ...
    self.uploadFileChangeEvent = function( elemet, event) {
        ...
        for (var k = 0; k < files.length; k++) {
            ...
            if(self.teps().length > 0) {
                app.makePost({
                    url: '/mard/18/tbdDinhKem18/create',
