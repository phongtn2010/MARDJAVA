...
$(document).ready(function () {
    ...
    function GhiHoSoViewModel() {
        ...
        self.getHoSo = function (idHoSo) {
            app.makePost({
                url: '/mard/api/17/edit/' + self.idHoSo,
                data: JSON.stringify({}),
                error: function (e) {
                    $('#fiApplicationNo').focus();
                    self.formData = e;
                    self.tbdHoSo17 = convertObjectToKnockout(e.hoSo, new TbdHoSo17());
        ...
        self.save = function (isGuiHoSo) {
            ...
            postData.thuocs.forEach(function (dataThuocs) {
                ...
                dataThuocs.fiTepDinhKemThuocs.forEach(function(dataTep){
                    var t = {};
                    t = convertKnockoutToObject(dataTep, createObject(new TepTinThuocModel()), true);
                    ps.push(t);
                })
            ...
            app.makePost({
                url: '/mard/api/17/save/' + self.idHoSo,
                data: ko.toJSON(postData),
                error: function (e) {
                    if (e.readyState == 4) {
                        if (idHoSo == 0) {
                            showToast(NSWLang["mard.17.msg.taoMoiHoSoNo"], false);
                        } else {
                            showToast(NSWLang["mard.17.msg.capNhatHoSoNo"], false);
                        }
                        return;
                    } else {
                        if (idHoSo == 0) {
                            showToast(NSWLang["mard.17.msg.taoMoiHoSoOk"], true);

                        } else {
                            showToast(NSWLang["mard.17.msg.capNhatHoSoOk"], true);
                        }
                    }
                    ...
                    if (isGuiHoSo) {
                        ...
                        var item = new TbdHoSo17();
                        item.fiIdHoSo(e.hoSo.fiIdHoSo);
                        item.fiDocumentName(e.hoSo.fiDocumentName);
                        sendHoSoView.show(item);
                    } else {
                        if (action == 2) {
                            location.href = app.appContext + '/mard/17/ycs/' + e.hoSo.fiIdHoSo;
                        } else {
                            location.href = app.appContext + '/mard/17/edit/' + e.hoSo.fiIdHoSo;
                        }
                    }
                }
            });
