function LoadHoSoView(self) {
	...
	self.layDanhSachTrangThai = function() {
		...
		app.makePost({
			url : '/mard/17/tbsTrangThai17/findAll',
			data : ko.toJSON({
				"number" : 0,
				"size" : 100,
				"sort" : "fiId",
			...
			error : function(e) {
				self.danhSachTrangThai.push({fiCode: '-1', fiStatusName: NSWLang["mard.17.select.option"]});
	...
	self.load = function(page) {
		...
		app.makePost({
			url : encode('/mard/17/tbdHoSo17/page', self.searchForm),
			data : ko.toJSON({
				"number" : page - 1,
