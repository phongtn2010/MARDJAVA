function LoadHoSoView(self) {
	...
	self.layDanhSachTrangThai = function() {
		...
		app.makePost({
			url : '/mard/19/tbsTrangThai19/findAll',
			data : ko.toJSON({
				"number" : 0,
				"size" : 100,
				"sort" : "fiId",
			...
			error : function(e) {
				self.danhSachTrangThai.push({fiCode: '-1', fiStatusName: NSWLang["mard.19.select.option"]});
	...
	self.load = function(page) {
		...
		app.makePost({
			url : encode('/mard/19/tbdHoSo19/page', self.searchForm),
			data : ko.toJSON({
				"number" : page - 1,
				"size" : self.pagingVM.pageSize(),
				"sort" : "fiIdHoSo",
				"direction" : "desc",
				"fiDocumentType" : "BNNPTNT06000073"
			}),
			/*url : encode('/mard/19/tbdHoSo19/findByDocumentTypeAndDocumentNameAndFiTaxCode', self.danhSachHoSo),
			data : ko.toJSON({
				"fiDocumentType" : documentType,
				"fiTaxCode" : taxCode
			}),*/
			error : function(e) {
	...
	self.checkShowHideButton = function(item) {
		...
		item.showButtonThongBaoPhi(self.showButtonThongBaoPhi(item.fiStatus()));
	}
