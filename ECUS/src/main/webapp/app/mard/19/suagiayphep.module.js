...
function SuaGiayPhepView() {
    ...
    self.show = function(item) {
        var callback = function (html) {
            var pop = app.popup({
                title: '<b style="text-transform: uppercase">' +  NSWLang["mard.19.index.table.th.14"] + " - " + item.fiDocumentName() + '</b>',
                html: html,
                width: 960,
                buttons: [
                ]
...
function SuaGpPopupViewModel(item){
    ...
    self.isValid = function() {
        if (self.teps().length < 1){
            app.Alert( NSWLang["mard.19.tepDinhKem.notValid"]);
            return false;
        }
        return true;
    }
    ...
    self.uploadFileChangeEvent = function( elemet, event) {
        ...
        for (var k = 0; k < files.length; k++) {
            ...
            if(self.teps().length > 0) {
                app.makePost({
                    url: '/mard/19/tbdDinhKem19/create',
