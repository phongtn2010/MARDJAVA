function ChiTietThuocViewModel(parent, moduleThongTinChung) {
    ...
    self.fiWeightUnits.push({fiUnitName: NSWLang["mard.14.select.option"], fiUnitCode: '-1'});
    ...
    self.layDanhMucMucDich = function () {
        readArrayObjects(parent.formData.danhMucMucDichs, function (item) {
            self.fiPurposes.push(convertObjectToKnockout(item, new TbsMucDich14()));
        });
    }
    ...
    self.clearData = function (item) {
        if (item.fiDisplayGroup()) {
            ...
            readArrayObjects(grps, function (loopItem2) {
                self.moduleThongTinChung.tbdHoSo14[loopItem2](null);
            });
        }
    }
    ...
    self.layDanhSachHangHoaTheoHS = function () {
        ...
        self.fiTypeGoods.push({fiCode: '-1', fiName: NSWLang["mard.14.select.option"]});
        ...
        readArrayObjects(parent.formData.thuocs, function (item) {
            item.enable = false;
            self.thuocs.push(convertObjectToKnockout(item, new TbdThuoc14()));
        }, function () {
        });
    ...
    self.addHangHoa = function (item) {
        self.openForm(new TbdThuoc14(), function () {
    ...
    self.editHangHoa = function (item) {
        item.enable(false);
        self.openForm(cloneKnockoutObject(item, new TbdThuoc14()));
    }
    ...
    self.deleteHangHoa = function (item) {
        ...
        self.callConfirm(function () {
            if (idHoSo > 0) {
                $.getJSON(app.appContext + '/mard/14/tbdThuoc14/delete/' + item.fiId(), function (d) {
    ...
    self.isValid = function (isGuiHoSo) {
        var check = true;
        if (action === 2) {
            self.moduleThongTinChung.tbdHoSo14["fiReasonCorrection"].rules.remove(function (item) {
                return item.rule == "required";
            });
            self.moduleThongTinChung.tbdHoSo14["fiReasonCorrection"].extend({
                required: true,
            });
        }
        readKnockoutArrays(self.thuocs, function (item, index) {
            if (index == self.thuocs().length - 1 && item.valid.errors().length == 6) return true;
        ...
        if (isGuiHoSo) {
            if (self.thuocs().length == 0) {
                app.Alert(NSWLang["mard.14.thuoc.notValid"]);
                check = false;
        ...
        readKnockoutArrays(self.fiPurposes, function (loopItem) {
            if (loopItem != undefined) {
                if (loopItem.fiDisplayGroup() != null && loopItem.fiDisplayGroup() != undefined) {
                    var grps = loopItem.fiDisplayGroup().split(';');
                    readArrayObjects(grps, function (loopItem2) {
                        self.moduleThongTinChung.tbdHoSo14[loopItem2].rules.remove(function (item) {
                            return item.rule == "required";
                        });
                    });
                }
        ...
        readKnockoutArrays(self.fiPurposeSelecteds, function (loopItem) {
            if (loopItem != undefined) {
                if (loopItem.fiDisplayGroup() != null && loopItem.fiDisplayGroup() != undefined) {
                    var grps = loopItem.fiDisplayGroup().split(';');
                    readArrayObjects(grps, function (loopItem2) {
                        self.moduleThongTinChung.tbdHoSo14[loopItem2].extend({
                            required: true,
                            maxLength: 500
                        });
                    });
        ...
        self.moduleThongTinChung.tbdHoSo14['fiImportTimeTo'].rules.remove(function (item) {
            return item.rule == "greaterThan";
        });
        self.moduleThongTinChung.tbdHoSo14['fiImportTimeTo'].extend({
            greaterThan: self.moduleThongTinChung.tbdHoSo14.fiImportTimeFrom()
        });
        var mucDich = '';
        ...
        if (mucDich.endsWith(";")) {
            self.moduleThongTinChung.tbdHoSo14.fiPurposes(mucDich.substring(0, mucDich.length - 1));
        }

        if (self.moduleThongTinChung.tbdHoSo14.valid.errors().length > 0) {
            showError(self.moduleThongTinChung.tbdHoSo14);
            self.moduleThongTinChung.tbdHoSo14.valid.errors.showAllMessages();
            return false;
        }
    ...
    self.convertData = function () {
        ...
        self.thuocs().forEach(function (item, index) {
            ...
            data.push(convertKnockoutToObject(item, createObject(new TbdThuoc14())));
        });
    ...
    self.openForm = function (item, endAction) {
        var callback = function (html) {
            var pop = app.popup({
                title: "<b style='text-transform: uppercase'>"+ NSWLang["mard.14.form.thongTinThuoc"] +"</b>",
                html: html,
                width: 960,
                backdrop: true,
                buttons: []
    ...
    self.uploadFileChangeEvent = function(elemet, event) {
        ...
        for (var k = 0; k < files.length; k++) {
            ...
            param.file = files[k];
            app.uploadFile({
                url: '/mard/api/14/uploadFileExcel/' + findId,
                data: param,
                success: function (d) {
                    if (d.success) {
                        $('#fileUploadExcel').val('');
