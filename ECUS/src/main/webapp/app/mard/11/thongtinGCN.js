...
var CreateGCNVM = function(options){
    ...
    self.fiManuocQc = ko.observable(gcn11.fiManuocQc).extend({
        required: { message: 'Thông tin phải nhập', params : true}
    });
    self.fiTennuocQc = ko.observable(gcn11.fiTennuocQc).extend({
        required: { message: 'Thông tin phải nhập', params : true}
    });
    ...
    self.fiKbbs = ko.observable(gcn11.fiKbbs).extend({
        maxLength: { message: '2000 ký tự', params : 2000}
    });
    ...
    self.gcnErrors = ko.validation.group([self.fiMabaobi,self.fiPtcc, self.fiLoaiCt, self.fiManuocXk, self.fiTennuocXk, self.fiManuocQc, self.fiTennuocQc, self.fiSoGcn], { deep: true, live: true, observable: true });
    ...
    getCategory("QUOCGIA", null, function(res){
        if(res.success){
            self.fiManuocXkList(mapCategory(res.data,"countrycode", "countryname"));
            self.fiManuocQcList(mapCategory(res.data,"countrycode", "countryname"));
            self.fiManuocSxList(mapCategory(res.data,"countrycode", "countryname"));
            if(gcn11.fiManuocXk){
