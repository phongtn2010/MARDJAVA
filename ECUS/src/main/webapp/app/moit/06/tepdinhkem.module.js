function ThongTinTepDinhKemViewModel(hoSo) {
	...
	self.selectedTep = ko.observable('NHAP_KHAU');
	if (hoSo.loaiGiayPhep != null && hoSo.loaiGiayPhep == 1) self.selectedTep('XUAT_KHAU');
	             ...
	             buttons: [
	                 {
	                     ...
	                     action: function () {
	                    	 $(self.teps()[position]()).each(function(index, loopItem){
	                    		 app.makePost({
		                             url:'/moit/api/06/xoaTepDK/' + loopItem.idTepTin(),
		                             data: JSON.stringify({}),
		                             success: function (d) {
		                                 var msg = d.message;
		                                 var fun = 'success';
	             ...
	             buttons: [
	                 {
	                     ...
	                     action: function () {
	                         app.makePost({
	                             url:'/moit/api/06/xoaTepDK/' + item.idTepTin(),
	                             data: JSON.stringify({}),
	                             success: function (d) {
	                                 var msg = d.message;
	                                 var fun = 'success';
