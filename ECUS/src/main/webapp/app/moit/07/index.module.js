...
function ViewModel() {
	...
	var SearchFormModel = function() {
		...
		this.fromNgayNop = ko.observable(null);
		this.toNgayNop = ko.observable(null);
		this.fromNgayCapGiayPhep = ko.observable(null);
		this.toNgayCapGiayPhep = ko.observable(null);
...
//            			console.log(d);
	...
	self.searchHoSo = function() {
		...
		if (!self.validDate(self.searchForm.fromNgayNop(), self.searchForm.toNgayNop())) {
			return;
		}
		if (!self.validDate(self.searchForm.fromNgayCapGiayPhep(), self.searchForm.toNgayCapGiayPhep())) {
			return;
		}
	...
	self.themMoiHoSo = function() {
		location.href = app.appContext + '/moit/07/edit';
	}
	
	self.chinhSuaHoSo = function(item) {
		location.href= app.appContext + '/moit/07/edit/' + item.idHoSo();
	}
	
	self.xinSuaHoSo = function(item) {
		location.href= app.appContext + '/moit/07/edit/' + item.idHoSo() + '/ycs';
	}
	
	self.xemHoSo = function(item) {
		location.href= app.appContext + '/moit/07/view/' + item.idHoSo();
	}
	...
	self.keypressdown = function(data, event) {
