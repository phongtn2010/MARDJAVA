...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/monre/06/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdLichsu06(res.data) : [];
                    self.historyItems([]);
...
function Monre06VM() {
    ...
    self.monre06Items = ko.observableArray([]);
    self.totalCount = ko.observable(0);
    ...
    self.searchAfterHasCatgory = function (page, pushState) {
        ...
        $.ajax({
            ...
            cache: false,
            crossDomain: true,
            url: app.appContext + "/monre/06/hoso/timkiem",
            data: JSON.stringify(data),
            ...
            success: function (res) {
                if (res.success) {
                    self.monre06Items([]);
                    var list = res.data ? mapTbdhoso06(res.data) : [];
                    self.monre06Items(list);
                    self.paging().update({
                        totalCount: res.data ? res.total : 0,
                        pageSize: self.pageSize(),
                        currentPage: page
    ...
    self.btnAddNewClick = function (e) {
        document.location = app.appContext + '/monre/06/create';
        return false;
    };
    ...
    self.bXemClick = function (item, e) {
        document.location = app.appContext + '/monre/06/view?fiIdHoso=' + item.fiIdHoso();
        return false;
    };
    /**
     * Xem Thông báo
    ...
    self.bXemThongBaoClick = function (item, e) {
        document.location = app.appContext + '/monre/06/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=mau';
        return false;
    };
    /**
     * Xem Lich su
    ...
    self.bSuaClick = function (item, e) {
        document.location = app.appContext + '/monre/06/create?fiIdHoso=' + item.fiIdHoso();
        return false;
    };
    /**
     * Hủy hồ sơ (xóa)
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/monre/06/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
    ...
    self.fiTrangThaiClick = function (item, e) {
        switch (item.fiTrangthai()) {
            ...
            case  TU_CHOI_CAP_TBNK:
                self.createPopFiTrangThai('kqxl-template', item);
                self.kqxlHoSo({
                    fiMaHoso: item.fiMaHoso(),
                    fiIdHoso: item.fiIdHoso(),
                    fiTrangthai: TU_CHOI_CAP_TBNK
                }, function (d) {
                    ko.applyBindings(new KetQuaXLVM(d.data), document.getElementById('kqxl-form'));
                });
                break;
            case  DUOC_CAP_TBNK:
            case  DA_CAP_LAI_TBNK:
                document.location = app.appContext + '/monre/06/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + "&tab=mau";
                break;
            case  DA_THU_HOI:
                self.createPopFiTrangThai('kqxl-template', item);
    ...
    self.kqycrutHoSo = function (message, cb) {
        app.makePost({
            url: '/monre/06/hoso/kqycr' + message.fiMaHoso,
            data: JSON.stringify({}),
            success: function (d) {
                var msg = '';
                if (d.success) {
...
$(document).ready(function () {
    var vm = new Monre06VM();
    ko.applyBindings(vm, document.getElementById('monre06'));
    vm.applyState(app.parseQuerystring());
    window.stateChangeIsLocal = false;
    window.stateChanging = true;
    $(".select2").select2({placeholder: '-- Chọn --', width: '100%', allowClear: true});
