...
function CoSoSXModel() {
	...
	this.maHuyen = ko.observable('-1');
	
	this.tenXaPhuong = ko.observable('').extend({
		maxLength : 255
	});

	this.maXaPhuong = ko.observable('-1');
...
function PheLieuModel() {
	...
	this.klCongSuatInput = ko.observable('');
	this.klCongSuat = ko.observable('').extend({
		required : false,
		number: true,
		min : 0,
		pattern: '^[0-9]{1,15}(.[0-9]{0,3})?'
