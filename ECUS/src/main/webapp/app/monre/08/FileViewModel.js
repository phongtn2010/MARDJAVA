...
var MDMConfig = {
    ...
    idTthc: '159', // BTNMT0800001
    client_key: 'MDM_AUTH_API',
    client_secret: 'secret'
};

function File08VM(item, idHoso) {
    var self = this;
...
function mapFiles08VM(dataFromServer, idHoso) {
    
    return ko.utils.arrayMap(dataFromServer, function (item, index) {
        return new File08VM(item, idHoso);
    });
}



function Monre08FilesVM(data) {
    var self = this;
...
//    console.log(data.dmDinhKem);
//    console.log("===========================");

    //Danh sach cac loai dinh kem trong ho so
//    self.lstDinhKem = ko.observableArray(mapFiles08VM(lstFileType, self.fiIdHoso()));
    
    self.lstDinhKem = ko.observableArray(mapFiles08VM(data.dmDinhKem, self.fiIdHoso()));
...
//        self.lstDinhKem = ko.observableArray(mapFiles08VM(self.lstDinhKem, self.fiIdHoso()));
    ...
    self.isValidForm = function () {
        ...
        for(var i=0;i<self.lstDinhKem().length;i++){
            if(self.lstDinhKem()[i].fiIdTep()==null && self.lstDinhKem()[i].isValidate() ==1){
                isVaild = false;
                break;
            }
        }
