...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/monre/08/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdLichsu08(res.data, curPage * self.pageSize()) : [];
                    self.historyItems([]);
...
function Monre08VM() {
    ...
    self.monre08Items = ko.observableArray([]);
    self.totalCount = ko.observable(0);
...
//        var st = self.ngayTaoTuNgay();
//        var end = self.ngayTaoDenNgay();
//        if (st.getTime() > end.getTime()) {
//            app.Alert("Ngày nộp từ không được nhỏ hơn thời gian kết thúc!")
//            return false;
//        }
//        return true;
//    };
//    
//    self.isValidDateNgayCap = function () {
//        var st = self.ngayCapTuNgay();
//        var end = self.ngayCapDenNgay();
//        if (st.getTime() > end.getTime()) {
...
//    };
    ...
    self.searchAfterHasCatgory = function (page, pushState) {
        ...
        $.ajax({
            ...
            cache: false,
            crossDomain: true,
            url: app.appContext + "/monre/08/hoso/timkiem",
            data: JSON.stringify(data),
            ...
            success: function (res) {
                if (res.success) {
                    self.monre08Items([]);
                    var list = res.data ? mapTbdhoso08(res.data, curPage * self.pageSize()) : [];
                    self.monre08Items(list);
                    self.paging().update({
                        totalCount: res.data ? res.total : 0,
                        pageSize: self.pageSize(),
                        currentPage: page
    ...
    self.btnAddNewClick = function (e) {
        document.location = app.appContext + '/monre/08/create';
        return false;
    };
    ...
    self.bXemClick = function (item, e) {
        document.location = app.appContext + '/monre/08/view?fiIdHoso=' + item.fiIdHoso();
        return false;
    };
    /**
     * Xem Thông báo
    ...
    self.bXemThongBaoClick = function (item, e) {
        ...
        console.log(item);
        console.log("============================");
        document.location = app.appContext + '/monre/08/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=mau';
        return false;
    };
    /**
     * Xem Lich su
    ...
    self.bSuaClick = function (item, e) {
        document.location = app.appContext + '/monre/08/create?fiIdHoso=' + item.fiIdHoso();
        return false;
    };
    /**
     * Hủy hồ sơ (xóa)
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/monre/08/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
    ...
    self.kqycrutHoSo = function (message, cb) {
        ...
        app.makePost({
            url: '/monre/08/hoso/kqycr/' + message.fiMaHoso,
            data: JSON.stringify({}),
            success: function (d) {
                var msg = '';
                if (d.success) {
...
$(document).ready(function () {
    var vm = new Monre08VM();
    ko.applyBindings(vm, document.getElementById('monre08'));
