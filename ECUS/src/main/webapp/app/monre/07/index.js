...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/monre/07/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdLichsu07(res.data) : [];
                    self.historyItems([]);
...
function Monre07VM() {
    ...
    self.monre07Items = ko.observableArray([]);
    self.totalCount = ko.observable(0);
    ...
    self.searchAfterHasCatgory = function (page, pushState) {
        ...
        $.ajax({
            ...
            cache: false,
            crossDomain: true,
            url: app.appContext + "/monre/07/hoso/timkiem",
            data: JSON.stringify(data),
            ...
            success: function (res) {
                if (res.success) {
                    self.monre07Items([]);
                    var list = res.data ? mapTbdhoso07(res.data) : [];
                    self.monre07Items(list);
                    self.paging().update({
                        totalCount: res.data ? res.total : 0,
                        pageSize: self.pageSize(),
                        currentPage: page
    ...
    self.btnAddNewClick = function (e) {
        document.location = app.appContext + '/monre/07/create';
        return false;
    };
    ...
    self.bXemClick = function (item, e) {
        document.location = app.appContext + '/monre/07/view?fiIdHoso=' + item.fiIdHoso();
        return false;
    };
    /**
     * Xem Thông báo
    ...
    self.bXemThongBaoClick = function (item, e) {
        document.location = app.appContext + '/monre/07/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=mau';
        return false;
    };
    /**
     * Xem Lich su
    ...
    self.bSuaClick = function (item, e) {
        document.location = app.appContext + '/monre/07/create?fiIdHoso=' + item.fiIdHoso();
        return false;
    };
    /**
     * Hủy hồ sơ (xóa)
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/monre/07/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
    ...
    self.kqycrutHoSo = function (message, cb) {
        app.makePost({
            url: '/monre/07/hoso/kqycr' + message.fiMaHoso,
            data: JSON.stringify({}),
            success: function (d) {
                var msg = '';
                if (d.success) {
...
$(document).ready(function () {
    var vm = new Monre07VM();
    ko.applyBindings(vm, document.getElementById('monre07'));
