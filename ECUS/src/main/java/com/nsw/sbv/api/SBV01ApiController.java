...
import com.nsw.sbv.p01.model.TepDinhKemHoSoEditForm;
...
public class SBV01ApiController extends SBV01BaseController {
	...
	public ResponseEntity<Object> sendHoSo(HttpServletRequest request, @RequestBody KySoModel kySoModel) {
		...
		LOGGER.info("Gui ho so: {}", kySoModel);
		...
		try {
			...
			if (!isValidGuiHoSo(hoSoNgoaiTe1)) {
				return createResponseEntity(null, 0L, getMessage(SBV01Constants.MessageKeys.ACTION_SEND_ERROR, null, request.getLocale()), false, HttpStatus.OK);
			}
	...
	public ResponseEntity<Object> getGiayPhep(HttpServletRequest request, @PathVariable("idHoSo") long idHoSo) {
		...
		try {
			...
			chiTietGiayPhep.setNgay(String.valueOf(calendar.get(Calendar.DATE)));
			chiTietGiayPhep.setThang(String.valueOf(calendar.get(Calendar.MONTH)));
	...
	public ResponseEntity<Object> getChiTietHoSo(HttpServletRequest request, @PathVariable("idHoSo") long idHoSo) {
		...
		try {
			...
			ChiTietGiayPhep chiTietGiayPhep = new ChiTietGiayPhep();
			Calendar calendar = Calendar.getInstance();
			List<KetQuaXuLyHoSo1> kqs = SBV01KetQuaXuLyHoSo1BackendUtil.searchKetQuaXuLys(getURL(SEARCH_KET_QUA_XU_LY), idHoSo, 1, 10);
			Optional<KetQuaXuLyHoSo1> ngayNopKQXL = kqs.stream().filter(p-> p.getTrangThai() == TrangThaiHoSo.DA_TIEP_NHAN_4).findFirst();
			if (ngayNopKQXL.isPresent()) {
				calendar.setTime(ngayNopKQXL.get().getNgayXuLy());
				chiTietGiayPhep.setNgay(String.valueOf(calendar.get(Calendar.DATE)));
				chiTietGiayPhep.setThang(String.valueOf(calendar.get(Calendar.MONTH)));
				chiTietGiayPhep.setNam(String.valueOf(calendar.get(Calendar.YEAR)));
			}
	...
	public ResponseEntity<Object> getHoSos(HttpServletRequest request, @RequestBody HoSoNgoaiTeSearchForm form) {
		...
		try {
			...
			if (StringUtils.hasText(form.getMaSoThue())) {
				item.setMaSoThue(form.getMaSoThue().trim());
			}
			if (StringUtils.hasText(form.getSoGiayPhep())) {
				item.setSoGiayPhep(form.getSoGiayPhep().trim());
			}
	...
	private List<LoaiTienTe1> getAllLoaiTienTes(HttpServletRequest request) {
		...
		if (!ObjectUtils.isEmpty(loaiTienTe1s)) list.addAll(loaiTienTe1s);
	...
	public ResponseEntity<Object> saveHoSo(HttpServletRequest request, @RequestBody HoSoNgoaiTeEditForm editForm) {
		...
		try {
			...
			List<TepDinhKemHoSoEditForm> tepLoai1EditForms = readJsonStringToList(editForm.getTepLoai1JsonString(), TepDinhKemHoSoEditForm.class);

			List<TepDinhKemHoSoEditForm> tepLoai2EditForms = readJsonStringToList(editForm.getTepLoai2JsonString(), TepDinhKemHoSoEditForm.class);

			List<TepDinhKemHoSoEditForm> tepLoai3EditForms = readJsonStringToList(editForm.getTepLoai3JsonString(), TepDinhKemHoSoEditForm.class);

			editForm.setTienTes(tienTeEditForms);
			editForm.setTepTins(tepLoai1EditForms);
			editForm.setTepTin2s(tepLoai2EditForms);
			editForm.setTepTin3s(tepLoai3EditForms);
			
			if (!editForm.getTienTes().isEmpty()) {
				saveTienTe(editForm, hoSoNgoaiTe1, request);
			}

			if (!editForm.getTepTins().isEmpty()) {
				saveTepTin(request, editForm, hoSoNgoaiTe1.getIdHoSo());
			}
			if (!editForm.getTepTin2s().isEmpty()) {
				saveTepTin2(request, editForm, hoSoNgoaiTe1.getIdHoSo());
			}
			if (!editForm.getTepTin3s().isEmpty()) {
				saveTepTin3(request, editForm, hoSoNgoaiTe1.getIdHoSo());
			}
	...
	private ResponseEntity<Object> sendRequiredSign(HttpServletRequest request, HoSoNgoaiTe1 hoSoNgoaiTe1, HoSoNgoaiTeEditForm editForm) {
		LOGGER.info("{}", "GUI HO SO YEU CAU KY SO");
		...
		if (!ObjectUtils.isEmpty(x509Cert)) {
			...
			editForm.setXmlContent(getXmlContent(hoSoNgoaiTe1));
			LOGGER.info("TAO KY SO VOI VOI NUMBER: {}", editForm.getSerialNumber());
			return createResponseEntity(editForm, 0L, getMessage(SBV01Constants.MessageKeys.ACTION_SUCCESS, null, request.getLocale()), true, HttpStatus.OK);
		}
	...
	private String getXmlContent(HoSoNgoaiTe1 hoSoNgoaiTe1) {
		SendMessage sendMessage = new SendMessage();
		...
		ResponseJson responseJson = callRestTemplate(getURL(SEND_ALL), sendMessage, HttpMethod.POST, null);
		return (String) responseJson.getData();
	}

	private boolean isValidTepTin(TepDinhKemHoSoEditForm item) {
		boolean isTrue = true;
		if (item.getIdTepDK() != 0) return false;
		if (!StringUtils.hasText(item.getGuID())) return false;
		if (!StringUtils.hasText(item.getTenTepDinhKem())) return false;
		if (!StringUtils.hasText(FilenameUtils.getExtension(item.getTenTepDinhKem()))) return false;
		if (!StringUtils.hasText(item.getMaLoaiTepDinhKem())) return false;
		if (!StringUtils.hasText(item.getDuongDanFile())) return false;
		return isTrue;
	}

	private void saveTepTin(HttpServletRequest request, HoSoNgoaiTeEditForm form, long idHoSo) {
		for (TepDinhKemHoSoEditForm item : form.getTepTins()) {
			
			if (isValidTepTin(item)) {
				TepDinhKemHoSo1 tepDinhKemHoSo1 = new TepDinhKemHoSo1();
				tepDinhKemHoSo1.setGuID(item.getGuID());
				tepDinhKemHoSo1.setDuongDanFile(item.getDuongDanFile());
				tepDinhKemHoSo1.setIdHoSo(idHoSo);
				tepDinhKemHoSo1.setTenTepDinhKem(item.getTenTepDinhKem());
				tepDinhKemHoSo1.setMaLoaiTepDinhKem(item.getMaLoaiTepDinhKem());
				tepDinhKemHoSo1.setTenLoaiTepDinhKem(getTenLoaiTepTin(request, item.getMaLoaiTepDinhKem()));
				tepDinhKemHoSo1.setLoaiTep(item.getLoaiTep());
				tepDinhKemHoSo1.setIdHoSo(idHoSo);
				tepDinhKemHoSo1.setLoaiTep(FilenameUtils.getExtension(item.getTenTepDinhKem()));
				if (idHoSo > 0 && item.getIdTepDK() == 0) {

					SBV01TepDinhKemHoSo1BackendUtil.createTepDinhKemHoSo1(getURL(CREATE_TEP_DINH_KEM), tepDinhKemHoSo1);
				}
			}
	...
	private void saveTepTin2(HttpServletRequest request, HoSoNgoaiTeEditForm form, long idHoSo) {
		for (TepDinhKemHoSoEditForm item : form.getTepTin2s()) {
	...
	private void saveTepTin3(HttpServletRequest request, HoSoNgoaiTeEditForm form, long idHoSo) {
		for (TepDinhKemHoSoEditForm item : form.getTepTin3s()) {
	...
	private boolean isValidGuiHoSo(HoSoNgoaiTe1 hoSoNgoaiTe1) {

		boolean isTrue = true;

		List<TienTe1> tienTe1s = SBV01TienTe1BackendUtil.getByTienTes(getURL(GET_TIEN_TE_BY_ID_HO_SO), hoSoNgoaiTe1.getIdHoSo());
		if (tienTe1s == null || tienTe1s.isEmpty()) return false;

		List<TepDinhKemHoSo1> tepDinhKemHoSo1s = SBV01TepDinhKemHoSo1BackendUtil.getByTepDinhKems(getURL(GET_TEP_DINH_KEM_BY_ID_HO_SO), hoSoNgoaiTe1.getIdHoSo());

		if (tepDinhKemHoSo1s == null || tepDinhKemHoSo1s.isEmpty()) return false;
		boolean isFindLoai1 = false;
		boolean isFindLoai2 = false;
		for (TepDinhKemHoSo1 item : tepDinhKemHoSo1s) {
			if (item.getMaLoaiTepDinhKem().equals(String.valueOf(SBV01Constants.TepDinhKems.TEP_LOAI_1))) {
				isFindLoai1 = true;
			} else if (item.getMaLoaiTepDinhKem().equals(String.valueOf(SBV01Constants.TepDinhKems.TEP_LOAI_2))) {
				isFindLoai2 = true;
			}
		}
		if (!isFindLoai1 || !isFindLoai2) return false;
	...
	private boolean guiHoSo(HoSoNgoaiTe1 hoSoNgoaiTe1, KySoModel kySoModel) {
		...
		if (kySoModel.isRequiredSign()) {
			sendMessage.setSignedXml(kySoModel.getXml());
		}
	...
	private String getTenLoaiTepTin(HttpServletRequest request, String maLoaiTepTin) {
		if (StringUtils.hasText(maLoaiTepTin) && maLoaiTepTin.equals(String.valueOf(SBV01Constants.TepDinhKems.TEP_LOAI_2))) {
			return getMessage("sbv.01.form.teptin.03", null, request.getLocale());
		}
		if (StringUtils.hasText(maLoaiTepTin) && maLoaiTepTin.equals(String.valueOf(SBV01Constants.TepDinhKems.TEP_LOAI_3))) {
