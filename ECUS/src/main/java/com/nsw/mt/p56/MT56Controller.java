...
@Controller
@RequestMapping(value = "/mt/56")
@PropertySource("classpath:mt_api.properties")
public class MT56Controller extends BaseController {
	public static final Logger LOGGER = LoggerFactory.getLogger(MT56Controller.class);
	...
	public String index(HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttributes,
		...
		return environment.getProperty("mt.p56.home");
	}
	...
	public String edit(ModelMap model, HttpServletRequest request, @PathVariable("id") Long id,
		...
		return environment.getProperty("mt.p56.edit");
	}
	...
	public String viewLicense(ModelMap model, HttpServletRequest request, @PathVariable("type") Integer type,
			@PathVariable("procedureId") String procedureId, @PathVariable("id") Long id) {
		try {
				setBreadcrumb(false, model);
				model.addAttribute(AppConstant.DanhMuc.Menu, getMenuData());
				model.addAttribute(AppConstant.DanhMuc.Version, AppConstant.getVersion());
		...
		return "nsw.mt.page.56.view.license";
	}
	@RequestMapping(value = "/vanban", method = RequestMethod.GET)
	public String viewLicenseT(ModelMap model) {
		try {
				setBreadcrumb(false, model);
				model.addAttribute(AppConstant.DanhMuc.Menu, getMenuData());
				model.addAttribute(AppConstant.DanhMuc.Version, AppConstant.getVersion());
		...
		return "nsw.mt.page.56.vanban";
	}
	@ResponseBody
	@RequestMapping(value = MTConstants.URL_XEM_GIAY_PHEP
			+ "/{id}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<Object> getGiayPhep(HttpServletRequest request, @PathVariable("id") long id) {

		try {
			Map<String, Object> params = new HashMap<>();
			ResponseJson responseJson = callRestTemplate(environment.getRequiredProperty("mt.backend")
					+ environment.getRequiredProperty("mt.56.xem_giay_phep"), id, HttpMethod.POST, params);
			return createResponseEntity(responseJson.getData(), 1l, "", true, HttpStatus.OK);
		} catch (Exception e) {
			LOGGER.error("[layDanhSachHoSo]", e);
		}
		return createResponseEntity(null, 0L,
				getMessage(MTConstants.MessageKeys.ACTION_ERROR, null, request.getLocale()), false, HttpStatus.OK);
	}

	@ResponseBody
	@RequestMapping(value = "/call_service", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<Object> callBackendService(HttpServletRequest request,
		...
		try {
			...
			ResponseJson responseJson = callRestTemplate(environment.getProperty("mt.backend") + urlBackEnd,
					requestObject, httpMethod, params);
			if (form.containsKey("URL_GET_TOTAL")) {
				String urlGetTotal = form.get("URL_GET_TOTAL").toString();
				ResponseJson response = callRestTemplate(environment.getProperty("mt.backend") + urlGetTotal,
						requestObject, httpMethod, params);
	...
	@RequestMapping(value = "/saveWithFile/{funcType}", method = RequestMethod.POST)
	public ResponseEntity<Object> saveWithFile(DefaultMultipartHttpServletRequest request,
			@PathVariable("funcType") int funcType) {
		ResponseJson json = new ResponseJson();
		json.setSuccess(false);
		json.setMessage(getMessage(MTConstants.MessageKeys.ACTION_ERROR, null, request.getLocale()));
		try {
			String uri = environment.getProperty("mt.backend") + environment.getProperty("mt.56.save_with_file") + "/"
					+ funcType;
	...
	@ResponseBody
	@RequestMapping(value = MTConstants.URL_PAGE_HISTORY
			+ "/{idHoSo}/{currentPage}", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<Object> getProcessHistory(HttpServletRequest request, @PathVariable("idHoSo") Long fiIdHoso,
		...
		try {
			...
			ResponseJson responseJson = callRestTemplate(
					environment.getRequiredProperty("mt.backend") + environment.getRequiredProperty("mt.56.history"),
					params, HttpMethod.POST, null);
			List lst = (List) responseJson.getData();
			ResponseJson response = callRestTemplate(
					environment.getProperty("mt.backend") + environment.getRequiredProperty("mt.56.history_total"),
					params, HttpMethod.POST, null);
	...
	@ResponseBody
	@RequestMapping(value = MTConstants.URL_PAGE_HISTORY
			+ "/giayphep/{soGiayPhep}/{currentPage}", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<Object> getProcessHistoryGP(HttpServletRequest request,
			@PathVariable("soGiayPhep") String fiSoGiayphep, @PathVariable("currentPage") Long currentPage) {
		try {
			Object principal = SecurityContextHolder.getContext().getAuthentication().getPrincipal();
			if (!(principal instanceof UserDetails)) {

				return createResponseEntity(null, 0L,
						getMessage(MTConstants.MessageKeys.ACTION_ERROR, null, request.getLocale()), false,
						HttpStatus.OK);
			}
			...
			params.put("pageSize", 10);
			ResponseJson responseJson = callRestTemplate(
					environment.getRequiredProperty("mt.backend") + environment.getRequiredProperty("mt.56.historyGP"),
					params, HttpMethod.POST, null);
			List lst = (List) responseJson.getData();
			ResponseJson response = callRestTemplate(
					environment.getProperty("mt.backend") + environment.getRequiredProperty("mt.56.history_totalGP"),
					params, HttpMethod.POST, null);
			Long total = ((Integer) response.getData()).longValue();
			return createResponseEntity(lst, total, "", true, HttpStatus.OK);
		} catch (Exception e) {
	...
	public ResponseEntity<Object> xinRut(HttpServletRequest request, @RequestBody Object lyDo) {
		try {
			...
			Map<String, Object> params = new HashMap<>();
			ResponseJson responseJson = callRestTemplate(
					environment.getRequiredProperty("mt.backend") + environment.getRequiredProperty("mt.56.rut_ho_so"),
					lyDo, HttpMethod.POST, params);
			return createResponseEntity(responseJson.getData(), 1l, "", true, HttpStatus.OK);
	...
	public ResponseEntity<Object> xinSuaGiayPhep(HttpServletRequest request, @RequestBody Object lyDo) {
		...
		try {
			...
			Map<String, Object> params = new HashMap<>();
			ResponseJson responseJson = callRestTemplate(
					environment.getRequiredProperty("mt.backend")
							+ environment.getRequiredProperty("mt.56.xin_sua_giay_phep"),
					lyDo, HttpMethod.POST, params);
			return createResponseEntity(responseJson.getData(), 1l, "", true, HttpStatus.OK);
	...
	public ResponseEntity<Object> traGiayPhep(HttpServletRequest request, @RequestBody Object lyDo) {
		...
		try {
			...
			Map<String, Object> params = new HashMap<>();
			ResponseJson responseJson = callRestTemplate(environment.getRequiredProperty("mt.backend")
					+ environment.getRequiredProperty("mt.56.tra_giay_phep"), lyDo, HttpMethod.POST, params);
			return createResponseEntity(responseJson.getData(), 1l, "", true, HttpStatus.OK);
	...
	public ResponseEntity<Object> downloadNewFile(HttpServletResponse response,
			@PathVariable("idDinhKem") Long idDinhKem) {
			try {
				if (idDinhKem != null) {
					String uri = environment.getProperty("mt.backend") + environment.getProperty("mt.56.download") + "/"
							+ idDinhKem;
					HttpHeaders headers = new HttpHeaders();
	...
	private String getTitleLabel(String procedureId) {
        switch (procedureId) {
            case "BGTVT0600056":
                return "Chấp thuận khai thác tuyến vận tải hành khách cố định liên vận quốc tế giữa Việt Nam - Lào - Campuchia" + 
                		"";
            case "BGTVT0600057":
                return "Gia hạn giấy phép liên vận CLV đối với phương tiện của Lào, Campuchia tại Việt Nam" + 
                		"";
            case "BGTVT0600058":
                return "Cấp giấy phép hoạt động vận tải đường bộ quốc tế GMS" + 
                		"";
            case "BGTVT0600059":
                return "Cấp lại giấy phép hoạt động vận tải đường bộ quốc tế GMS do hết hạn" + 
                		"";
            case "BGTVT0600060":
                return "Cấp lại giấy phép hoạt động vận tải đường bộ quốc tế GMS do bị mất" + 
                		"";
            default:
                return "Cấp Giấy phép liên vận CLV cho phương tiện thương mại";
        }
    }
}
