package com.nsw.mt.p44.model;

import java.io.Serializable;
import java.util.Date;

public class Tbstuyencodinh implements Serializable {

    private Long fiIdTuyen;

    private String fiMaTuyen;

    private String fiTenTuyen;

    private String fiMaCqql;

    private String fiMaTinhDi;

    private String fiMaTinhDen;

    private String fiBenDi;

    private String fiBenDen;

    private String fiHanhtrinh;

    private String fiKhoangcach;

    private String fiLlQhoach;

    private String fiLlBandau;

    private String fiLlDacap;

    private String fiGhichu;

    private String fiDiemDung;

    private String fiLoaituyen;

    private String fiLoaiQhoach;

    private String fiMaCk;

    private Long fiHoatdong;

    private String fiNguoitao;

    private Date fiNgaytao;

    private Date fiNgCapnhat;

    public Tbstuyencodinh() {
        super();
    }

    public void setFiIdTuyen(Long fiIdTuyen) {
        this.fiIdTuyen = fiIdTuyen;
    }

    public Long getFiIdTuyen() {
        return this.fiIdTuyen;
    }

    public void setFiMaTuyen(String fiMaTuyen) {
        this.fiMaTuyen = fiMaTuyen;
    }

    public String getFiMaTuyen() {
        return this.fiMaTuyen;
    }

    public void setFiTenTuyen(String fiTenTuyen) {
        this.fiTenTuyen = fiTenTuyen;
    }

    public String getFiTenTuyen() {
        return this.fiTenTuyen;
    }

    public void setFiMaCqql(String fiMaCqql) {
        this.fiMaCqql = fiMaCqql;
    }

    public String getFiMaCqql() {
        return this.fiMaCqql;
    }

    public void setFiMaTinhDi(String fiMaTinhDi) {
        this.fiMaTinhDi = fiMaTinhDi;
    }

    public String getFiMaTinhDi() {
        return this.fiMaTinhDi;
    }

    public void setFiMaTinhDen(String fiMaTinhDen) {
        this.fiMaTinhDen = fiMaTinhDen;
    }

    public String getFiMaTinhDen() {
        return this.fiMaTinhDen;
    }

    public void setFiBenDi(String fiBenDi) {
        this.fiBenDi = fiBenDi;
    }

    public String getFiBenDi() {
        return this.fiBenDi;
    }

    public void setFiBenDen(String fiBenDen) {
        this.fiBenDen = fiBenDen;
    }

    public String getFiBenDen() {
        return this.fiBenDen;
    }

    public void setFiHanhtrinh(String fiHanhtrinh) {
        this.fiHanhtrinh = fiHanhtrinh;
    }

    public String getFiHanhtrinh() {
        return this.fiHanhtrinh;
    }

    public void setFiKhoangcach(String fiKhoangcach) {
        this.fiKhoangcach = fiKhoangcach;
    }

    public String getFiKhoangcach() {
        return this.fiKhoangcach;
    }

    public void setFiLlQhoach(String fiLlQhoach) {
        this.fiLlQhoach = fiLlQhoach;
    }

    public String getFiLlQhoach() {
        return this.fiLlQhoach;
    }

    public void setFiLlBandau(String fiLlBandau) {
        this.fiLlBandau = fiLlBandau;
    }

    public String getFiLlBandau() {
        return this.fiLlBandau;
    }

    public void setFiLlDacap(String fiLlDacap) {
        this.fiLlDacap = fiLlDacap;
    }

    public String getFiLlDacap() {
        return this.fiLlDacap;
    }

    public void setFiGhichu(String fiGhichu) {
        this.fiGhichu = fiGhichu;
    }

    public String getFiGhichu() {
        return this.fiGhichu;
    }

    public void setFiDiemDung(String fiDiemDung) {
        this.fiDiemDung = fiDiemDung;
    }

    public String getFiDiemDung() {
        return this.fiDiemDung;
    }

    public void setFiLoaituyen(String fiLoaituyen) {
        this.fiLoaituyen = fiLoaituyen;
    }

    public String getFiLoaituyen() {
        return this.fiLoaituyen;
    }

    public void setFiLoaiQhoach(String fiLoaiQhoach) {
        this.fiLoaiQhoach = fiLoaiQhoach;
    }

    public String getFiLoaiQhoach() {
        return this.fiLoaiQhoach;
    }

    public void setFiMaCk(String fiMaCk) {
        this.fiMaCk = fiMaCk;
    }

    public String getFiMaCk() {
        return this.fiMaCk;
    }

    public void setFiHoatdong(Long fiHoatdong) {
        this.fiHoatdong = fiHoatdong;
    }

    public Long getFiHoatdong() {
        return this.fiHoatdong;
    }

    public void setFiNguoitao(String fiNguoitao) {
        this.fiNguoitao = fiNguoitao;
    }

    public String getFiNguoitao() {
        return this.fiNguoitao;
    }

    public void setFiNgaytao(Date fiNgaytao) {
        this.fiNgaytao = fiNgaytao;
    }

    public Date getFiNgaytao() {
        return this.fiNgaytao;
    }

    public void setFiNgCapnhat(Date fiNgCapnhat) {
        this.fiNgCapnhat = fiNgCapnhat;
    }

    public Date getFiNgCapnhat() {
        return this.fiNgCapnhat;
    }

}
