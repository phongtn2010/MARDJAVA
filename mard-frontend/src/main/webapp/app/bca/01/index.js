...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/bca/01/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdlichsu01(res.data, data.currentPage * data.pageSize) : [];
                    self.historyItems([]);
...
function IndexVM(params) {
    ...
    self.search = function (page, pushState) {
        if (!RAW_HS_STATUS || RAW_HS_STATUS.length <= 0) {
            app.getCategory('/bca/01/danhmuc', 'HS_TRANGTHAI', null, function (res) {
                if (res.success) {
    ...
    self.searchAfterHasCatgory = function (page, pushState) {
        ...
        app.makePost({
            url: "/bca/01/hoso/timkiem",
            data: JSON.stringify(data),
            success: function (res) {
                window.stateChanging = false;
                if (res.success) {
                    self.Items([]);
                    var list = res.data ? mapTbdhoso01(res.data, data.currentPage * data.pageSize) : [];
                    self.Items(list);
                    self.paging().update({
                        totalCount: res.data ? res.total : 0,
                        pageSize: self.pageSize(),
    ...
    self.btnAddNewClick = function (e) {
        document.location = app.appContext + '/bca/01/edit?maThuTuc=BCA0100001';
        return false;
    };
    ...
    self.bXemClick = function (item, e) {
        document.location = app.appContext + '/bca/01/view?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=BCA0100001';
        return false;
    };
    /**
     * Xem Thông báo
    ...
    self.bXemThongBaoClick = function (item, e) {
        document.location = app.appContext + '/bca/01/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=cert&maThuTuc=BCA0100001';
        return false;
    };
    /**
     * Xem Thông tin phi
    ...
    self.bThongTinPhiClick = function (item, e) {
        document.location = app.appContext + '/bca/01/charge?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso()+ '&fiTrangthai=' + item.fiTrangthai();
        return false;
    };
    /**
     * Xem Lich su
    ...
    self.bSuaClick = function (item, e) {
        document.location = app.appContext + '/bca/01/edit?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=BCA0100001';
        return false;
    };
    /**
     * Xin sua ho so
    ...
    self.bXinSuaClick = function (item, e) {
        document.location = app.appContext + '/bca/01/edit?fiIdHoso=' + item.fiIdHoso() + '&requestedit=true&maThuTuc=BCA0100001';
        return false;
    };
    /**
     * Hủy hồ sơ (xóa)
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/bca/01/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
    ...
    self.fiTrangThaiClick = function (item, e) {
        switch (item.fiTrangthai()) {
            ...
            case  YC_BO_SUNG:
                ...
                break;
            case  TU_CHOI_CAP_GIAYPHEP:
                self.createPopFiTrangThai('kqxl-template', item);
                self.kqxlHoSo({
                    fiMaHoso: item.fiMaHoso(),
                    fiIdHoso: item.fiIdHoso(),
                    fiTrangthai: TU_CHOI_CAP_GIAYPHEP
                }, function (d) {
                    ko.applyBindings(new KetQuaXLVM(d.data), document.getElementById('kqxl-form'));
                });
                break;
            case  DUOC_CAP_GIAYPHEP:
            case  DA_CAP_LAI_GIAYPHEP:
                document.location = app.appContext + '/bca/01/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=cert&maThuTuc=' + self.maThuTuc();
                break;
//            case  YC_SUA_GIAYPHEP:
//                self.createPopFiTrangThai('kqycsuavb-template', item);
//                self.kqYeuCauSuaVB({
//                    fiMaHoso: item.fiMaHoso(),
//                    fiIdHoso: item.fiIdHoso(),
//                    fiTrangthai: YC_SUA_GIAYPHEP
//                }, function (d) {
//                    ko.applyBindings(new YeuCauSuaVBVM(d.data), document.getElementById('kqxl-form'));
//                });
//                break;
//            case  DONG_Y_YC_SUA_GP:
//                self.createPopFiTrangThai('kqycsuavb-template', item);
//                self.kqYeuCauSuaVB({
//                    fiMaHoso: item.fiMaHoso(),
//                    fiIdHoso: item.fiIdHoso(),
//                    fiTrangthai: DONG_Y_YC_SUA_GP
//                }, function (d) {
//                    ko.applyBindings(new YeuCauSuaVBVM(d.data), document.getElementById('kqxl-form'));
//                });
//                break;
    ...
    self.guiYcXuly = function (message) {
        $.ajax({
            ...
            cache: false,
            crossDomain: true,
            url: app.appContext + '/bca/01/hoso/xuly',
...
$(document).ready(function () {
    var d = {};
    $('#loading10').show();
    $.when(app.getCategory('/bca/01/danhmuc', 'DM_TRANGTHAI', null, function (res) {
        if (res.success) {
