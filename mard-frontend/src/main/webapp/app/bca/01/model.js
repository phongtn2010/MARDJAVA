...
function Tbdhoso01(STT, item) {
    ...
    self.bXemThongBao = ko.dependentObservable(function () {
        //return true;
        return [DONG_Y_YC_SUA_GP,DUOC_CAP_GIAYPHEP, DA_CAP_LAI_GIAYPHEP, TU_CHOI_YC_SUA_GP, YC_SUA_GIAYPHEP, DA_THU_HOI].indexOf(self.fiTrangthai()) >= 0;
    }, self);
...
function TbdhsHanghoa01(data, i) {
    ...
    self.fiSoluong = ko.observable(data ? data.fiSoluong : null).extend({
        required: {message: NSWLang["common_msg_formvaild_required"], params: true},
        maxLength: {message: ' phải nhập số hoặc số thập phân sau dấu phẩy 2 chữ số', params: 15},
        pattern: {
            message: 'Không  được nhập chữ và nhập trước dấu chấm quá 13 chữ số, sau dấu chấm 2 chữ số , số âm', 
            params: /^([0-9]{1,13}(\.[0-9]{1,2})?)$/
        }
    });
...
function TbdhsNhomHanghoa01(data, i) {
    ...
    debugger;
     ...
     if(self.fiThoigianraTungay() !=null) self.fiThoigianraTungay(new Date(self.fiThoigianraTungay()));  
      self.fiThoigianraDenngay = ko.observable(data ? data.fiThoigianraDenngay : null);
            
     if(self.fiThoigianraDenngay() !=null) self.fiThoigianraDenngay(new Date(self.fiThoigianraDenngay()));  
    
    self.fiCuakhauVao = ko.observable(data ? data.fiCuakhauVao : null).extend({
        required: {message: NSWLang["common_msg_formvaild_required"], params: true},
        maxLength: {message: 'Tối đa 250 ký tự', params: 250}
    });
    self.fiCuakhauRa = ko.observable(data ? data.fiCuakhauRa : null).extend({
        required: {message: NSWLang["common_msg_formvaild_required"], params: true},
        maxLength: {message: 'Tối đa 250 ký tự', params: 250}
    });
    ...
    self.changeHTItem = function(){
        ...
        if(self.fiHinhthuc()===2){
            $("#tgratungay_lbl_"+self.STT()).show();
            $("#tgradenngay_lbl_"+self.STT()).show();
        }else{
            debugger;
            $("#tgratungay_lbl_"+self.STT()).hide();
            $("#tgradenngay_lbl_"+self.STT()).hide();
            $("#fiThoigianraTungay_lbl_"+self.STT()).hide();
            $("#fiThoigianraDenngay_lbl_"+self.STT()).hide();
...
var Tbdgiayphep01 = function (STT, item) {
    ...
    self.isDisable = ko.dependentObservable(function () {
        // return True
        return [YC_SUA_GIAYPHEP,DONG_Y_YC_SUA_GP].indexOf(self.fiTrangthai()) >= 0;

    }, self);
     self.bXemThongBao = ko.dependentObservable(function () {
        // return True
        return [YC_SUA_GIAYPHEP,TU_CHOI_YC_SUA_GP,DA_CAP_LAI_GIAYPHEP,DUOC_CAP_GIAYPHEP].indexOf(self.fiTrangthai()) >= 0;
