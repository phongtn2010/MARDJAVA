...
//        }
        ...
        for(var i=0;i<self.lstNguoiDieukhien().length;i++){
            var ndkObj = self.lstNguoiDieukhien()[i];
            errorHoso = ngdk(ndkObj,(i+1));
        }
...
//        
//        for(var i=0;i<self.lstNguoiDieukhien().length;i++){
//            if(self.lstNguoiDieukhien()[i].nguoiDkError().length>0){
//                errorHoso = false;
//            }
//        }
...
//        return errorHoso;
    ...
    //tamdt
    function ngdk(objNDK,stt){
        var flag = true;
        //show lbl err (Loai,ten,cmnd,bks)
        if(objNDK.fiLoai() === undefined) {
            flag = false;
            $("#fiLoai_lbl_"+stt).show();
        }else $("#fiLoai_lbl_"+stt).hide();

        if(objNDK.fiTenNguoidk()===null || objNDK.fiTenNguoidk()==="") {
            flag = false;
            $("#fiTenNguoidk_lbl_"+stt).show();
        }else $("#fiTenNguoidk_lbl_"+stt).hide(); 

        if(objNDK.fiSocmndNguoidk()===null || objNDK.fiSocmndNguoidk()==="") {
            flag = false;
            $("#fiSocmndNguoidk_lbl_"+stt).show();
        }else $("#fiSocmndNguoidk_lbl_"+stt).hide();

        if(objNDK.fiBksPhuongtien()===null || objNDK.fiBksPhuongtien() === "") {
            flag = false;
            $("#fiBksPhuongtien_lbl_"+stt).show();
        }else $("#fiBksPhuongtien_lbl_"+stt).hide();

        if(objNDK.flagValid1()===false){
            flag = false;
            $("#fiTenDv_lbl_"+stt).show();
        }else{
            $("#fiTenDv_lbl_"+stt).hide();
        }
        
        if(objNDK.flagValid2()===false){
            flag = false;
            $("#fiKhoiluongVc_lbl_"+stt).show();
        }else{
            $("#fiKhoiluongVc_lbl_"+stt).hide();
        }
        
        if(objNDK.flagValid3()===false){
            flag = false;
            $("#fiGhichu_lbl_"+stt).show();
        }else{
            $("#fiGhichu_lbl_"+stt).hide();
        }
        if(objNDK.nguoiApTaiError().length>0 || objNDK.nguoiDkError().length>0){
            flag = false;
        }
        
        return flag;
    }
    self.valiLydo = function(){
        var flag = true;
        if(self.fiLydo() != null && self.fiLydo() != "") {
            $('#fiLydo_').hide();
        } else{
            $('#fiLydo_').show();
            flag = false;
