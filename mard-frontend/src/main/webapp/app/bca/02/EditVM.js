...
function EditVM(options) {
    ...
    if (self.formVM().fiIdHoso() > 0) {
        if (self.formVM().fiTrangthai() == TAO_MOI
                ...
                || self.formVM().fiTrangthai() == DA_DUYET) {
            self.btnGui(true);
        }
    } else {
        self.btnGui(true);
    ...
    self.btnLuuClick = function () {
        ...
        if (!self.formVM().isValidForm()) {
            ...
            if(self.formVM().lstNguoiDieukhien().length===0){
                app.Alert('Chưa khai báo danh sách người điều khiển phương tiện, người áp tải và phương tiện vận chuyển.');
                return;
            }
        }
        ...
        var url = !fiIdHoso || fiIdHoso <= 0 ? '/bca/02/hoso/taomoi' : '/bca/02/hoso/capnhap';
...
$(document).ready(function () {
    ...
    $.when(
            ...
            app.getCategory('/bca/02/danhmuc', 'DM_LOAINGUOI', null, function (res) {
                if (res.success) {
                    options.lstLoainguoi = res.data;
                } else {
                    options.lstLoainguoi = [];
                }
            }),
            app.getCategory('/bca/02/danhmuc', 'DM_DONVI', null, function (res) {
                if (res.success) {
                    options.lstDonvi = res.data;
                } else {
                    options.lstDonvi = [];
                }
            }),
            app.getCategory('/bca/02/danhmuc', 'DM_LOAIFILE', null, function (res) {
                if (res.success) {
    ...
    var init = function () {
        if (options && options.fiIdHoso > 0) {
            var url = '/bca/02/hoso/t/' + options.fiIdHoso;
            app.makePost({
                url: url,
                data: JSON.stringify({}),
                success: function (d) {
            ...
            ko.applyBindings(editVM, document.getElementById('EditVM'));
            $(".fiMaHs").select2({placeholder: '-- Chọn --', width: '100%', allowClear: true});
        }
    };
});
