...
function Tbdhoso02(STT, item) {
    ...
    if ([DUOC_CAP_GIAYPHEP, DA_CAP_LAI_GIAYPHEP, YC_SUA_GIAYPHEP, DONG_Y_YC_SUA_GP, TU_CHOI_CAP_GIAYPHEP,TU_CHOI_YC_SUA_GP].indexOf(self.fiTrangthai()) < 0) {
...
function TbdhsNguoidk02(data, i) {
    ...
    self.fiKhoiluongVc = ko.observable(data.fiKhoiluongVc ? data.fiKhoiluongVc : null).extend({
        ...
        pattern: {
            message: 'Không nhập chữ , quá 12 chữ số trước dấu chấm , sau chấm phẩy quá 3 chữ số va số âm ', 
    ...
    self.changeValidData = function () {
        if(self.fiLoai() === undefined || self.fiLoai() === null) {
            $("#fiLoai_lbl_"+self.STT()).show();
        }else {
            $("#fiLoai_lbl_"+self.STT()).hide();
            validType();
        }

        if(self.fiTenNguoidk()===null || self.fiTenNguoidk() === ""){
            $("#fiTenNguoidk_lbl_"+self.STT()).show();
        }else{
            $("#fiTenNguoidk_lbl_"+self.STT()).hide();
        } 

        if(self.fiSocmndNguoidk()===null || self.fiSocmndNguoidk() === ""){
            $("#fiSocmndNguoidk_lbl_"+self.STT()).show();
        }else {
            $("#fiSocmndNguoidk_lbl_"+self.STT()).hide();
        }

        if(self.fiBksPhuongtien()===null || self.fiBksPhuongtien() === "") {
            $("#fiBksPhuongtien_lbl_"+self.STT()).show();
        }else{
            $("#fiBksPhuongtien_lbl_"+self.STT()).hide();
        }
    ...
    function validType(){
        if(self.fiLoai()===1){
            if(self.fiTenDv()===undefined || self.fiTenDv()===null) {
            ...
            if(self.fiKhoiluongVc()===null || self.fiKhoiluongVc()==="") {
            ...
            if(self.fiGhichu()===undefined || self.fiGhichu()===null || self.fiGhichu()=== ""){
