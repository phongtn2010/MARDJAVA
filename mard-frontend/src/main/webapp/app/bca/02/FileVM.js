...
function FileVM(item, idHoso) {
    ...
    self.canDeleteFile = ko.computed(function () {
        return (self.fiTenTep() !== null && self.fiTenTep() !== undefined);
    }, this);
    ...
    self.downloadUrl = ko.computed(function () {
        if (self.fiTenTep()) {
            if (self.fiIdHoso() == null) {
                return app.appContext + '/bca/02/file/bca/02/' + self.fiGuiid();
            } else {
                return app.appContext + '/bca/02/download/' + self.fiGuiid() + '/' + self.fiIdDk();
            }
        }
        return null;
    }, this);
