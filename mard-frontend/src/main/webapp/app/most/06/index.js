...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/most/06/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    debugger;
                    var list = res.data ? mapTbdlichsu06(res.data, data.currentPage * data.pageSize) : [];
                    self.historyItems([]);
...
function IndexVM(params) {
        ...
        app.makePost({
            ...
            success: function (res) {
                ...
                if (res.success) {
                    ...
                    self.Items([]);
                    var list = res.data ? mapTbdhoso06(res.data, data.currentPage * data.pageSize) : [];
                    self.Items(list);
                    self.paging().update({
                        totalCount: res.data ? res.total : 0,
                        pageSize: self.pageSize(),
    ...
    self.btnAddNewClick = function (e) {
        document.location = app.appContext + '/most/06/edit';
        return false;
    };
    ...
    self.bXemClick = function (item, e) {
        document.location = app.appContext + '/most/06/view?fiIdHoso=' + item.fiIdHoso();
        debugger;
        return false;
    };
    /**
    ...
    self.bXemThongBaoClick = function (item, e) {
        document.location = app.appContext + '/most/06/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=cert';
        return false;
    };
    /**
     * Xem Lich su
    ...
    self.bSuaClick = function (item, e) {
        document.location = app.appContext + '/most/06/edit?fiIdHoso=' + item.fiIdHoso();
        return false;
    };
    ...
    self.bXinSuaClick = function (item, e) {
        document.location = app.appContext + '/most/06/edit?fiIdHoso=' + item.fiIdHoso() + '&requestedit=true';
        return false;
    };
    /**
     * Hủy hồ sơ (xóa)
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/most/06/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
...
$(document).ready(function () {
    var d = {};
    $('#loading10').show();
    $.when(app.getCategory('/most/06/danhmuc', 'DM_TRANGTHAI', null, function (res) {
        if (res.success) {
