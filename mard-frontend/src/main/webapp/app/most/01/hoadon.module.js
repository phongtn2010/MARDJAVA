 ...
 */
var HoaDonController = function () { };
HoaDonController.prototype = {
    ministryCode: 'most',
    code: '01',
    pagination: {
        isReady: false,
    ...
    registerEvents: function () {
        ...
        $('#hoadon-file-form').find('.tooltips').tooltip();

        $("#btnFileHoaDonThemMoi").on('click', function (e) {
            e.stopPropagation();
            e.preventDefault();
            var isOk = app.isFormVaild('hoadon-file-form');
            if (!isOk)
                return;

            var fileInfo = app.form2Object('#hoadon-file-form');

            var formObj = $('#hoadon-file-form');
            var d = fileInfo.fiNgayCap.trim().toValidDate();
            var now = new Date().toDayFirstString().toValidDate();

            var fieldName = formObj.find('#txtNgayHoaDon').attr('field');
            if (Util.compareDate(d, now) === 1) {
                app.Alert(NSWLang[fieldName] + ' <b>' + NSWLang['common_msg_khonglonhon'] + '</b> ' + NSWLang['common_msg_ngayhientai']);
                return false;
            }
            var file = $('#hoadon-ip')[0].files[0];
            fileInfo.fiFileSize = file.size;
            fileInfo.fiIdDinhkem = 1 - (self.list.length + 1);
            fileInfo.fiMaLoai = self.fileType;
            fileInfo.fiTenLoai = NSWLang["most_01_filetypename_hoadon"];
            fileInfo.fiTenTep = file.name;
            if (Util.validateAttachByDom('hoadon-ip')) {
        ...
        $('#hoadon-tmpl').find('.date-picker').datepicker({rtl: false, orientation: "left", autoclose: true});
    ...
    bindEventsGrid: function () {
        ...
        con.find('.fa-remove').off('click').on('click', function (e) {
            ...
            if (i > -1) {
                var pop = app.popup({
                    title: NSWLang["common_msg_thong_bao"],
                    html: '<i class="fa fa-3x fa-warning"></i> ' + NSWLang["most_common_msg_xoahoadon"] + ' <b>' + docCode + '</b>',
                    width: 400,
                    buttons: [
                        {
    ...
    clearForm: function () {
        $('#txtSoHoaDon').val('').focus();
        $('#txtNgayHoaDon').val('');
        $('#hoadon-tmpl').find('.fileinput-filename').text('');
    },
    isVaild: function () {
        if (this.list == null || (this.list != null && this.list.length == 0)) {
            alert(NSWLang["common_msg_chuanhapthongtin"] + NSWLang["most_01_hoadon"]);
            return false;
        }
        return true;
    }
