...
HopDongController.prototype = {
    ...
    registerEvents: function () {
        ...
        $("#btnFileHopDongThemMoi").on('click', function (e) {
            ...
            fileInfo.fiTenLoai = NSWLang["most_02_filetypename_hopdong"];

            if (Util.validateFieExtensionByDom('hopdong-ip')) {
    ...
    bindGrid: function () {
        ...
        if (self.list != null && self.list.length > 0) {
            $('#btnFileHopDongThemMoi').parents('tfoot').hide();
        } else {
            $('#btnFileHopDongThemMoi').parents('tfoot').show();
        }
    ...
    clearForm: function () {
        ...
        document.getElementById("hopdong-ip").value = "";
    },
    isVaild: function () {
        if (this.list == null || (this.list != null && this.list.length == 0)) {
            app.Alert(NSWLang["common_msg_chuanhapthongtin"] + NSWLang["most_01_hopdong"]);
            return false;
        }
        return true;
    }
