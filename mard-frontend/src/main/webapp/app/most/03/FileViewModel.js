...
//        required: {message: NSWLang["common_msg_formvaild_required"], params: true}
    ...
    self.showDonViThuNghiem = ko.observable(false);
    self.fiNgayguiMau = ko.observable(null);
    self.fiNgaynhanKq = ko.observable(null);
    self.fiDvThunghiem = ko.observable(null);

    self.lstLoaiFile = ko.observableArray([]);
    self.lstDonViThuNghiem = ko.observableArray([]);
    ...
    self.validateForm = function () {
        ...
        if (self.isFourType()) {
            if (!self.fiNgayguiMau() || !self.fiNgaynhanKq() || !self.fiDvThunghiem()) {
                app.Alert('Bạn phải nhập đầy đủ các trường được đánh dấu *');
                return false;
            }
        }
    ...
    self.isFourType = function () {
        return parseInt(self.fiMaLoai()) === 4;
    };
    ...
    self.uploadClick = function () {
        ...
        var fileInfo = {
            ...
            fiDvThunghiem: self.isFourType() ? $('#fiDvThunghiem').find("option:selected").text() : null,
            fiNgayguiMau: self.fiNgayguiMau(),
            fiNgaynhanKq: self.fiNgaynhanKq()
        };

        var cb = function (d) {
            var data = d.data;
            self.showDonViThuNghiem(false);
            if (d.success) {
    ...
    self.toJSON = function () {
        ...
        if (jsonList.length > 0) {
            for (var i = 0; i < jsonList.length; i++) {
                ...
                delete jsonList[i]["fiNgayguiMauText"];
                delete jsonList[i]["fiNgaynhanKqText"];
    ...
    self.onChange = function () {
        if (parseInt(self.fiMaLoai()) === 4) {
            self.showDonViThuNghiem(true);
        } else {
            self.showDonViThuNghiem(false);
        }
    };
    ...
    $.when(
            ...
            app.getCategory('/most/03/danhmuc', 'DM_DVTHUNGHIEM', null, function (res) {
                if (res.success) {
                    self.lstDonViThuNghiem(res.data);
                } else {
                    self.lstDonViThuNghiem([]);
                }
            })
            ).done(function (data) {
        $('#loading10').hide();
