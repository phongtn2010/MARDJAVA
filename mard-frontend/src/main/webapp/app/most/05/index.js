...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/most/p05/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdlichsu05(res.data, data.currentPage * data.pageSize) : [];
                    self.historyItems([]);
...
function TBPhiThamDinhVM(options) {
    debugger;
    if (options) {
        ko.mapping.fromJS(options, {}, this);
    }
...
function IndexVM(params) {
    ...
    self.trangThaiLePhi = ko.observable(null);
    ...
    //tamdt
    self.fiTrangThaiLePhi = [
        {name: "Lệ phí thẩm định", id: 0},
        {name: "Xác nhận thanh toán", id: 1}
    ]
    ...
    self.applyState = function (data) {
        if (!data) {
            ...
            self.trangThaiLePhi(data.trangThaiLePhi);
    ...
    self.searchAfterHasCatgory = function (page, pushState) {
        ...
        var data = {
            ...
            trangThaiLePhi: self.trangThaiLePhi(),
            maHoSo: self.maHoSo(),
        ...
        app.makePost({
            ...
            success: function (res) {
                ...
                if (res.success) {
                    debugger;
                    self.Items([]);
                    var list = res.data ? mapTbdhoso05(res.data, data.currentPage * data.pageSize) : [];
                    self.Items(list);
                    self.paging().update({
                        totalCount: res.data ? res.total : 0,
                        pageSize: self.pageSize(),
    ...
    self.btnAddNewClick = function (e) {
        document.location = app.appContext + '/most/05/edit';
        return false;
    };
    ...
    self.bXemClick = function (item, e) {
        document.location = app.appContext + '/most/05/view?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=BKHCN0500001';
        return false;
    };
    /**
     * Xem Thông báo
    ...
    self.bXemThongBaoClick = function (item, e) {
        document.location = app.appContext + '/most/05/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=cert&maThuTuc=BKHCN0500001';
        return false;
    };
    /**
     * Xem Thông tin phi
     ...
     * @returns {Boolean}
     */
    self.bThongTinPhiClick = function (item, e) {
        document.location = app.appContext + '/most/05/charge?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&fiTrangthai=' + item.fiTrangthai();
        return false;
    };
    /**
     * Xem Lich su
    ...
    self.bSuaClick = function (item, e) {
        document.location = app.appContext + '/most/05/edit?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=BKHCN0500001';
        return false;
    };
    /**
     * Xin sua ho so
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/most/p05/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
    ...
    self.fiTrangThaiLePhiClick = function (item, e) {
        switch (item.fiTrangThaiLePhi()) {
            case  TT_LE_PHI_THAM_DINH:
                self.createPopFiTrangThai('tbphithamdinh-template', item);
                self.tbThamdinh({
                    fiMaHoso: item.fiMaHoso(),
                    fiIdHoso: item.fiIdHoso(),
                    fiTrangthai: LE_PHI_THAM_DINH
                }, function (d) {
                    ko.applyBindings(new TBPhiThamDinhVM(d.data), document.getElementById('tbphithamdinh-form'));
                });
                break;
            case  TT_XAC_NHAN_THANH_TOAN:
                app.Alert("Lệ phí thẩm định đang ở trạng thái: " + $(e.target).text());
//                self.createPopFiTrangThai('kqxl-template', item);
//                self.kqxlHoSo({
//                    fiMaHoso: item.fiMaHoso(),
//                    fiIdHoso: item.fiIdHoso(),
//                    fiTrangthai: XAC_NHAN_THANH_TOANS
//                }, function (d) {
...
//                });
    ...
    self.fiTrangThaiClick = function (item, e) {
        ...
        switch (item.fiTrangthai()) {
            ...
            case  YC_BO_SUNG:
                ...
                break;
            case  TU_CHOI_CAP_GIAY_PHEP:
                self.createPopFiTrangThai('kqxl-template', item);
                self.kqxlHoSo({
                    fiMaHoso: item.fiMaHoso(),
                    fiIdHoso: item.fiIdHoso(),
                    fiTrangthai: TU_CHOI_CAP_GIAY_PHEP
                }, function (d) {
                    ko.applyBindings(new KetQuaXLVM(d.data), document.getElementById('kqxl-form'));
                });
                break;
            ...
            case  XAC_NHAN_THANH_TOANS:
                ...
                break;
            case  CAP_GIAY_PHEP:
                document.location = app.appContext + '/most/05/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=cert&maThuTuc=' + self.maThuTuc();
                break;
            default:
    ...
    self.guiYcXuly = function (message) {
        $.ajax({
            ...
            cache: false,
            crossDomain: true,
            url: app.appContext + '/most/p05/hoso/xuly',
    ...
    /**
     * Lay thong bao le phi tham dinh
     * @param {type} message
     * @returns {undefined}
     */
    self.tbThamdinh = function (message, cb) {
        app.makePost({
            url: '/most/p05/hoso/tbthamdinh',
            data: JSON.stringify(message),
            success: function (d) {
                debugger;
                var msg = '';
    ...
    self.kqYeuCauTraLaiVB = function (message, cb) {
        app.makePost({
            url: '/most/p05/hoso/kqyctralaivb',
            data: JSON.stringify(message),
            success: function (d) {
                var msg = '';
                if (d.success) {
...
$(document).ready(function () {
    var d = {};
//    $('#loading10').show();
    $.when(app.getCategory('/most/p05/danhmuc', 'DM_TRANGTHAI', null, function (res) {
        if (res.success) {
