    ...
    function ViewModel() {
        ...
        self.searching = function (showMsg) {
            ...
            app.makePost({
                url: '/mt/23/call_service',
                data: JSON.stringify({
                	URL_BACKEND: "/mt/23/hoso/fullSearchPaging",
                    URL_GET_TOTAL:"/mt/23/hoso/getTotalSearch",
                    METHOD: "POST",
        ...
        self.xemGiayPhep = function (item) {
            location.href = app.appContext + '/mt/23/view/license/' + procedureId +'/'+ item.fiIdHoso()+'/1';
        }

        self.typeForm = ko.observable('search');

        self.suaHoSo = function (item) {
            location.href = app.appContext + '/mt/23/edit/' +  procedureId + "/" +item.fiIdHoso();
        }
        self.xemHoSo = function (item) {
            location.href = app.appContext + '/mt/23/view/' + procedureId + "/" + item.fiIdHoso();
        }
        self.themMoiHoSo = function (item) {
            location.href = app.appContext + '/mt/23/new/'+procedureId;
        }
                ...
                buttons: [
                    {
                        ...
                        action: function () {
                            app.makePost({
                                url: '/mt/23/call_service',
                                data: JSON.stringify({
                                    URL_BACKEND: "/mt/23/hoso/delete_all",
                                    METHOD: "POST",
        ...
        self.xemLichSu = function (item) {
            var url = '/mt/23/history/' + item.fiIdHoso()+'/1';
                    ...
                    if (d.success) {
                        ...
                        $('#' + 'lich-su-pagination').pagination({
                            ...
                            onPageClick: function (pageNumber, event) {
                                var url = '/mt/23/history/' + item.fiIdHoso() + '/' + pageNumber;
                                app.bindData({
                                    ministryCode: 'mt',
                                    code: 'p23',
