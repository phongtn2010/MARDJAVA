...
$(document).ready(function () {
    function ViewModel() {
        ...
        self.hoSoChiTiet.Tbdhoso.fiLoaihinh("VT_VC");
        app.makePost({
        ...
        if (hoSoId) {
            ...
            self.hoSoChiTiet.Tbdhoso.fiNhomthutuc("VT_VC");
        ...
        self.dong = function () {
            location.href = app.appContext + '/mt/19/home/' + self.hoSoChiTiet.Tbdhoso.fiLoaihoso();
        }
        ...
        self.action = function (item, functionType) {
            ...
            if (self.danhSachFileCheck().length > 0) {
                app.uploadMutipleFile({
                    url: '/mt/19/saveWithFile/' + functionType,
                ...
                app.makePost({
                    url: '/mt/19/call_service',
                    data: JSON.stringify({
                        URL_BACKEND: "/mt/19/hoso/save/" + functionType,
                        METHOD: "POST",
...
// -- file
        ...
        self.download = function (item) {
            if (item.fiIdDinhkem()) {
                $('#loading_c4e6a343-dd41-b5c5-1ef9-75eeebba032d').show();
                app.makeGet({
                    url: "/mt/19/download/" + item.fiIdDinhkem(), // my URL
                    responseType: 'blob',
