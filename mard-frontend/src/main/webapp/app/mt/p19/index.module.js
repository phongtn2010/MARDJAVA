    ...
    function ViewModel() {
        ...
        self.searching = function (showMsg) {
            ...
            app.makePost({
                url: '/mt/19/call_service',
                data: JSON.stringify({
                    URL_BACKEND: "/mt/19/hoso/searchPaging",
                    URL_GET_TOTAL:"/mt/19/hoso/getTotalRecord",
                    METHOD: "POST",
        ...
        self.xemGiayPhep = function (item) {
            location.href = app.appContext + '/mt/19/view/license/' + procedureId +'/'+ item.fiIdGiayphep()+'/1';
        }

        self.typeForm = ko.observable('search');

        self.suaHoSo = function (item) {
            location.href = app.appContext + '/mt/19/edit/' + procedureId + "/" + item.fiIdHoso();
        }
        self.xemHoSo = function (item) {
            location.href = app.appContext + '/mt/19/view/' + procedureId + "/" + item.fiIdHoso();
        }
        self.themMoiHoSo = function (item) {
            location.href = app.appContext + '/mt/19/new/'+procedureId;
        }
                ...
                buttons: [
                    {
                        ...
                        action: function () {
                            app.makePost({
                                url: '/mt/19/call_service',
                                data: JSON.stringify({
                                    URL_BACKEND: "/mt/19/hoso/delete_all",
                                    METHOD: "POST",
        ...
        self.xemLichSu = function (item) {
            var url = '/mt/19/history/' + item.fiIdHoso()+'/1';
                    ...
                    if (d.success) {
                        ...
                        $('#' + 'lich-su-pagination').pagination({
                            ...
                            onPageClick: function (pageNumber, event) {
                                var url = '/mt/19/history/' + item.fiIdHoso() + '/' + pageNumber;
                                app.bindData({
                                    ministryCode: 'mt',
                                    code: 'p19',
