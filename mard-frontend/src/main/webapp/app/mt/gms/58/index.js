...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/mt/58/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdlichsu58(res.data, data.currentPage * data.pageSize) : [];
                    self.historyItems([]);
...
function IndexVM(params) {
    ...
    self.search = function (page, pushState) {
        if (!RAW_HS_STATUS || RAW_HS_STATUS.length <= 0) {
            app.getCategory('/mt/58/danhmuc', 'HS_TRANGTHAI', null, function (res) {
                if (res.success) {
    ...
    self.searchAfterHasCatgory = function (page, pushState) {
        ...
        app.makePost({
            url: "/mt/58/hoso/timkiem",
            data: JSON.stringify(data),
            success: function (res) {
                window.stateChanging = false;
                if (res.success) {
                    self.Items([]);
                    var list = res.data ? mapTbdhoso58(res.data, data.currentPage * data.pageSize) : [];
                    self.Items(list);
                    self.paging().update({
                        totalCount: res.data ? res.total : 0,
                        pageSize: self.pageSize(),
    ...
    self.btnAddNewClick = function (e) {
        document.location = app.appContext + '/mt/' + self.fiThuTucId() + '/edit?maThuTuc=' + self.maThuTuc();
        return false;
    };
    ...
    self.bXemClick = function (item, e) {
        document.location = app.appContext + '/mt/' + self.fiThuTucId() + '/view?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        return false;
    };
    /**
     * Xem Thông báo
    ...
    self.bXemThongBaoClick = function (item, e) {
        document.location = app.appContext + '/mt/' + self.fiThuTucId() + '/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=cert&maThuTuc=' + self.maThuTuc();
        return false;
    };
    /**
     * Xem Lich su
    ...
    self.bSuaClick = function (item, e) {
        document.location = app.appContext + '/mt/' + self.fiThuTucId() + '/edit?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        return false;
    };
    /**
     * Hủy hồ sơ (xóa)
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/mt/58/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
    ...
    self.guiYcXuly = function (message) {
        $.ajax({
            ...
            cache: false,
            crossDomain: true,
            url: app.appContext + '/mt/' + self.fiThuTucId() + '/hoso/xuly',
    ...
    self.kqxlHoSo = function (message, cb) {
        app.makePost({
            url: '/mt/' + self.fiThuTucId() + '/hoso/kqxl',
            data: JSON.stringify(message),
            success: function (d) {
                var msg = '';
                if (d.success) {
    ...
    self.kqYeuCauSuaVB = function (message, cb) {
        app.makePost({
            url: '/mt/' + self.fiThuTucId() + '/hoso/kqycsuavb',
            data: JSON.stringify(message),
            success: function (d) {
                var msg = '';
                if (d.success) {
    ...
    self.kqYeuCauTraLaiVB = function (message, cb) {
        app.makePost({
            url: '/mt/' + self.fiThuTucId() + '/hoso/kqyctralaivb',
            data: JSON.stringify(message),
            success: function (d) {
                var msg = '';
                if (d.success) {
...
$(document).ready(function () {
    var d = {};
    $('#loading10').show();
    $.when(app.getCategory('/mt/58/danhmuc', 'HS_TRANGTHAI', null, function (res) {
        if (res.success) {
