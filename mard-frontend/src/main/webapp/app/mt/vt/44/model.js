...
var mapTbdlichsu44 = function (dataFromServer, p) {
    return ko.utils.arrayMap(dataFromServer, function (item, index) {
        return new Tbdlichsu44(index + 1 + p, item);
    });
};
var Tbdlichsu44 = function (STT, item) {
    var self = this;
...
var mapTbdhoso44 = function (d, p) {
    return ko.utils.arrayMap(d, function (item, index) {
        return new Tbdhoso44((index + p) + 1, item);
    });
};
function Tbdhoso44(STT, item) {
    var self = this;
...
function TbdhsXe44(data) {
    ...
    self.fiBksXe = ko.observable(data ? data.fiBksXe : null).extend({
        ...
        maxLength: {message: 'Tối đa 4455 ký tự', params: 4455}
    });
...
var mapTbdhsXe44 = function (d) {
    return ko.utils.arrayMap(d, function (item, index) {
        item.fiStt = index + 1;
        return new TbdhsXe44(item);
    });
};