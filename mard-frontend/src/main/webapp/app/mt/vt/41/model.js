...
var mapTbdlichsu41 = function (dataFromServer,p) {
    return ko.utils.arrayMap(dataFromServer, function (item, index) {
        return new Tbdlichsu41(index + 1+p, item);
    });
};
var Tbdlichsu41 = function (STT, item) {
    var self = this;
...
var mapTbdhoso41 = function (d, p) {
    return ko.utils.arrayMap(d, function (item, index) {
        return new Tbdhoso41((index + 1) + p, item);
    });
};
function Tbdhoso41(STT, item) {
    var self = this;
    ...
    self.bXemThongBao = ko.dependentObservable(function () {
        //return true;
         return [DUOC_CAP_VANBAN,TU_CHOI_YC_SUA_VB, DA_CAP_LAI_VANBAN, DONG_Y_YC_SUA_VB, YC_SUA_VANBAN, TU_CHOI_CAP_VANBAN, DA_THU_HOI, YC_TRA_LAI_KQ].indexOf(self.fiTrangthai()) >= 0;
    }, self);

    self.fiNgayguiText = ko.observable(item.fiNgaygui ? new Date(item.fiNgaygui).toDayFirstWithTime() : null);
}

function TbdhsXe41(data) {
    var self = this;
    ...
    self.fiSoGpMat = ko.observable(data ? data.fiSoGpMat : null);
...
var mapTbdhsXe41 = function (d) {
    return ko.utils.arrayMap(d, function (item, index) {
        item.fiStt = index + 1;
        return new TbdhsXe41(item);
    });
};

var mapTbdgiayphep41 = function (d) {
    return ko.utils.arrayMap(d, function (item, index) {
        return new Tbdgiayphep41(index + 1, item);
    });
};
var Tbdgiayphep41 = function (STT, item) {
    var self = this;
    this.STT = ko.observable(STT);
    if (item) {
        ko.mapping.fromJS(item, {}, self);
...
var mapTbdgpLichsu41 = function (dataFromServer) {
    return ko.utils.arrayMap(dataFromServer, function (item, index) {
        return new Tbdgplichsu41(index + 1, item);
    });
};
var Tbdgplichsu41 = function (STT, item) {
    var self = this;
    this.STT = ko.observable(STT);
    if (item != null) {
        ko.mapping.fromJS(item, {}, self);
