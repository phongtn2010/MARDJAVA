...
//            })
        ...
        self.searching = function (showMsg) {
            ...
            app.makePost({
                url: '/mt/48/call_service',
                data: JSON.stringify({
                    URL_BACKEND: "/mt/48/hoso/fullSearchPaging",
                    URL_GET_TOTAL:"/mt/48/hoso/getTotalSearch",
                    METHOD: "POST",
        ...
        self.xemGiayPhep = function (item) {
            location.href = app.appContext + '/mt/48/view/license/' + procedureId +'/'+ item.fiIdHoSo()+'/1';
        }

        self.typeForm = ko.observable('search');

        self.suaHoSo = function (item) {
            location.href = app.appContext + '/mt/48/edit/' + procedureId + "/" + item.fiIdHoSo();
        }
        self.xemHoSo = function (item) {
            location.href = app.appContext + '/mt/48/view/' + procedureId + "/" + item.fiIdHoSo();
        }
        self.themMoiHoSo = function (item) {
            location.href = app.appContext + '/mt/48/new/'+procedureId;
        }
                ...
                buttons: [
                    {
                        ...
                        action: function () {
                            app.makePost({
                                url: '/mt/48/call_service',
                                data: JSON.stringify({
                                    URL_BACKEND: "/mt/48/hoso/delete_all",
                                    METHOD: "POST",
        ...
        self.xemLichSu = function (item) {
            var url = '/mt/48/history/' + item.fiIdHoSo()+'/1';
                    ...
                    if (d.success) {
                        ...
                        $('#' + 'lich-su-pagination').pagination({
                            ...
                            onPageClick: function (pageNumber, event) {
                                var url = '/mt/48/history/' + item.fiIdHoSo() + '/' + pageNumber;
                                app.bindData({
                                    ministryCode: 'mt',
                                    code: 'p48',
