    ...
    function ViewModel() {
        ...
        self.searching = function (showMsg) {
            ...
            app.makePost({
                url: '/mt/01/call_service',
                data: JSON.stringify({
                    URL_BACKEND: "/mt/01/hoso/searchPaging",
                    URL_GET_TOTAL: "/mt/01/hoso/getTotalRecord",
                    METHOD: "POST",
        ...
        self.xemGiayPhep = function (item) {
            location.href = app.appContext + '/mt/01/view/license/' + procedureId + '/' + item.fiIdGiayphep()+'/1';
        }

        self.typeForm = ko.observable('search');

        self.suaHoSo = function (item) {
            location.href = app.appContext + '/mt/01/edit/' + procedureId + "/" + item.fiIdHoso();
        }
        self.xemHoSo = function (item) {
            location.href = app.appContext + '/mt/01/view/' + procedureId + "/" + item.fiIdHoso();
        }
        self.themMoiHoSo = function (item) {
            location.href = app.appContext + '/mt/01/new/' + procedureId;
        }
                ...
                buttons: [
                    {
                        ...
                        action: function () {
                            app.makePost({
                                url: '/mt/01/call_service',
                                data: JSON.stringify({
                                    URL_BACKEND: "/mt/01/hoso/delete_all",
                                    METHOD: "POST",
        ...
        self.xemLichSu = function (item) {
            var url = '/mt/01/history/' + item.fiIdHoso() + '/1';
                    ...
                    if (d.success) {
                        ...
                        $('#' + 'lich-su-pagination').pagination({
                            ...
                            onPageClick: function (pageNumber, event) {
                                var url = '/mt/01/history/' + item.fiIdHoso() + '/' + pageNumber;
                                app.bindData({
                                    ministryCode: 'mt',
                                    code: 'p01',
