...
$(document).ready(function () {
    function ViewModel() {
        ...
        if (hoSoId) {
            ...
            self.hoSoChiTiet.Tbdhoso.fiNhomthutuc('LV_PTM_VC');
        ...
        self.dong = function () {
            location.href = app.appContext + '/mt/27/home/' + self.hoSoChiTiet.Tbdhoso.fiLoaihoso();
        }
        ...
        self.action = function (item, functionType) {
            ...
            if (self.danhSachFileCheck().length > 0) {
                app.uploadMutipleFile({
                    url: '/mt/27/saveWithFile/' + functionType,
                ...
                app.makePost({
                    url: '/mt/27/call_service',
                    data: JSON.stringify({
                        URL_BACKEND: "/mt/27/hoso/save/" + functionType,
                        METHOD: "POST",
...
//=====them xe====
        ...
        self.download = function (item) {
            if (item.fiIdDinhkem()) {
                $('#loading_c4e6a343-dd41-b5c5-1ef9-75eeebba032d').show();
                app.makeGet({
                    url: "/mt/27/download/" + item.fiIdDinhkem(), // my URL
                    responseType: 'blob',
