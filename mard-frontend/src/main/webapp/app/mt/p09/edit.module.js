...
$(document).ready(function () {
    function ViewModel() {
        ...
        function getLoaihinh(procedureId) {
            if (procedureId === 'BGTVT0600009') {
                return 1;
            }
            if (procedureId === 'BGTVT0600010') {
                return 2;
            }
            if (procedureId === 'BGTVT0600011') {
                return 3;
            }
            if (procedureId === 'BGTVT0600012') {
                return 4;
            }
        };

        self.dong = function () {
            location.href = app.appContext + '/mt/09/home/' + self.hoSoChiTiet.Tbdhoso.fiLoaihoso();
        }
        ...
        self.action = function (item, functionType) {
            ...
            if (self.danhSachFileCheck().length > 0) {
                app.uploadMutipleFile({
                    url: '/mt/09/saveWithFile/' + functionType,
                ...
                app.makePost({
                    url: '/mt/09/call_service',
                    data: JSON.stringify({
                        URL_BACKEND: "/mt/09/hoso/save/" + functionType,
                        METHOD: "POST",
...
// -- file
        ...
        self.download = function (item) {
            if (item.fiIdDinhkem()) {
                $('#loading_c4e6a343-dd41-b5c5-1ef9-75eeebba032d').show();
                app.makeGet({
                    url: "/mt/09/download/" + item.fiIdDinhkem(), // my URL
                    responseType: 'blob',
