...
function FormVM(options) {
    ...
    self.fiNamdkNk = ko.observable(null).extend({
        required: {message: NSWLang["common_msg_formvaild_required"], params: true},
        maxLength: {message: 'Tối đa 4 ký tự', params: 9999},
        pattern: {
            message: 'Bạn phải nhập đúng định dạng YYYY(năm)',
            params: /^[0-9]{4}?$/
        }
    });
    ...
    self.isValidForm = function () {
        ...
        if (!self.lstNguyenlieus() || self.lstNguyenlieus().length <= 0) {
            $("#lstNguyenlieus-lbl").show();
            $("#lstNguyenlieus-lbl-1").hide();
            errorHH = false;
            return errorHH;
        } else {
            $("#lstNguyenlieus-lbl").hide();
        }
        if (self.lstNguyenlieus().length <= 0) {
            $('#tbdHH').show();
            flag = false;
        }
        ...
        if (self.lstNguyenlieus() && self.lstNguyenlieus().length > 0) {
            for (var i = 0; i < self.lstNguyenlieus().length; i++) {
                ...
                if(nguyenLieu.hhErrors().length>0){
                    nguyenLieu.hhErrors.showAllMessages();
                    errorHH = false;
                }
...
//                    $("#lstNguyenlieus-lbl-1").text('Hàng ' + (i + 1) + ' Tên nguyên liệu và loại nguyên liệu không tương thích');
//                    $("#lstNguyenlieus-lbl").hide();
//                    $("#lstNguyenlieus-lbl-1").show();
                    errorHH = false;
                    return errorHH;
                }
            }
    ...
    self.valueLoai = function () {
        for (var i = 0; i < self.lstNguyenlieus().length; i++) {
            var nguyenLieu = self.lstNguyenlieus()[i];
            if ( self.lstNguyenlieus()[i].fiLoaiNguyenlieu()  === undefined || self.lstNguyenlieus()[i].fiLoaiNguyenlieu()  === "")
                $('#fiLoaiNguyenlieulb_' + (i + 1)).show();
            else
                $('#fiLoaiNguyenlieulb_' + (i + 1)).hide();

            if ( self.lstNguyenlieus()[i].fiMaNguyenlieu()  === undefined || self.lstNguyenlieus()[i].fiMaNguyenlieu()  === "")
                $('#fiMaNguyenlieulb_' + (i + 1)).show();
            else
                $('#fiMaNguyenlieulb_' + (i + 1)).hide();
            if (self.lstNguyenlieus()[i].fiDangkyNhapkhauNamx1()  === undefined || self.lstNguyenlieus()[i].fiDangkyNhapkhauNamx1()  === "")
                $('#fiDangkyNhapkhauNamx1lb_' + (i + 1)).show();
            else
                $('#fiDangkyNhapkhauNamx1lb_' + (i + 1)).hide();
            if ( self.lstNguyenlieus()[i].fiSoluongCapX1() === undefined || self.lstNguyenlieus()[i].fiSoluongCapX1()  === "")
                $('#fiSoluongCapX1lb_' + (i + 1)).show();
            else
                $('#fiSoluongCapX1lb_' + (i + 1)).hide();
            
            if ( self.lstNguyenlieus()[i].fiUocThuchienX1() === undefined || self.lstNguyenlieus()[i].fiUocThuchienX1()  === "")
                $('#fiUocThuchienX1lb_' + (i + 1)).show();
            else
                $('#fiUocThuchienX1lb_' + (i + 1)).hide();
            
            if ( self.lstNguyenlieus()[i].fiDangkyNhapkhauNamx()  === undefined || self.lstNguyenlieus()[i].fiDangkyNhapkhauNamx()  === "")
                $('#fiDangkyNhapkhauNamxlb_' + (i + 1)).show();
            else
                $('#fiDangkyNhapkhauNamxlb_' + (i + 1)).hide();
            if(nguyenLieu.fiLoaiNguyenlieu() != undefined && nguyenLieu.fiMaNguyenlieu()!= undefined){
                if ((nguyenLieu.fiLoaiNguyenlieu()).charAt(0) != (nguyenLieu.fiMaNguyenlieu()).charAt(0)) {
                        $("#lstNguyenlieus-lbl-1").text('Hàng ' + (i + 1) + ' Tên nguyên liệu và loại nguyên liệu không tương thích');
                        $("#lstNguyenlieus-lbl").hide();
                        $("#lstNguyenlieus-lbl-1").show();
                    }else $("#lstNguyenlieus-lbl-1").hide();
            }
        }
    }
