...
function EditVM(options) {
    ...
    self.btnLuuClick = function () {
        ...
        var url = !fiIdHoso || fiIdHoso <= 0 ? '/moit/01/hoso/taomoi' : '/moit/01/hoso/capnhap';
...
$(document).ready(function () {
    ...
    $('#loading10').show();
    $.when(
            app.getCategory('/moit/01/danhmuc', 'DM_MAHS', null, function (res) {
                if (res.success) {
                    options.lstMaHS = res.data;
                } else {
                    options.lstMaHS = [];
                }
            }),
            app.getCategory('/moit/01/danhmuc', 'DM_LOAIFILE', null, function (res) {
                if (res.success) {
    ...
    var init = function () {
        if (options && options.fiIdHoso > 0) {
            var url = '/moit/01/hoso/t/' + options.fiIdHoso;
            app.makePost({
                url: url,
                data: JSON.stringify({}),
                success: function (d) {
                    if (d.success) {
                        ...
                        for (var i = 0; i < d.data.lstHanghoas.length; i++) {
                            var objHH = d.data.lstHanghoas[i];
                            $("#fiMaHs-" + objHH.fiStt).select2({placeholder: '-- Chọn --', width: '100%', allowClear: true});
                        }
                    }
                },
                error: function (e) {
