function GiayXacNhanView() {
	...
	self.show = function(item) {
		app.makePost({
            url: '/moit/api/07/layThongTinGP/' + item.idHoSo(),
            data: JSON.stringify({}),
            success: function (d) {
                var fun = 'success';
                if (d.success) {
                	...
                	if(d.data.ngayCapGiayChungNhan != null){
                		date = d.data.ngayCapGiayChungNhan.split('/');
                		d.data.ngayCapGiayChungNhan_Ngay = date[0];
                		d.data.ngayCapGiayChungNhan_Thang = date[1];
                		d.data.ngayCapGiayChungNhan_Nam = date[2];
                	}
                	if(d.data.hangHoas != null){
                		for(i=0; i< d.data.hangHoas.length; i++){
                			var trangThai = d.data.hangHoas[i].trangThaiChat;
                			if(trangThai == 1){
                				var LoaiHangHoa = "Rắn";
	                			d.data.hangHoas[i].trangThaiChat = LoaiHangHoa;
	                		}
	                		if(trangThai == 2){
