...
$(document).ready(function() {
    ...
    function GhiHoSoViewModel(v1, v2, v3, v4, v5) {
    	...
    	self.getHoSo = function(idHoSo) {
    		app.makePost({
				url :'/moit/api/07/suaHoSo/' + self.idHoSo,
				data : JSON.stringify({}),
				success : function(d) {
					if (d.success) {
						...
						if ($.isEmptyObject(container1VM)) {
						 ...
						 if ($.isEmptyObject(container3VM)) {
						        container3VM = new ChiTietHangHoaViewModel(d.data, container1VM);
					    ...
					    if ($.isEmptyObject(container4VM)) {
					        container4VM = new ThongTinTepDinhKemViewModel(d.data, v1.thongTinChungModel);
    	...
    	self.guiHoSo = function(){
            ...
            var pop = app.popup({
                title: NSWLang["common_msg_thong_bao"],
                html: '<i class="fa fa-3x fa-warning"></i> ' + NSWLang["moit_06_popup_guihs_confirm"] +'</b>',
                width: 400,
                buttons: [
                    {
    	...
    	self.save = function(isGuiHoSo, success) {
    		...
    		data.hinhThucXNK = v1.thongTinChungModel.hinhThucXNK();
    		...
    		data.soGiayPhepDaCap = v1.thongTinChungModel.soGiayPhepDaCap();
    		data.lyDoGiaHan = v1.thongTinChungModel.lyDoGiaHan();
    		data.loaiGiayPhep = v1.thongTinChungModel.loaiGiayPhep();
    		data.dienThoaiNoiSanXuat = v1.thongTinChungModel.dienThoaiNoiSanXuat();
    		data.dienThoaiNguoiLienHe = v1.thongTinChungModel.dienThoaiNguoiLienHe();
    		data.faxNoiSanXuat = v1.thongTinChungModel.faxNoiSanXuat();
    		data.tenNguoiDaiDien = v1.thongTinChungModel.tenNguoiDaiDien();
    		data.tenNguoiLienHe = v1.thongTinChungModel.tenNguoiLienHe();
    		data.emailNguoiLienHe = v1.thongTinChungModel.emailNguoiLienHe();
    		data.noiCapGDKKD = v1.thongTinChungModel.noiCapGDKKD();
    		data.maTinhTP = v1.thongTinChungModel.maTinhTP();
    		data.cuaKhaus = v2.getCuaKhauToJson();
    		data.phuongTiens = v2.getPhuongTienToJson();
    		data.haiQuans = v2.getHaiQuanToJson();
    		data.mucDich = v2.thongTinHangHoaModel.mucDich();
    		data.xuatNhapKhauTuNgay = v2.thongTinHangHoaModel.xuatNhapKhauTuNgay();
    		data.xuatNhapKhauDenNgay = v2.thongTinHangHoaModel.xuatNhapKhauDenNgay();
    		data.soLanThucHien = v2.thongTinHangHoaModel.soLanThucHien();
    		data.hangHoa7DTOs = v3.getHangHoas();
					...
					if (d.success) {
						 if (!isGuiHoSo) {
							 ...
							 if (d.data.isChinhSua) {
								 location.href = app.appContext + "/moit/07/edit/" + idHoSoGoc + "/ycs";
							 } else {
								 if (idHoSoGoc == 0) {
									 idHoSoGoc = d.data.idHoSo;
								 }
								 location.href = app.appContext + "/moit/07/edit/" + idHoSoGoc;
							 }
							 
						 } else {
							 if (requiredSign) {
								 var sign = CAPlugin.signXml(d.data.xmlContent, d.data.serialNumber);
								 d.data.xmlContent = sign;
							}
							 success(d);
						 }
    	...
    	self.send = function(d) {
    		 var href = app.appContext + "/moit/07/home/";
    		app.makePost({
                url: '/moit/api/07/guiHS',
