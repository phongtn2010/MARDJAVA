function ThongTinTepDinhKemViewModel(hoSo, thongTinChungModel) {
	...
	self.selectedTep = ko.observable(thongTinChungModel.loaiGiayPhep() == 2 ? 'NHAP_KHAU' : 'XUAT_KHAU');
        ...
        ds.forEach(function(t, tIndex){
            $(hoSo.tepDinhKems).each(function(index, loopItem){
                if (t.maLoaiTep == loopItem.maLoaiTep) {
                    ...
                    tepTin.link(app.appContext +  '/moit/api/06/download/' + loopItem.idTepTin);
    ...
    thongTinChungModel.loaiGiayPhep.subscribe(function () {
        ...
        if (thongTinChungModel.loaiGiayPhep() == 2) {
            self.selectedTep('NHAP_KHAU');
            self.groupFileIndexSelected(1);
        } else {
            self.selectedTep('XUAT_KHAU');
        ...
        app.makePost({
            url:'/moit/api/06/xoaTepDKByIdHoSo/' + idHoSo,
            data: JSON.stringify({}),
            success: function (d) {
            },
            error: function (e) {
	...
	self.uploadFileChangeEvent = function(loaiTep, position, elemet, event) {
	   	...
	   	$(files).each(function(index, item){
	   	 ...
	   	 if (self.totalFileSize() > totalMB) {
	   		cssError('fileUpload' + position);
	   	  	...
	   	  	$('#fileUpload' + position).val('');
	   		 app.Alert(NSWLang["moit_06_totalSizeUploadError"] + ' 3MB!');
	   		 return false;
	   	 }
		 var findId = idHoSo;
        var groupIndex = 0;
        if (thongTinChungModel.loaiGiayPhep() == 2) {
			...
			for (var k = 0; k < files.length; k++) {
				...
				param.file = files[k];
		        app.uploadFile({
		          url: '/moit/api/06/upload/' + findId + '/' + loaiTep,
		          data: param,
		          success: function (d) {
		            if (d.success) {
		            	 var tepTin = new TepTinModel();
			                ...
			                if (idHoSo > 0) {
			                	  tepTin.link(app.appContext +  '/moit/api/06/download/' + d.data.idTepTin);
			                } else {
			                	tepTin.link(app.appContext +  '/moit/api/06/getfile/moit/06/' + d.data.guID + '.' + d.data.loaiTep);
			                }
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        ...
                        $(arr).each(function(index, loopItem){
                            if (idHoSo > 0) {
                                var url = '/moit/api/06/xoaTepDK/' + loopItem.idTepTin();
                                app.makePost({
                                    url: url,
                                    data: JSON.stringify({}),
                                    success: function (d) {
