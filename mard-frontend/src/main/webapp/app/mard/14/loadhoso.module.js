function LoadHoSoView(self) {
	...
	self.layDanhSachTrangThai = function() {
		...
		app.makePost({
			url : '/mard/14/tbsTrangThai14/findAll',
			data : ko.toJSON({
				"number" : 0,
				"size" : 100,
				"sort" : "fiId",
			...
			error : function(e) {
				self.danhSachTrangThai.push({fiCode: '-1', fiStatusName: NSWLang["mard.14.select.option"]});
	...
	self.load = function(page) {
		...
		app.makePost({
			url : encode('/mard/14/tbdHoSo14/page', self.searchForm),
			data : ko.toJSON({
				"number" : page - 1,
