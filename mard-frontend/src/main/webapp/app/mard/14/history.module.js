function HistoryView() {
    ...
    self.show = function(item) {
        var callback = function (html) {
            var pop = app.popup({
                title: '<b style="text-transform: uppercase">' + NSWLang["mard.14.popup.lichSuHoSo"] + ' - ' + item.fiDocumentName() + '</b>',
                html: html,
                width: 960,
                buttons: [
                ]
...
function HistoryPopupView(item) {
    ...
    var url =  '/mard/14/tbdKetQuaXuLy14/page?fiIdHoSo='+item.fiIdHoSo();
