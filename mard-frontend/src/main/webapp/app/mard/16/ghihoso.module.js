$(document).ready(function () {
    ...
    function GhiHoSoViewModel() {
        ...
        self.moduleTbdToKhaiKyThuat16ViewModel = null;
        ...
        self.getHoSo = function (idHoSo) {
            app.makePost({
                url: '/mard/api/16/edit/' + self.idHoSo,
                data: JSON.stringify({}),
                error: function (e) {
                    ...
                    self.moduleTbdToKhaiKyThuat16ViewModel = new TbdToKhaiKyThuat16ViewModel(self);
                    ko.applyBindings(self.moduleTbdToKhaiKyThuat16ViewModel, document.getElementById("TbdToKhaiKyThuat16ViewModel"));
        ...
        self.save = function (isGuiHoSo) {
            ...
            postData.toKhaiKyThuats = self.moduleTbdToKhaiKyThuat16ViewModel.convertData();
            postData.action = action;

            app.makePost({
                url: '/mard/api/16/save/' + self.idHoSo,
                data: ko.toJSON(postData),
                error: function (e) {
                    if (e.readyState == 4) {
                        if (idHoSo == 0) {
                            showToast(NSWLang["mard.16.msg.taoMoiHoSoNo"], false);
                        } else {
                            showToast(NSWLang["mard.16.msg.capNhatHoSoNo"], false);
                        }
                        return;
                    } else {
                        if (idHoSo == 0) {
                            showToast(NSWLang["mard.16.msg.taoMoiHoSoOk"], true);

                        } else {
                            showToast(NSWLang["mard.16.msg.capNhatHoSoOk"], true);
                        }
                    }
                    if (isGuiHoSo) {
                        if (idHoSo === 0) {
                        ...
                        var item = new TbdHoSo16();
                        item.fiIdHoSo(e.hoSo.fiIdHoSo);
                        item.fiDocumentName(e.hoSo.fiDocumentName);
                        sendHoSoView.show(item);
                    } else {
                       if (action == 2) {
                           location.href = app.appContext + '/mard/16/ycs/' + e.hoSo.fiIdHoSo;
                       } else {
                           location.href = app.appContext + '/mard/16/edit/' + e.hoSo.fiIdHoSo;
                       }
                    }
                }
            });
