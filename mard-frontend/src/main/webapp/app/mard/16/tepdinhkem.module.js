function ThongTinTepDinhKemViewModel(parent) {
		 ...
		 parent.formData.danhMucTepDinhKems.forEach(function(t, tIndex){
     		 $(parent.formData.tepDinhKems).each(function(index, loopItem){
     			 if (t.fiFileTypeCode == loopItem.fiFileTypeCode) {
                     ...
                     tepTin.link(app.appContext +  '/mard/api/16/download/' + loopItem.fiId);
	...
	self.uploadFileChangeEvent = function(loaiTep, position, elemet, event) {
		...
		for (var i = 0, file; file = files[i]; i++) {
			...
			if ($.inArray(ext, self.accepFiles) === -1) {
				...
				app.Alert( NSWLang["mard.16.uploadInfo"]);
				return false;
			}
		}
		var totalMB = 1024 * 1024 * 3;// 3MB
	   	...
	   	$(files).each(function(index, item){
	   	 ...
	   	 if (self.totalFileSize() > totalMB) {
	   	  	...
	   	  	$('#fileUpload' + position).val('');
	   	  	app.Alert( NSWLang["mard.16.uploadSize"]);
	   		 return false;
	   	 }
		 var findId = idHoSo;
			for (var k = 0; k < files.length; k++) {
				...
				param.file = files[k];
		        app.uploadFile({
		          url: '/mard/api/16/upload/' + findId + '/' + loaiTep,
		          data: param,
		          success: function (d) {
		            if (d.success) {
		            	 var tepTin = convertObjectToKnockout(d.data, new TepTinModel());
			                if (idHoSo > 0) {
			                	  tepTin.link(app.appContext +  '/mard/api/16/download/' + d.data.fiId);
			                } else {
                                tepTin.link(app.appContext +  '/mard/api/15/getfile/' + param.mcode + '/' + param.pcode + '/' + d.data.fiFileCode);
			                }
		...
		self.callConfirm(function () {
            ...
            if (idHS > 0) {
                ...
                $(self.teps()[position]()).each(function(index, loopItem){
                    $.getJSON(app.appContext + '/mard/16/tbdDinhKem16/delete/' + loopItem.fiId(), function (d) {
	...
	self.xoaMotFile = function(item, position) {
		self.callConfirm(function () {
            var idHS = idHoSo;
            if (idHS > 0) {
                $.getJSON(app.appContext + '/mard/16/tbdDinhKem16/delete/' + item.fiId(), function (d) {
			...
			var check = true;
   			for (var i = 0; i < parent.formData.danhMucTepDinhKems.length; i++) {
   			 var item = parent.formData.danhMucTepDinhKems[i];
   				if (item.fiFileGroup == self.selectedTep() && item.fiRequired) {
   			...
   			if (check == false) {
                app.Alert( NSWLang["mard.16.tepDinhKem.notValid"]);
   				return false;
			}
   		 }
		return true;
