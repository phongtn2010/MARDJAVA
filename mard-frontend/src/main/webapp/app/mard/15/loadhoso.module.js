function LoadHoSoView(self) {
	...
	self.layDanhSachTrangThai = function() {
		...
		app.makePost({
			url : '/mard/15/tbsTrangThai15/findAll',
			data : ko.toJSON({
				"number" : 0,
				"size" : 100,
				"sort" : "fiId",
			...
			error : function(e) {
				self.danhSachTrangThai.push({fiCode: '-1', fiStatusName: NSWLang["mard.15.select.option"]});
	...
	self.load = function(page) {
		...
		app.makePost({
			url : encode('/mard/15/tbdHoSo15/page', self.searchForm),
			data : ko.toJSON({
				"number" : page - 1,
	...
	self.checkShowHideButton = function(item) {
		...
		item.showButtonXinSuaGiayPhep(self.showButtonXinSuaGiayPhep(item.fiStatus()));
	}
