function ThongTinTepDinhKemViewModel(parent) {
		 ...
		 parent.formData.danhMucTepDinhKems.forEach(function(t, tIndex){
     		 $(parent.formData.tepDinhKems).each(function(index, loopItem){
     			 if (t.fiFileTypeCode == loopItem.fiFileTypeCode) {
                     ...
                     tepTin.link(app.appContext +  '/mard/api/15/download/' + loopItem.fiId);
	...
	self.uploadFileChangeEvent = function(loaiTep, position, elemet, event) {
		...
		for (var i = 0, file; file = files[i]; i++) {
			...
			if ($.inArray(ext, self.accepFiles) === -1) {
				...
				app.Alert( NSWLang["mard.15.uploadInfo"]);
				return false;
			}
		}
		var totalMB = 1024 * 1024 * 3;// 3MB
	   	...
	   	$(files).each(function(index, item){
	   	 ...
	   	 if (self.totalFileSize() > totalMB) {
	   	  	...
	   	  	$('#fileUpload' + position).val('');
	   	  	app.Alert( NSWLang["mard.15.uploadSize"]);
	   		 return false;
	   	 }
		 var findId = idHoSo;
			for (var k = 0; k < files.length; k++) {
				...
				param.file = files[k];
		        app.uploadFile({
		          url: '/mard/api/15/upload/' + findId + '/' + loaiTep,
		          data: param,
		          success: function (d) {
		            if (d.success) {
		            	 var tepTin = convertObjectToKnockout(d.data, new TepTinModel());
			                if (idHoSo > 0) {
			                	  tepTin.link(app.appContext +  '/mard/api/15/download/' + d.data.fiId);
			                } else {
		...
		self.callConfirm(function () {
            ...
            if (idHS > 0) {
                $(self.teps()[position]()).each(function(index, loopItem){
                    $.getJSON(app.appContext + '/mard/15/tbdDinhKem15/delete/' + loopItem.fiId(), function (d) {
	...
	self.xoaMotFile = function(item, position) {
        self.callConfirm(function () {
            var idHS = idHoSo;
            if (idHS > 0) {
                $.getJSON(app.appContext + '/mard/15/tbdDinhKem15/delete/' + item.fiId(), function (d) {
	...
	self.isValid = function(isGuiHoSo) {
		if (isGuiHoSo) {
			var check = true;
   			...
   			if (check == false) {
                app.Alert( NSWLang["mard.15.tepDinhKem.notValid"]);
   				return false;
			}
   		 }
		return true;
