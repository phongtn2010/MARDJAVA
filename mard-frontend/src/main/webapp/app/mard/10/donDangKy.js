...
function Mard10FormVM(options)
{
        ...
        getCategory("CUAKHAU", data.fiMaQgxk, function (res) {
            ...
            if (data.lstHanghoa10) {
                ...
                for (var i = 0; i < data.lstHanghoa10.length; i++) {
                    ...
                    --fiIdHh;
                    data.lstHanghoa10[i].fiIdHh = fiIdHh;
                }
    ...
    self.updateHanghoaClick = function () {
        ...
        if (self.fiSoluong()) {
            if (self.fiSoluong().toString().indexOf('.') >= 0 || self.fiSoluong().toString().indexOf(',') >= 0) {
                app.Alert('Trường số lượng không được phép nhập số thập phân!');
                return;
            }
        }
        
        if (self.fiTlTinh()) {
            if (self.fiTlTinh().toString().indexOf(',') >= 0) {
                app.Alert('Trọng lượng tịnh chỉ được nhập dấu . ngăn cách giữ phần nguyên và phần thập phân');
                return;
            }
        }
        
        if (self.fiTlBi()) {
            if (self.fiTlBi().toString().indexOf(',') >= 0) {
                app.Alert('Trọng lượng cả bì chỉ được nhập dấu . ngăn cách giữ phần nguyên và phần thập phân');
                return;
            }
        }
    ...
    self.fiMaQgxkChange = function () {
        if (self.fiMaQgxk()) {
            if (!self.fiMaQgxk2()) {
                ...
                //self.fiMaQgxk2(self.fiMaQgxk());
            }
        } else {
            self.fiMaQgxkList([]);
        }
    };
    self.fiMaQgxk2Change = function () {
        if (self.fiMaQgxk2()) {
            getCategory("CUAKHAU", self.fiMaQgxk2(), function (res) {
                if (res.success) {
                    self.fiMaCkxkList(mapCategory(res.data, "portcode", "portname"));
                }
            });
    ...
    self.showFiMaQgxk2Click = function () {
        var hide = self.showFiMaQgxk2();
        self.showFiMaQgxk2(!hide);
    };
    ...
    self.fiSoHd = ko.observable(hoso.fiSoHd).extend({
        maxLength: {message: '30 ký tự', params: 30}
    });
    ...
    self.fiMaQgxk2 = ko.observable(hoso.fiMaQgxk2);
    self.showFiMaQgxk2 = ko.observable(false);
    ...
    var hosoVG = [self.fiSoVandon, self.fiTenCty, self.fiDiachiCty, self.fiTenDtxk, self.fiMaQgxk, 
        self.fiTenQgxk, self.fiTenCkxk, self.fiTenDtnk, self.fiTenQgnk, self.fiPtvt, self.fiTenCknk, 
        self.fiMucdichSd, self.fiDdkd, self.fiNoiky, self.fiNguoikyfiSoVandon, self.fiTenCty, 
        self.fiDiachiCty, self.fiTenDtxk, self.fiMaQgxk, self.fiTenQgxk, self.fiTenCkxk, 
        self.fiTenDtnk, self.fiPtvt, self.fiTenCknk, self.fiMucdichSd, self.fiDdkd, 
        self.fiNoiky, self.fiNguoiky, self.fiNgayky];
    ...
    getCategory("QUOCGIA", null, function (res) {
        if (res.success) {
            self.fiMaQgxkList(mapCategory(res.data, "statecode", "name"));
            if (hoso.fiMaQgxk) {
                self.fiMaQgxk(hoso.fiMaQgxk);
                self.fiTenQgxk(hoso.fiTenQgxk);
            }
            if (hoso.fiMaQgxk2) {
                self.fiMaQgxk2(hoso.fiMaQgxk2);
            }
            if (self.fiMaQgxk2()) {
                getCategory("CUAKHAU", self.fiMaQgxk2(), function (res) {
                    if (res.success) {
