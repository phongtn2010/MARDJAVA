...
function Mard10CreateVM(options){
    ...
    if (self.mard10FormVM().fiIdHoso() > 0) {
        if (self.mard10FormVM().fiTrangthai() == TAO_MOI) {
            self.btnLuu(true);
        }
    } else {
        self.btnLuu(true);
    ...
    self.btnGuiClick = function(){
        if(self.tabFiLydoSua() && !self.fiLydoSua()){
            ...
            var url = !fiIdHoso || fiIdHoso <= 0 ? app.appContext + '/mard/10/hoso/taomoi' : (fiTrangthai == YEU_CAU_BO_SUNG || fiTrangthai == CHO_TIEP_NHAN || fiTrangthai == TAO_MOI || fiTrangthai == CHO_TIEP_NHAN_BO_SUNG ? app.appContext + '/mard/10/hoso/capnhap' : app.appContext + "/mard/10/hoso/yc-capnhap");
            $.ajax({
                async: true,
                type: 'POST',
                cache: false,
...
//                            }
                            if(fiTrangthai == YEU_CAU_BO_SUNG || fiTrangthai == CHO_TIEP_NHAN_BO_SUNG){
                                ...
                                //Update lai Id ho so moi tao
                                fiIdHoso = self.mard10FormVM().fiIdHoso();
                                data = {
                                    type : "16",
                                    "function" : "12",
                                    fiIdHoso : fiIdHoso,
