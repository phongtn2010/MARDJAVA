var AddGoodVM = function(data, onAddProduct) {
    ...
    self.onClose = function() {
        if (indexToUpdate > -1){
            self.listGood().forEach(function (goodVM) {
                if (goodVM.fiProductCode().indexOf(idEdit) === 0){
                    goodVM.fiNetWeight(goodVM.fiNetWeight() - netWeightSub);
                    goodVM.fiQtyFemale(goodVM.fiQtyFemale() - femaleSub);
                    goodVM.fiQtyMale(goodVM.fiQtyMale() - maleSub);
                    mapGoods[idEdit].fiNetWeight = goodVM.fiNetWeight();
                    mapGoods[idEdit].fiQtyFemale = goodVM.fiQtyFemale();
                    mapGoods[idEdit].fiQtyMale = goodVM.fiQtyMale();
                }
            });
        }
        indexToUpdate = -1;
    ...
    self.addProduct = function () {
        if (validateData()) {
            ...
            self.listGood().forEach(function (goodVM) {
                if (goodVM.fiProductCode().indexOf(self.fiProductCode()) === 0){
                    goodVM.fiNetWeight(goodVM.fiNetWeight() - self.fiNetWeight());
                    goodVM.fiQtyFemale(goodVM.fiQtyFemale() - self.fiQtyFemale());
                    goodVM.fiQtyMale(goodVM.fiQtyMale() - self.fiQtyMale());
                    mapGoods[self.fiProductCode()].fiNetWeight = goodVM.fiNetWeight();
                    mapGoods[self.fiProductCode()].fiQtyFemale = goodVM.fiQtyFemale();
                    mapGoods[self.fiProductCode()].fiQtyMale = goodVM.fiQtyMale();
                }
            });
            ...
            onAddProduct(data, indexToUpdate);
            indexToUpdate = -1;
    ...
    self.update = function(index, data) {
        indexToUpdate = index;
        idEdit = data.fiProductCode();

        self.listGood().forEach(function (goodVM) {
            if (goodVM.fiProductCode().indexOf(data.fiProductCode()) === 0){
                netWeightSub = Number(data.fiNetWeight());
                femaleSub = Number(data.fiQtyFemale());
                maleSub = Number(data.fiQtyMale());
                goodVM.fiNetWeight(Number(goodVM.fiNetWeight()) + Number(data.fiNetWeight()));
                goodVM.fiQtyFemale(Number(goodVM.fiQtyFemale()) + Number(data.fiQtyFemale()));
                goodVM.fiQtyMale(Number(goodVM.fiQtyMale()) + Number(data.fiQtyMale()));
                mapGoods[data.fiProductCode()].fiNetWeight = goodVM.fiNetWeight();
                mapGoods[data.fiProductCode()].fiQtyFemale = goodVM.fiQtyFemale();
                mapGoods[data.fiProductCode()].fiQtyMale = goodVM.fiQtyMale();
