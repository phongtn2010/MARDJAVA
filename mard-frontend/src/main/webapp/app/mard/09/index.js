    ...
    app.makeGet({
        ...
        success: function (res) {
            if (res.success) {
                ...
                var hasCNKD = (res.data.cnkd && res.data.cnkd.length > 0) ? true : false;
                res.data.cnkd.forEach(function (item) {
                    item.fiEditable = true;
...
function IndexVM(params) {
    ...
    self.fiQuarantineDepartmentCode = ko.observable(null);
    self.fiMonitoringDepartmentCode = ko.observable(null);
    ...
    self.searchAfterHasCatgory = function (page, pushState) {
        ...
        var data = {
            ...
            fiQuarantineDepartmentCode: self.fiQuarantineDepartmentCode(),
            fiMonitoringDepartmentCode: self.fiMonitoringDepartmentCode(),
            ...
            success: function (res) {
                ...
                if (res.success) {
                    ...
                    res.data.data.forEach(function (item) {
                        ...
                        item.isShowHS = [3, 7, 8, 9, 10, 11, 13, 14, 15, 16, 17, 18, 19, 20].includes(item.fiGSStatus) || [3, 7, 8, 9, 10, 11, 13, 14, 15, 16, 17, 18, 19, 20].includes(item.fiKDStatus)
