    ...
    self.getGiayXacNhan01 = function (gxn, cb) {
        ...
        app.makePost({
            ...
            success: function (d) {
                if (d && d.success) {
                    var gxn = d.data;
                    self.fiNgaycapGcn(new Date(gxn.fiNgayky));
                    self.fiCqCapGcn(gxn.fiCoquancap);
                    self.fiHieulucdenngay(new Date(gxn.fiHieulucdenngay));
                    self.fiHieuluctungay(new Date(gxn.fiHieuluctungay));
                    self.dsLoaiPheLieu(gxn.lstThongtinphelieu);
                    if (cb) {
                        cb();
                    }
                } else {
