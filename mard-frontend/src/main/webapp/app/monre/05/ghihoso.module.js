...
$(document).ready(function() {
	...
	function GhiHoSoViewModel(v1, v2, v3, v4, v5) {
		...
		self.save = function(guiHoSo, callback) {
			...
			if (yeuCauKySo && guiHoSo) {
				 CAPlugin.initPlugin();
   		     var cert = CAPlugin.getCert();
   		     if (cert == 'E04') {
   		    	return;
   		     }
   		     postData.serialNumber = cert;
			}
			if (yeuCauKySo && guiHoSo && (postData.serialNumber == null  || postData.serialNumber.trim() == '')) {
				return;
			}
			...
			app.makePost({
				...
				success : function(d) {
					...
					if (d.success) {
						...
						if (guiHoSo) {
							if (yeuCauKySo) {
								var sign = CAPlugin.signXml(d.data.xmlBody, d.data.serialNumber);
								 $('#xmlEnvelop').val(d.data.xmlEnvelop);
								 $('#xmlBody').val(sign);
							}
							callback();
						} else {
