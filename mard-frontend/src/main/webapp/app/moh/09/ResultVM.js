...
function TbdgcnHanghoa9(d){
    ...
    self.resultLable = ko.computed(function(){
        return self.fiDat() == 2 ? 'Đạt':'Không đạt';
    });
    
    self.fiTenPtkt = ko.computed(function(){
        return self.fiDat() == 2 ? 'Thường':'Chặt';
    });
};
...
function ResultVM(options) {
    ...
    self.fiNgayNkTuText = ko.observable(data !== null ? new Date(data.fiNgayNkTu).toDayFirstWithTime() : null);
    self.fifiNgayNkDenText = ko.observable(data !== null ? new Date(data.fiNgayNkDen).toDayFirstWithTime() : null);
