...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/moh/36/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdlichsu36(res.data) : [];
                    self.historyItems([]);
...
function IndexVM(params) {
    ...
    self.search = function (page, pushState) {
        if (!RAW_HS_STATUS || RAW_HS_STATUS.length <= 0) {
            app.getCategory('/moh/36/danhmuc', 'DANHMUC_TRANGTHAI', null, function (res) {
                if (res.success) {
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/moh/36/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
...
$(document).ready(function () {
    ...
    $('#loading10').show();
    $.when(app.getCategory('/moh/36/danhmuc', 'DANHMUC_TRANGTHAI', null, function (res) {
        if (res.success) {
