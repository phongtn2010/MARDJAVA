...
function HistoryVM(item) {
    ...
    self.search = function (cb) {
        ...
        app.makePost({
            url: '/moh/39/hoso/lichsu',
            data: JSON.stringify(data),
            success: function (res) {
                if (res.success) {
                    var list = res.data ? mapTbdlichsu39(res.data) : [];
                    self.historyItems([]);
...
function KetQuaXLVM(options) {
    ...
    if (options) {
        ...
        if(options.hasOwnProperty('fiLinkCvan')) {
            self.isVisibleLink((null != options.fiLinkCvan && options.fiLinkCvan.trim().length > 0) ? true : false);
        }
    }
}
...
function IndexVM(params) {
    ...
    self.search = function (page, pushState) {
        if (!RAW_HS_STATUS || RAW_HS_STATUS.length <= 0) {
            app.getCategory('/moh/39/danhmuc', 'DANHMUC_TRANGTHAI', null, function (res) {
                if (res.success) {
    ...
    self.searchAfterHasCatgory = function (page, pushState) {
        ...
        app.makePost({
            url: "/moh/39/hoso/timkiem",
            data: JSON.stringify(data),
            success: function (res) {
                window.stateChanging = false;
                if (res.success) {
                    self.Items([]);
                    var list = res.data ? mapTbdhoso39(res.data, data.currentPage * data.pageSize) : [];
                    self.Items(list);
                    self.paging().update({
                        totalCount: res.data ? res.total : 0,
                        pageSize: self.pageSize(),
    ...
    self.btnAddNew12Click = function (e) {
        document.location = app.appContext + '/moh/' + self.fiThuTucId() + '/12/edit?maThuTuc=' + self.maThuTuc();
        return false;
    };

    self.btnAddNew34Click = function (e) {
        document.location = app.appContext + '/moh/' + self.fiThuTucId() + '/34/edit?maThuTuc=' + self.maThuTuc();
        return false;
    };
    ...
    self.bXemClick = function (item, e) {
        if (null == item.fiLoaiDon()) {
            document.location = app.appContext + '/moh/' + self.fiThuTucId() + '/view?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        } else if (item.fiLoaiDon() <= 2) {
            document.location = app.appContext + '/moh/' + self.fiThuTucId() + '/12/view?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        } else {
            document.location = app.appContext + '/moh/' + self.fiThuTucId() + '/34/view?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        }
    ...
    self.bXemThongBaoClick = function (item, e) {
        ...
        if (null == item.fiLoaiDon()) {
            document.location = app.appContext + '/moh/' + self.fiThuTucId() + '/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=cert&maThuTuc=' + self.maThuTuc();
        } else if (item.fiLoaiDon() <= 2) {
            document.location = app.appContext + '/moh/' + self.fiThuTucId() + '/12/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=cert&maThuTuc=' + self.maThuTuc();
        } else {
            document.location = app.appContext + '/moh/' + self.fiThuTucId() + '/34/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=cert&maThuTuc=' + self.maThuTuc();
        }
        //document.location = app.appContext + '/moh/' + self.fiThuTucId() + '/view?fiIdHoso=' + item.fiIdHoso() + '&fiMaHoso=' + item.fiMaHoso() + '&tab=cert&maThuTuc=' + self.maThuTuc();
        return false;
    };
    ...
    self.bSuaClick = function (item, e) {
        if (null == item.fiLoaiDon()) {
            document.location = app.appContext + '/moh/' + self.fiThuTucId() + '/edit?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        } else if (item.fiLoaiDon() <= 2) {
            document.location = app.appContext + '/moh/' + self.fiThuTucId() + '/12/edit?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        } else {
            document.location = app.appContext + '/moh/' + self.fiThuTucId() + '/34/edit?fiIdHoso=' + item.fiIdHoso() + '&maThuTuc=' + self.maThuTuc();
        }
        return false;
    };
    /**
            ...
            buttons: [
                {
                    ...
                    action: function () {
                        app.makePost({
                            url: '/moh/39/hoso/xoa/' + item.fiIdHoso(),
                            data: JSON.stringify({}),
                            success: function (d) {
                                var msg = '';
                                if (d.success) {
...
$(document).ready(function () {
    ...
    $('#loading10').show();
    $.when(app.getCategory('/moh/39/danhmuc', 'DANHMUC_TRANGTHAI', null, function (res) {
        if (res.success) {
