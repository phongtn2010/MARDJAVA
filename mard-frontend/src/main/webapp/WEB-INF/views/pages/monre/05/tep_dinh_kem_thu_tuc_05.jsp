	...
	<tr>
		...
		<td class="td-left">
				<c:if test="<%= form != null && !form.isXemHoSo() && form.getIdHS() > 0 %>">
					<input type="file" id="fileUpload<%= 1 %>" multiple="multiple" accept=".jpg, .pdf, .doc, .tif"   data-bind=" event:{ change: $root.uploadFileChangeEvent.bind($data, <%= 1 %>, 'fileUpload<%= 1 %>') }"></div>
				</c:if>
			<div data-bind="visiable: false, foreach: { data: tepTinModel.tepLoai<%= 1 %>(), as: 'item' }">
		...
		<td align="center">
			<c:if test="<%= form != null && !form.isXemHoSo() %>">
				<p style="border: 0px solid;" align="center"><p data-bind="if: tepTinModel.tepLoai<%= 1 %>().length > 0">
