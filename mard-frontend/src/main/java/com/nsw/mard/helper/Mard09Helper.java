...
public class Mard09Helper {
    public static Variables genWord15A(Tbdgiaycnkd09 gp) {
        ...
        if (!TextUtils.isEmpty(gp.getFiParentDepartmentName())){
            parentDepartmentName = gp.getFiParentDepartmentName();
        }
    ...
    public static Variables genWord15B(Tbdgiaycnkd09 gp) {
        ...
        for (TbdCnkdHh09 hh: gp.getLstGood()) {
            if ("ton".equalsIgnoreCase(hh.getFiNWUnitCode())) {
                donVi = "Tấn";
            }
            total += hh.getFiNumber();
        ...
        variables.addTextVariable(new TextVariable("#{DonVi}", donVi));
