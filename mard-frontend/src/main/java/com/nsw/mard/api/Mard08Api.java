...
import com.nsw.mard.helper.Mard08Helper;
...
public class Mard08Api extends BaseApi {
    ...
    private @ResponseBody ResponseJson rollbackHoso(@RequestParam("nswFileCode") String nswFileCode) {
        ...
        try {
            rs = BackendRequestHelper.getInstance()
                    .doGetRequest(Mard08Constant.getInstance().getApiUrl(environment, Mard08Constant.API.ROLL_BACK) + "?nswFileCode=" + nswFileCode);
            return rs;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            rs.setData(null);
    ...
    ResponseJson createHoso(
            @RequestBody Tbdhoso08 tbdhoso08
    ) {
        return save(tbdhoso08);
    }
    ...
    ResponseJson sendHoso(
        ...
        try {
            if (!Constants.SIGN.ON.equals(environment.getRequiredProperty(Mard08Constant.EnableSign))) {
                returnJson = send(tbdhoso08);
            } else {
                tbdhoso08.setFiSignedDate(new Date());
                tbdhoso08.setFiTaxCode(getUsername());
                ResponseJson tmpJson = save(tbdhoso08);
                ...
                if (returnJson.isSuccess() && returnJson.getData() != null) {
                    ...
                    sendMessage.setType(Mard08Constant.TYPE.TYPE_10);

                    if (fiTrangthai.equals(Mard08Constant.HosoStatus.TAO_MOI)) {
                        sendMessage.setFunction(Mard08Constant.FUNCTION.FUNCTION_01);
                    } else if (fiTrangthai.equals(Mard08Constant.HosoStatus.CHO_TIEP_NHAN)) {
                        sendMessage.setFunction(Mard08Constant.FUNCTION.FUNCTION_02);
                    } else if (fiTrangthai.equals(Mard08Constant.HosoStatus.YEU_CAU_BO_SUNG)) {
                        sendMessage.setFunction(Mard08Constant.FUNCTION.FUNCTION_03);
                    }
    ...
    public void getHDSD(
        ...
        try {
            String templatePath = request.getSession().getServletContext().getRealPath("/WEB-INF/downloads/mard/08/hdsd.docx");
    ...
    @RequestMapping(value = "/hoso/edit", method = RequestMethod.PUT, headers = {"content-type=application/json"})
    public @ResponseBody
    ResponseJson editHoso(
        ...
        try {
            ...
            if (!Constants.SIGN.ON.equals(environment.getRequiredProperty(Mard08Constant.EnableSign))) {
                tbdhoso08.setFiSignedDate(new Date());
                tbdhoso08.setFiTaxCode(getUsername());
                return BackendRequestHelper.getInstance().doCustomRequest(Mard08Constant.getInstance().getApiUrl(environment, Mard08Constant.API.HOSO_EDIT), HttpMethod.POST, tbdhoso08, new HashMap<>());
            } else {
                tbdhoso08.setFiTaxCode(getUsername());
                tbdhoso08.setFiSignedDate(new Date());
                returnJson = saveBeforeUpdate(tbdhoso08);

                if (returnJson.isSuccess() && returnJson.getData() != null) {
                    HashMap<String, Object> data = (HashMap<String, Object>) returnJson.getData();
                    SendMessage sendMessage = new SendMessage();
                    sendMessage.setFiNSWFileCode(data.get("fiHSCode").toString());
                    sendMessage.setType(Mard08Constant.TYPE.TYPE_17);
                    sendMessage.setFunction(Mard08Constant.FUNCTION.FUNCTION_15);
                    sendMessage.setFiReason(tbdhoso08.getFiModifyReason());

                    SignData signData = getXMLForSign(sendMessage);
                    returnJson.setSign(signData);
                }

                returnJson.setData(tbdhoso08);
    ...
    ResponseJson requestCancelHoso(
            @RequestBody Tbdycrut08 tbdhoso08
    ) {
        ResponseJson returnJson = new ResponseJson();
        try {
            if (!isOwner(null, tbdhoso08.getFiHSCode())) {
                returnJson.setSuccess(false);
                return returnJson;
            }
            ResponseJson json = BackendRequestHelper.getInstance().doPostRequest(Mard08Constant.getInstance().getApiUrl(environment, Mard08Constant.API.HOSO_CANCEL), tbdhoso08);
            return json;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
    ...
    ResponseJson deleteHoso(
        ...
        try {
            ...
            ResponseJson json = BackendRequestHelper.getInstance().doGetRequest(Mard08Constant.getInstance().getApiUrl(environment, Mard08Constant.API.HOSO_DELETE) + "?fiNSWFileCode=" + fiNSWFileCode + "&fiTaxCode=" + fiTaxCode);
            return json;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
    ...
    ResponseJson getHosoByFilter(@RequestBody FilterForm tbdFindHosoFilter08) {
        ...
        try {
            tbdFindHosoFilter08.setFiCompanyTaxCode(getUsername());
    ...
    public ResponseJson createAttachment(
        ...
        try {
            ...
            if (!".jpg,.jpeg,.pdf,.tif,.doc,.docx,.zip,.xls,.xlsx".contains(format)) {
                ResponseJson responseJson = new ResponseJson();
                responseJson.setSuccess(false);
                responseJson.setMessage("Tệp đính kèm không đúng định dạng");
                return responseJson;
            }
            return attachmentService.uploadFile(file, fiMaLoai, "BNNPTNT0600011");
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
            RabbitMQErrorHelper.pushLogToRabbitMQ(errorInfo, getRabbitMQ());
    ...
    ResponseJson deleteAttachment(
        ...
        ResponseJson returnJson = new ResponseJson();
        try {
            ResponseJson json = BackendRequestHelper.getInstance().doCustomRequest(Mard08Constant.getInstance().getApiUrl(environment, Mard08Constant.API.ATTACHMENT_DELETE) + "?fiGuid=" + fiGuid, HttpMethod.DELETE, new Object(), new HashMap<>());
            return json;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
    ...
    ResponseJson getGiayCNKDByTaxCode(
            @RequestParam(name = "mst", required = true) String maSoThue
    ) {
        ResponseJson json = BackendRequestHelper.getInstance().doGetRequest(
                String.format("%s?fiTaxCode=%s", Mard08Constant.getInstance().getApiUrl(environment, Mard08Constant.API.GIAYPHEP_GET_BY_TAX_CODE), maSoThue)
        );
        return json;
    }
    ...
    ResponseJson readDataFromExcel(
        ...
        try {
            switch (type) {
                case "animal":
                    List<Tbdhanghoa08> listProduct = Mard08Helper.readAnimalFromExcel(file.getInputStream());
                    json.setSuccess(true);
                    json.setData(listProduct);
                    break;
                case "animal_product":
                    List<Tbdhanghoa08> listAnimalProduct = Mard08Helper.readAnimalProductFromExcel(file.getInputStream());
                    json.setSuccess(true);
                    json.setData(listAnimalProduct);
                    break;
                case "animal_20a":
                    List<Tbdhanghoa08> listAnimal20a = Mard08Helper.readAnimal20aFromExcel(file.getInputStream());
                    json.setSuccess(true);
                    json.setData(listAnimal20a);
                    break;
                case "animal_product_20a":
                    List<Tbdhanghoa08> listAnimalProduct20a = Mard08Helper.readAnimalProduct20aFromExcel(file.getInputStream());
                    json.setSuccess(true);
                    json.setData(listAnimalProduct20a);
                    break;
                case "exporter":
                    List<Tbdctyxk08> listExporter = Mard08Helper.readExporterFromExcel(file.getInputStream());
                    json.setSuccess(true);
                    json.setData(listExporter);
                    break;
                case "quaratine":
                    List<Tbdddclkd08> listQuarantine = Mard08Helper.readQuaratineFromExcel(file.getInputStream());
                    json.setSuccess(true);
                    json.setData(listQuarantine);
                    break;
                case "prodmfr":
                    List<Tbdcssxsp08> listProdMfr = Mard08Helper.readProdMfrFromExcel(file.getInputStream());
                    json.setSuccess(true);
                    json.setData(listProdMfr);
                    break;
                case "mfgfactory":
                    List<Tbdnmsx08> listMfgFactory = Mard08Helper.readMfgFactoryFromExcel(file.getInputStream());
                    json.setSuccess(true);
                    json.setData(listMfgFactory);
                    break;
                case "buyer":
                    List<Tbdctyxk08> listBuyer = Mard08Helper.readBuyerFromExcel(file.getInputStream());
                    json.setSuccess(true);
                    json.setData(listBuyer);
                    break;
            }
            return json;
        }
...
//        return this.verifySignature(token);
    ...
    private SignData getXMLForSign(SendMessage sendMessage) throws Exception {
        ResponseJson resultSignFlow = BackendRequestHelper.getInstance()
                .doPostRequest(Mard08Constant.getInstance().getApiUrl(environment, Mard08Constant.API.GET_XML), sendMessage);

        if (resultSignFlow.isSuccess()) {
            SignData signData = new SignData();
            signData.setFiDocumentCode(sendMessage.getFiNSWFileCode());
            signData.setFiDocType("BNNPTNT0600011");
    ...
    private ResponseJson send(Tbdhoso08 tbdHoso) {
        ResponseJson returnJson = new ResponseJson();
        tbdHoso.setFiSignedDate(new Date());
        tbdHoso.setFiTaxCode(getUsername());
        try {
            ResponseJson json = BackendRequestHelper.getInstance().doPostRequest(Mard08Constant.getInstance().getApiUrl(environment, Mard08Constant.API.HOSO_SEND), tbdHoso);
            return json;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
    ...
    private ResponseJson save(Tbdhoso08 tbdHoso) {
        ResponseJson returnJson = new ResponseJson();
        try {
            ResponseJson json = BackendRequestHelper.getInstance().doPostRequest(Mard08Constant.getInstance().getApiUrl(environment, Mard08Constant.API.HOSO_CREATE), tbdHoso);
            return json;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
    ...
    private ResponseJson saveBeforeUpdate(Tbdhoso08 tbdHoso) {
        try {
            tbdHoso.setFiSignedDate(new Date());
            tbdHoso.setFiTaxCode(getUsername());
            ...
            return BackendRequestHelper.getInstance()
                    .doPostRequest(
                            Mard08Constant.getInstance()
                                    .getApiUrl(environment, Mard08Constant.API.SAVE_BEFORE_UPDATE),
                            tbdHoso);
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
    ...
    ResponseJson guiYeucauSuaHosoDaKy(
        ...
        try {
            tbdHoso.setFiSignedDate(new Date());
            tbdHoso.setFiTaxCode(getUsername());
            return BackendRequestHelper.getInstance().doPostRequest(Mard08Constant.getInstance().getApiUrl(environment, Mard08Constant.API.UPDATE_ONLY), tbdHoso);
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
            RabbitMQErrorHelper.pushLogToRabbitMQ(errorInfo, getRabbitMQ());
...
//
    ...
    private boolean isOwner(String idHS, String nswFileCode) {
        ResponseJson json = BackendRequestHelper.getInstance().doGetRequest(
                Mard08Constant.getInstance().getApiUrl(
                        environment,
                        Mard08Constant.API.HOSO_GET_BY_FILTER
    ...
    private Variables genKDNK(Tbdcvcnkd08 cnkd) {
        ...
        if (cnkd.getFiSignConfirmTitle() != null) {
            if ("cục trưởng".equalsIgnoreCase(cnkd.getFiSignConfirmTitle().trim())) {
    ...
    private Variables genVSTY(Tbdcvvsty08 vsty) {
        ...
        if (vsty.getFiSignConfirmTitle() != null) {
            if ("cục trưởng".equalsIgnoreCase(vsty.getFiSignConfirmTitle().trim())) {
