...
import com.nsw.mard.helper.Mard01Helper;
...
public class Mard01Api extends BaseApi {
    ...
    protected String getUsername() {
        //return taxcode on FE, return ECUS on FCAP
    ...
    ResponseJson getHosoById(@RequestParam("id") String id) {
        ...
        try {
            json = BackendRequestHelper.getInstance().doGetRequest(
                    Mard01Constant.getInstance().getApiUrl(
                            environment,
                            Mard01Constant.API.HOSO_GET_BY_FILTER
                    ) + "?id=" + id + "&taxCode=" + getUsername());
            return json;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
    ...
    ResponseJson sendHoso(
            @RequestBody Tbdhoso01 tbdhoso01
    ) {
        ResponseJson returnJson = new ResponseJson();
        returnJson.setSuccess(false);
        try {
            if (!Constants.SIGN.ON.equals(environment.getRequiredProperty(Mard01Constant.EnableSign))) {
                returnJson = send(tbdhoso01);
            } else {
                ResponseJson tmpJson = save(tbdhoso01);
                ...
                if (returnJson.isSuccess() && returnJson.getData() != null) {
                    ...
                    sendMessage.setType(Mard01Constant.MSG_TYPE.TYPE_10);

                    if (fiTrangthai.equals(Mard01Constant.FileStatus.TAO_MOI)) {
                        sendMessage.setFunction(Mard01Constant.MSG_FUNC.FUNC_01);
                    } else if (fiTrangthai.equals(Mard01Constant.FileStatus.CHO_TIEP_NHAN)) {
                        sendMessage.setFunction(Mard01Constant.MSG_FUNC.FUNC_02);
                    } else if (fiTrangthai.equals(Mard01Constant.FileStatus.YEU_CAU_SUA_DOI_BO_SUNG)) {
                        sendMessage.setFunction(Mard01Constant.MSG_FUNC.FUNC_03);
                    }
    ...
    @RequestMapping(value = "/hoso/cancelCer", method = RequestMethod.POST, headers = {"content-type=application/json"})
    public @ResponseBody
    ResponseJson ychgcn(
            @RequestBody YeuCauHuyGCN yeuCauHuyGCN
    ) {
        ResponseJson returnJson = new ResponseJson();
        try {
            if (!isOwner(null, yeuCauHuyGCN.getFiNSWFileCode()))  {
                returnJson.setSuccess(false);
                returnJson.setMessage("Không có quyền truy cập hồ sơ");
                return returnJson;
            }
            yeuCauHuyGCN.setFiRequestDate(new Date());
            ResponseJson json = BackendRequestHelper.getInstance().doPostRequest(
                    Mard01Constant.getInstance().getApiUrl(environment, Mard01Constant.API.YEUCAUHUY_GCN),
                    yeuCauHuyGCN);
            return json;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
    ...
    ResponseJson createHoso(
        ...
        return save(tbdhoso01);
    }


    @RequestMapping(value = "/hoso/ycs", method = RequestMethod.POST, headers = {"content-type=application/json"})
    public @ResponseBody
    ResponseJson yeucauSuaHoso(
            @RequestBody YeucauSuaHoso yeucauSuaHoso
    ) {
        ResponseJson returnJson = new ResponseJson();
        try {
            if (!Constants.SIGN.ON.equals(environment.getRequiredProperty(Mard01Constant.EnableSign))) {
                yeucauSuaHoso.setFiRequestDate(new Date());
                yeucauSuaHoso.getFiRegistrationProfile().setFiTaxCode(getUsername());
                returnJson = BackendRequestHelper.getInstance().doPostRequest(Mard01Constant.getInstance().getApiUrl(environment, Mard01Constant.API.YCS_HOSO), yeucauSuaHoso);
                return returnJson;
            } else {
                returnJson = saveBeforeUpdate(yeucauSuaHoso);

                if (returnJson.isSuccess() && returnJson.getData() != null) {
                    HashMap<String, Object> data = (HashMap<String, Object>) returnJson.getData();
                    SendMessage sendMessage = new SendMessage();
                    sendMessage.setFiNSWFileCode(data.get("fiNSWFileCode").toString());
                    sendMessage.setType(Mard01Constant.MSG_TYPE.TYPE_14);
                    sendMessage.setFunction(Mard01Constant.MSG_FUNC.FUNC_08);

                    SignData signData = getXMLForSign(sendMessage);
                    returnJson.setSign(signData);
                }
                returnJson.setData(yeucauSuaHoso);
    ...
    private ResponseJson saveBeforeUpdate(YeucauSuaHoso yeucauSuaHoso) {
        try {
            yeucauSuaHoso.setFiRequestDate(new Date());
            yeucauSuaHoso.getFiRegistrationProfile().setFiTaxCode(getUsername());
            ...
            return BackendRequestHelper.getInstance()
                    .doPostRequest(
                            Mard01Constant.getInstance()
                                    .getApiUrl(environment, Mard01Constant.API.SAVE_BEFORE_UPDATE),
                            yeucauSuaHoso);
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
    ...
    ResponseJson guiYeucauSuaHosoDaKy(
        ...
        try {
            yeucauSuaHoso.setFiRequestDate(new Date());
            yeucauSuaHoso.getFiRegistrationProfile().setFiTaxCode(getUsername());
            returnJson = BackendRequestHelper.getInstance().doPostRequest(Mard01Constant.getInstance().getApiUrl(environment, Mard01Constant.API.UPDATE_ONLY), yeucauSuaHoso);
            return returnJson;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
    ...
    @RequestMapping(value = "/hoso/xinrut", method = RequestMethod.POST, headers = {"content-type=application/json"})
    public @ResponseBody
    ResponseJson xinRut(
        ...
        ResponseJson returnJson = new ResponseJson();
        try {
            if (!isOwner(xinRutHoso.getFiIdHS().toString(), xinRutHoso.getFiNSWFileCode())) {
                returnJson.setSuccess(false);
                returnJson.setMessage("Không có quyền truy cập hồ sơ");
                return returnJson;
            }
            xinRutHoso.setFiRequestedDate(new Date());
            ResponseJson json = BackendRequestHelper.getInstance().doPostRequest(Mard01Constant.getInstance().getApiUrl(environment, Mard01Constant.API.XINRUT_HOSO), xinRutHoso);
            return json;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
    ...
    @RequestMapping(value = "/hoso/xinrutttn", method = RequestMethod.POST, headers = {"content-type=application/json"})
    public @ResponseBody
    ResponseJson xinRutTTN(
            @RequestBody XinRutHoso xinRutHoso
    ) {
        ResponseJson returnJson = new ResponseJson();
        try {
            ...
            xinRutHoso.setFiRequestedDate(new Date());
            ResponseJson json = BackendRequestHelper.getInstance().doPostRequest(Mard01Constant.getInstance().getApiUrl(environment, Mard01Constant.API.XINRUT_HOSO_TTN), xinRutHoso);
            return json;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
    ...
    private @ResponseBody ResponseJson rollbackHoso(@RequestParam("nswFileCode") String nswFileCode) {
        ...
        try {
            rs = BackendRequestHelper.getInstance()
                    .doGetRequest(Mard01Constant.getInstance().getApiUrl(environment, Mard01Constant.API.ROLL_BACK) + "?nswFileCode=" + nswFileCode);
            return rs;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            rs.setData(null);
    ...
    @RequestMapping(value = "/gcn/xinsua", method = RequestMethod.POST)
    public @ResponseBody
    ResponseJson xinSuaGCN(
            @RequestParam(required = false) MultipartFile file,
            @RequestParam String fiNSWFileCode,
            @RequestParam(required = false) String fiFileName,
            @RequestParam String fiReason
    ) {
        XinsuaGiayphep xinsuaGiayphep = new XinsuaGiayphep();
        ResponseJson json = new ResponseJson();
        try {
            ...
            if (file != null && file.getSize() > 0) {
                ...
                xinsuaGiayphep.setFiFileAttack(encodeFileToBase64Binary(uploadFile));
                xinsuaGiayphep.setFiFileName(fiFileName);
            }

            xinsuaGiayphep.setFiNSWFileCode(fiNSWFileCode);
            xinsuaGiayphep.setFiReason(fiReason);
            xinsuaGiayphep.setFiRequestDate(new Date());

            json = BackendRequestHelper.getInstance().doPostRequest(Mard01Constant.getInstance().getApiUrl(environment, Mard01Constant.API.YEUCAUSUA_GCN), xinsuaGiayphep);
            return json;
        } catch (Exception e) {
            LogUtil.addLog(e);
            json.setSuccess(false);
    ...
    public void downloadDinhkem(
            ...
            @RequestParam(name = "mode", required = false) String mode) {
        try {
            String url = String.format("%s?id=%s&taxcode=%s", Mard01Constant.getInstance().getApiUrl(environment, Mard01Constant.API.DOWNLOAD_DINHKEM), fiIdDinhkem, getUsername());
    ...
    public void downloadFeeAttachment(
            ...
            @PathVariable("fiIdDinhkem") Long fiIdDinhkem) {
        try {
            String url = String.format("%s?id=%s&taxcode=%s", Mard01Constant.getInstance().getApiUrl(environment, Mard01Constant.API.DOWNLOAD_DINHKEM_FEE), fiIdDinhkem, getUsername());
            ResponseJson json = BackendRequestHelper.getInstance().doGetRequest(url);
    ...
    public ResponseJson createAttachment(
        ...
        try {
            ...
            return attachmentService.uploadFile(file, fiMaLoai, "BNNPTNT0600004");
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
            RabbitMQErrorHelper.pushLogToRabbitMQ(errorInfo, getRabbitMQ());
    ...
    ResponseJson deleteHoso(
        ...
        try {
            ...
            ResponseJson json = BackendRequestHelper.getInstance().doGetRequest(Mard01Constant.getInstance().getApiUrl(environment, Mard01Constant.API.HOSO_DELETE) + "?fiNSWFileCode=" + fiNSWFileCode + "&fiTaxCode=" + fiTaxCode);
            return json;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
    ...
    ResponseJson readDataFromExcel(
        ...
        try {
            switch (type) {
                case "animal":
                    List<TbdAnimal01> listAnimal = Mard01Helper.readAnimalFromExcel(file.getInputStream());
                    json.setSuccess(true);
                    json.setData(listAnimal);
                    break;
                case "animal_product":
                    List<TbdAnimalProduct01> listAnimalProduct = Mard01Helper.readAnimalProductFromExcel(file.getInputStream());
                    json.setSuccess(true);
                    json.setData(listAnimalProduct);
                    break;
                case "test":
                    List<TbdTest01> listTest = Mard01Helper.readTestFromExcel(file.getInputStream());
                    json.setSuccess(true);
                    json.setData(listTest);
                    break;
                case "vaccine":
                    List<TbdVaccin01> listVaccin = Mard01Helper.readVaccinFromExcel(file.getInputStream());
                    json.setSuccess(true);
                    json.setData(listVaccin);
                    break;
            }
            return json;
        } catch (Exception ex) {
    ...
    public void getHDSD(
        ...
        try {
            String templatePath = request.getSession().getServletContext().getRealPath("/WEB-INF/downloads/mard/01/hdsd.docx");
...
//        return this.verifySignature(token);
    ...
    private SignData getXMLForSign(SendMessage sendMessage) throws Exception {
        ResponseJson resultSignFlow = BackendRequestHelper.getInstance()
                .doPostRequest(Mard01Constant.getInstance().getApiUrl(environment, Mard01Constant.API.GET_XML), sendMessage);

        if (resultSignFlow.isSuccess()) {
            SignData signData = new SignData();
            signData.setFiDocumentCode(sendMessage.getFiNSWFileCode());
            signData.setFiDocType("BNNPTNT0600004");
    ...
    private ResponseJson save(Tbdhoso01 tbdhoso01) {
        try {
            tbdhoso01.setFiSignDate(new Date());
            tbdhoso01.setFiTaxCode(getUsername());
            tbdhoso01.setFiRegistrationNo("");

            ResponseJson json = BackendRequestHelper.getInstance()
                    .doPostRequest(
                            Mard01Constant.getInstance()
                                    .getApiUrl(environment, Mard01Constant.API.CREATE_HOSO),
                            tbdhoso01);
            return json;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
    ...
    private ResponseJson send(Tbdhoso01 tbdhoso01) {
        ResponseJson returnJson = new ResponseJson();
        try {
            tbdhoso01.setFiSignDate(new Date());
            tbdhoso01.setFiTaxCode(getUsername());
            tbdhoso01.setFiRegistrationNo("");
            ResponseJson json = BackendRequestHelper.getInstance().doPostRequest(Mard01Constant.getInstance().getApiUrl(environment, Mard01Constant.API.SEND_HOSO), tbdhoso01);
            return json;
        } catch (Exception ex) {
            LogUtil.addLog(ex);
            String errorInfo = AppConstant.APP_NAME + AppConstant.MESSAGE_SEPARATOR + TAG + AppConstant.MESSAGE_SEPARATOR + Thread.currentThread().getStackTrace()[1].getMethodName() + AppConstant.MESSAGE_SEPARATOR + ex.toString();
    ...
    private boolean isOwner(String idHS, String nswFileCode) {
        ResponseJson json = BackendRequestHelper.getInstance().doGetRequest(
                Mard01Constant.getInstance().getApiUrl(
                        environment,
                        Mard01Constant.API.HOSO_GET_BY_FILTER
                ) + "?id=" + idHS + "&nswFileCode=" + nswFileCode + "&taxCode=" + getUsername());
...
//            String tempFoleder = "/Users/v/Documents/temp/";
            String fileName = "";
            File tempFile;
            Docx docx;
            // preparing variables
    ...
    private Variables gen13B(Tbdcnkd13b01 gp){
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
        Variables variables = new Variables();
        String space = ".....";
