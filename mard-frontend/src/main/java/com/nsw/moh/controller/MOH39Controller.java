...
public class MOH39Controller extends BaseController {
    ...
    @RequestMapping(value = {MOH39Constant.View.RESULT_TYPE}, method = RequestMethod.GET)
    public String resultByType(@PathVariable String code, @PathVariable String type, ModelMap model) {
        Object principal = SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        if (principal instanceof UserDetails) {
            UserCustom user = (UserCustom) principal;
            initData(model, user, code);
        ...
        String page = getPageByType(code, type, MOH39Constant.ViewType.RESULT);
        return page;
    }
     ...
     * @param user
     */
    private void initData(ModelMap model, UserCustom user, String code) {
        ...
        model.addAttribute(AppConstant.DanhMuc.Sign, environment.getRequiredProperty(String.format(MOH39Constant.ENABLE_SIGN_PARTTERN, code)));
        ...
        String uploadKey = String.format(MOH39Constant.UPLOAD_PATTERN_TYPE, code);
        if(environment.containsProperty(uploadKey)) {
            model.addAttribute(AppConstant.DanhMuc.UPLOAD_URL, environment.getRequiredProperty(uploadKey));
        }
    }
