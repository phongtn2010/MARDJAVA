...
public class Mic03Api extends Mic03CallBack  {
	...
	public ResponseEntity<ResponseJson03> send(@RequestBody @Valid SendMessage03 sendMessage03) {
		...
		try {
			...
			responseJson03.setSuccess(responseJson03.isSuccess());
		} catch (Exception e) {
	...
	@RequestMapping(value = "/save/{idHoSo}/update/{tongSoTenXBP}/{tongSoBan}", method = RequestMethod.POST)
	public ResponseEntity<ResponseJson> capNhatTongSoTenXBP(@PathVariable("idHoSo") Long idHoSo, @PathVariable("tongSoTenXBP") Integer tongSoTenXBP, @PathVariable("tongSoBan") Integer tongSoBan) {

		ResponseJson responseJson = new ResponseJson();

		if (idHoSo > 0) {
			TbdHoSo03DTO tbdHoSo03DTO = fldMic03TbdHoSo03Api.getTbdHoSo03(idHoSo).getBody();
			tbdHoSo03DTO.setFiTongSoTenXBP(tongSoTenXBP);
			tbdHoSo03DTO.setFiTongSoXBPham(tongSoBan);
			fldMic03TbdHoSo03Api.updateTbdHoSo03(idHoSo, tbdHoSo03DTO);
		}
		responseJson.setSuccess(true);
		return ResponseEntity.ok().body(responseJson);
	}
