...
@Controller
@RequestMapping(value = "/mic/04")
public class Mic04Controller extends Mic04CallBack {

    public static final Logger LOGGER = LoggerFactory.getLogger(Mic04Controller.class);

    private static final String REDIRECT_HOME_INDEX = "redirect:/mic/04/home";
    ...
    @Autowired
    private Mic04TepDinhKemApi fldMic04TepDinhKemApi;

    @Autowired
    private Mic04TbdNguoiThamDinh04Api fldMic04TbdNguoiThamDinh04Api;
    ...
    private long cloneHoSo(final long id) {
        TbdHoSo04DTO tbdHoSo04 = fldMic04TbdHoSo04Api.getTbdHoSo04(id).getBody();
        Assert.notNull(tbdHoSo04, "KHONG TIM THAY HO SO ID = " + id);
    ...
    private void addAttribute(ModelMap model, long idHoSo, int action) {
        ...
        model.addAttribute("documentType", Mic04Constant.DOCUMENT_TYPE);
        ...
        model.addAttribute("complieTemplateCode", "04");
        model.addAttribute("pathAPI", "/mic/api/04");
    ...
    private void setBreadcrumb(boolean isDetail, ModelMap model) {
        ...
        Breadcrumb nav = new Breadcrumb("/mic/04/home", environment.getRequiredProperty(AppConstant.Breadcrumb.LIST));
        breadcrumbs.add(nav);
        if (isDetail) {
    ...
    private boolean isHasEditPermission(Long id) {
        ...
        try {
            ...
            if (StringUtils.hasText(createrName) && !Objects.equals(createrName, tbdHoSo04.getFiTaxCode())) {
                LOGGER.info("BAN KHONG PHAI LA NGUOI TAO RA HO SO: {} - NGUOI TAO: {} - USER: {}", tbdHoSo04.getFiIdHoSo(), tbdHoSo04.getFiTaxCode(), createrName);
                return false;
            }
            if (Arrays.asList(0, 1, 4, 5).contains(tbdHoSo04.getFiStatus())) return true;

            LOGGER.error("BAN KHONG CO QUYEN CHINH SUA HO SO NAY: ID HO SO = {}", tbdHoSo04.getFiIdHoSo());
            return false;
         } catch (Exception e) {
            LOGGER.error(e.getMessage(), e);
            return false;
