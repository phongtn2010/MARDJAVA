...
public class SBV01LoaiTienTe1BackendUtil {

	private static final Logger LOGGER = LoggerFactory.getLogger(SBV01LoaiTienTe1BackendUtil.class);

	private SBV01LoaiTienTe1BackendUtil() {
	}

	public static LoaiTienTe1 getLoaiTienTe1(String url, long primaryKey) {
		...
		try {
			...
			if (responseJson.getData() != null) {
				return mapper.readValue(mapper.writeValueAsString(responseJson.getData()), LoaiTienTe1.class);
			}
		} catch (Exception e) {
	...
	public static List<LoaiTienTe1> getAllLoaiTienTe1s(String url) {
		...
		try {
			...
			ObjectMapper mapper = new ObjectMapper();
			if (responseJson.getData() != null) {
				return mapper.readValue(mapper.writeValueAsString(responseJson.getData()), new TypeReference<List<LoaiTienTe1>>(){});
			}
		} catch (Exception e) {
			LOGGER.error(e.getMessage(), e);
		}
